...
# Some toolchain/helper combinations stage autotools installs under usr/local.
# Normalize into CURRENT_PACKAGES_DIR so downstream fixups can use standard paths.
if(VCPKG_TARGET_IS_OSX AND EXISTS "${CURRENT_PACKAGES_DIR}/usr/local")
    foreach(_dir bin include lib sbin share)
        if(EXISTS "${CURRENT_PACKAGES_DIR}/usr/local/${_dir}")
            if(EXISTS "${CURRENT_PACKAGES_DIR}/${_dir}")
                file(COPY "${CURRENT_PACKAGES_DIR}/usr/local/${_dir}/" DESTINATION "${CURRENT_PACKAGES_DIR}/${_dir}")
                file(REMOVE_RECURSE "${CURRENT_PACKAGES_DIR}/usr/local/${_dir}")
            else()
                file(RENAME "${CURRENT_PACKAGES_DIR}/usr/local/${_dir}" "${CURRENT_PACKAGES_DIR}/${_dir}")
            endif()
        endif()
    endforeach()
...
if(VCPKG_TARGET_IS_LINUX OR VCPKG_TARGET_IS_OSX AND "tools" IN_LIST FEATURES)
    set(ICU_SBIN_DIR "${CURRENT_PACKAGES_DIR}/tools/icu/sbin")
    if(VCPKG_TARGET_IS_OSX AND NOT EXISTS "${ICU_SBIN_DIR}" AND EXISTS "${CURRENT_PACKAGES_DIR}/sbin")
        set(ICU_SBIN_DIR "${CURRENT_PACKAGES_DIR}/sbin")
    endif()

    vcpkg_copy_tools(
        TOOL_NAMES icupkg gennorm2 gencmn genccode gensprep
        SEARCH_DIR "${ICU_SBIN_DIR}"
        DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin"
    )

    # Some layouts install icu-config to bin instead of tools/icu/bin.
    if(VCPKG_TARGET_IS_OSX AND EXISTS "${CURRENT_PACKAGES_DIR}/bin/icu-config" AND NOT EXISTS "${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin/icu-config")
        file(COPY "${CURRENT_PACKAGES_DIR}/bin/icu-config" DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin")
    endif()
endif()
