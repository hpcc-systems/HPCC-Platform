...
if(VCPKG_TARGET_IS_WINDOWS)
    # Autoconf AC_SEARCH_LIBS cache value is either "none required" or a library name.
...
set(_jemalloc_usr_local "${CURRENT_PACKAGES_DIR}/usr/local")
if(IS_DIRECTORY "${_jemalloc_usr_local}")
    foreach(_dir bin include lib share)
        if(IS_DIRECTORY "${_jemalloc_usr_local}/${_dir}")
            file(MAKE_DIRECTORY "${CURRENT_PACKAGES_DIR}/${_dir}")
            file(COPY "${_jemalloc_usr_local}/${_dir}/" DESTINATION "${CURRENT_PACKAGES_DIR}/${_dir}")
            file(REMOVE_RECURSE "${_jemalloc_usr_local}/${_dir}")
        endif()
    endforeach()
endif()
...
# Some autotools builds place pkg-config files under usr/local even when vcpkg
# sets the package prefix. Move them where vcpkg_fixup_pkgconfig() expects them.
set(_pc_rel_path "lib/pkgconfig/jemalloc.pc")
if(EXISTS "${CURRENT_PACKAGES_DIR}/usr/local/${_pc_rel_path}")
    file(MAKE_DIRECTORY "${CURRENT_PACKAGES_DIR}/lib/pkgconfig")
    file(RENAME "${CURRENT_PACKAGES_DIR}/usr/local/${_pc_rel_path}" "${CURRENT_PACKAGES_DIR}/${_pc_rel_path}")
endif()

if(NOT VCPKG_BUILD_TYPE AND EXISTS "${CURRENT_PACKAGES_DIR}/debug/usr/local/${_pc_rel_path}")
    file(MAKE_DIRECTORY "${CURRENT_PACKAGES_DIR}/debug/lib/pkgconfig")
    file(RENAME "${CURRENT_PACKAGES_DIR}/debug/usr/local/${_pc_rel_path}" "${CURRENT_PACKAGES_DIR}/debug/${_pc_rel_path}")
endif()
