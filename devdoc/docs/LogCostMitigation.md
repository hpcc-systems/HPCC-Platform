...
- Summarize multiple related messages into a single entry.
    ...
    LOG("Finished processing request request")
    ```
    After:
    ```c++
...
- Utilize the metrics framework to report quantitative performance or resource related values.
    ...
    After:
    ```c++
    std::shared_ptr xResourceAvailable = std::make_shared("AvailableXResource", "Available X Resource at a given time", SMeasureCount);
    std::shared_ptr xResourceUsed = std::make_shared("UsedXResource", " Utilized X Resource at a given time ", SMeasureCount);
