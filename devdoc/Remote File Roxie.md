...
Each of these environments is configured to have pull the files from the FileServer, but share the target location so no files are actually copied when packages are deployed.  ?Should queries avoid copying?
