...
## Update Frequency and Collection Patterns

Statistics in HPCC Platform are updated according to different patterns based on their nature and use cases. Understanding these patterns is crucial for proper interpretation and real-time monitoring decisions.

### Update Frequency Categories

The frequency patterns shown in the Statistics Reference table indicate when and how often each statistic is updated. These patterns are driven by the underlying system architecture and performance considerations.

#### Real-Time Updates

**Milestone-Based Updates (At milestone):**
- **Trigger:** Immediate when significant events occur
- **Examples:** Timestamps (StWhen* statistics), activity start/stop
- **Frequency:** Once per event
- **Use Cases:** Timeline analysis, critical path identification
- **Monitoring:** Suitable for real-time dashboards

**State Transition Updates (On start/stop, On completion):**
- **Trigger:** Activity lifecycle transitions
- **Examples:** Activity start/stop counters, execution times
- **Frequency:** Once per state change
- **Use Cases:** Activity lifecycle tracking, completion monitoring
- **Monitoring:** Good for status tracking

#### Batched Updates

**Thor Progress Updates (Every 30 seconds):**
- **Trigger:** Thor's watchdog progress reporting mechanism
- **Examples:** Row counts, disk I/O sizes, index operations
- **Frequency:** 30-second intervals during execution
- **Configuration:** `watchdogProgressInterval` in Thor configuration
- **Use Cases:** Progress monitoring, throughput analysis
- **Monitoring:** Suitable for progress bars, trend analysis

**ECL Agent Monitoring:**
- **Cost Check (Every 120 seconds):** Workunit cost limit verification
- **Abort Check (Every 30 seconds):** Cancellation and timeout monitoring
- **Configuration:** `CHECK_COST_INTERVAL`, `ABORT_CHECK_INTERVAL`

**Roxie Monitoring:**
- **Socket Monitoring (Every 5 seconds):** Network I/O health checks
- **Timeout Check (Every 1 second):** Request timeout monitoring
- **Configuration:** `socketCheckInterval`, `timeoutCheckInterval`

#### Event-Driven Updates

**Per-Operation Updates:**
- **Trigger:** Each discrete operation occurrence
- **Examples:** Cache hits/misses, individual I/O operations
- **Frequency:** Real-time during operation
- **Serialization:** Accumulated and reported at graph end
- **Use Cases:** Detailed operation analysis

**Resource-Based Updates:**
- **Memory Allocations:** Per allocation event (reported at graph end)
- **Cache Operations:** Per access (accumulated for efficiency)
- **Network Operations:** Per socket operation (with monitoring intervals)
- **Error Conditions:** Per occurrence (immediate for critical errors)

#### Completion-Based Updates

**Graph End Updates (At graph end):**
- **Trigger:** Graph execution completion
- **Examples:** Peak memory, system resource consumption, final totals
- **Frequency:** Once per graph execution
- **Use Cases:** Post-execution analysis, resource summaries
- **Monitoring:** Historical analysis only

**Activity Completion Updates:**
- **Trigger:** Individual activity completion
- **Examples:** Total execution times, final counts
- **Frequency:** Once per activity
- **Use Cases:** Activity performance analysis

### Update Frequency Impact on Monitoring

#### Real-Time Monitoring Suitability

**Excellent for Real-Time (< 5s latency):**
- Milestone timestamps (StWhen* statistics)
- Activity state changes (start/stop)
- Error conditions and failures
- Critical resource alerts

**Good for Near Real-Time (5-30s latency):**
- Roxie socket monitoring (5s intervals)
- Progress indicators (30s Thor intervals)  
- Throughput trends (row processing rates)
- Basic performance metrics

**Poor for Real-Time (> 30s latency):**
- Detailed I/O statistics (30s+ batching)
- Cache performance metrics (graph end reporting)
- Memory usage details (completion-based)
- Cost accumulation (120s intervals)

#### Historical Analysis Capabilities

**Trend Analysis Granularity:**
```
Statistic Type          | Update Frequency    | Trend Granularity
--------------------|---------------------|------------------
Row Throughput      | 30s Thor progress   | 30-second intervals
Memory Usage        | Graph end           | Per-query only  
Cache Hit Ratios    | Graph end           | Per-query only
Network I/O         | 5s Roxie monitoring | 5-second intervals
Cost Accumulation   | 120s cost check     | 2-minute intervals
```
...
**Thor Progress Interval:**
```xml
<!-- In Thor configuration -->
<Software>
    <ThorCluster>
        <Thor watchdogProgressInterval="30"/>  <!-- seconds -->
    </Thor>
</ThorCluster>
</Software>
```
...
**ECL Agent Monitoring:**
```cpp
// In eclagent configuration  
#define CHECK_COST_INTERVAL 120000    // 120 seconds in ms
#define ABORT_CHECK_INTERVAL 30000    // 30 seconds in ms
```

**Roxie Monitoring:**
```xml
<!-- In Roxie configuration -->
<Software>
    <RoxieCluster>
        <Roxie socketCheckInterval="5"/>     <!-- seconds -->
        <Roxie timeoutCheckInterval="1"/>    <!-- seconds -->
    </Roxie>
</RoxieCluster>
</Software>
```
...
**High-Frequency Update Impact:**
- More frequent updates provide better monitoring granularity
- But increase system overhead and network traffic
- Balance monitoring needs with performance impact

**Batching Benefits:**
- Reduces system overhead for high-volume statistics
- Provides smoother trend analysis (less noise)
- Enables efficient cross-node aggregation

**Memory and Storage Impact:**
- Frequent updates require more memory for intermediate storage
- Graph-end reporting minimizes long-term storage requirements
- Consider retention policies for high-frequency data

### Update Patterns by Component

#### Thor (Batch Processing Engine)

**Progress Reporting (30s intervals):**
- Row processing counts and rates
- Disk I/O volumes and operation counts  
- Index operation statistics
- Memory spill events

**Completion Reporting:**
- Final execution times and resource usage
- Peak memory consumption
- Total operation counts

#### Roxie (Real-Time Query Engine)  

**Real-Time Monitoring (1-5s intervals):**
- Socket health and network performance
- Request queue lengths and wait times
- Active connection counts

**Per-Request Reporting:**
- Individual query execution times
- Agent processing and queue times
- Request/response sizes

#### ECL Agent (Workunit Management)

**Cost Monitoring (120s intervals):**
- Resource cost accumulation
- Budget limit checking
- Billing threshold alerts

**Abort Monitoring (30s intervals):**
- Timeout detection
...
**Request Processing:**
- SOAP call timing and success rates
...
- Focus on milestone and state-transition statistics
- Use Roxie 5s intervals for near real-time trends
- Avoid graph-end statistics for live monitoring

**For Performance Analysis:**
- Use 30s Thor intervals for throughput analysis
- Combine with completion statistics for full picture
- Consider cost monitoring for resource optimization

**For Troubleshooting:**
- Enable detailed per-operation statistics
- Monitor error conditions in real-time
- Use completion statistics for root cause analysis

#### Monitoring System Design

**Efficient Data Collection:**
```cpp
// Prefer batched updates for high-volume statistics
stats.addStatistic(StNumRowsProcessed, batchSize);  // Efficient

// Avoid individual updates for high-frequency events  
stats.addStatisticAtomic(StNumRowsProcessed, 1);    // Inefficient
```

**Balanced Monitoring Strategy:**
```
Layer 1: Real-time alerts (errors, critical thresholds)
Layer 2: Near real-time trends (5-30s intervals)  
Layer 3: Detailed analysis (completion-based statistics)
```
