# How are indexes represented

The indexes used by the HPCC systems are based on B-trees.  The B-trees are stored in fixed-size nodes (typically 8K), and there are 3 main types of node:
- Branch nodes.\
...
I think there are four main factors that affect the performance of the indexes.
...
So 'hybrid:zstds' would use zstd compression for the leaf nodes.
...
There are some indexes that are pathologically compressible - e.g., leaves take up less than 5% of the size on disk.  This can cause problems when they are read - because an 8K page can expand to 400K in memory.  That can cause fragmentation problems with the memory manager and means the leaf cache can only hold a few entries.
...
* Zstd compression of blobs\
  Blobs have not had any work done on them currently, but it would be relatively trivial to use zstd to improve the decompression speed and compressibility of the data.
