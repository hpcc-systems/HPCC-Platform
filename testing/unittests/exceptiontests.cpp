...
public:
    ...
    void testThorWrapException()
    {
        ...
        {
            Owned<IThorException> thorEx = ThorWrapException(e, "Error with zero subgraph ID");

            StringBuffer msg;
            thorEx->errorMessage(msg);

            // According to the implementation, should show "Graph graphname" when subgraph ID is 0
            CPPUNIT_ASSERT_EQUAL_STR("789, Zero subgraph test : Error with zero subgraph ID, Graph ZeroSubgraph", msg.str());
        ...
        {
            Owned<IThorException> thorEx = ThorWrapException(e, "Error with both empty");

            StringBuffer msg;
            thorEx->errorMessage(msg);

            // According to the implementation, no graph context should be added when both are empty/zero
            CPPUNIT_ASSERT_EQUAL_STR("999, Both empty test : Error with both empty", msg.str());
