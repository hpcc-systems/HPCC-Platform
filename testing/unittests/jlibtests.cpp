...
public:
    ...
    {
        testDeserializeVsDeserializeFromStream(__func__, createBinaryDataCompressionTestPTree());
    }
...
protected:
    void testDeserializeVsDeserializeFromStream(const char *testName, IPropertyTree *_originalTree)
    {
        ...
        CPPUNIT_ASSERT(areTreesEquivalent(originalTree, deserializedTree));
        CPPUNIT_ASSERT(areTreesEquivalent(originalTree, deserializedFromStreamTree));
        CPPUNIT_ASSERT(areTreesEquivalent(deserializedTree, deserializedFromStreamTree));
    ...
    bool areTreesEquivalent(IPropertyTree *tree1, IPropertyTree *tree2)
    {
        if (!tree1 && !tree2)
            return true;
        if (!tree1 || !tree2)
        ...
        // Compare names
        const char *name1 = tree1->queryName();
        const char *name2 = tree2->queryName();
        if (!name1 && !name2)
            ; // both null, continue
        else if (!name1 || !name2 || strcmp(name1, name2) != 0)
            return false;

        // Compare values
        const char *value1 = tree1->queryProp(nullptr);
        const char *value2 = tree2->queryProp(nullptr);
        if (!value1 && !value2)
            ; // both null, continue
        else if (!value1 || !value2 || strcmp(value1, value2) != 0)
            return false;

        // Compare attribute counts
        if (tree1->getAttributeCount() != tree2->getAttributeCount())
            return false;

        // Compare attributes
        Owned<IAttributeIterator> attrs1 = tree1->getAttributes();
        ForEach(*attrs1)
        {
            const char *name = attrs1->queryName();
            const char *value1 = attrs1->queryValue();
            const char *value2 = tree2->queryProp(name);
            if (!value2 || strcmp(value1, value2) != 0)
                return false;
        }

        // Compare child counts
        if (tree1->numChildren() != tree2->numChildren())
            return false;

        // Compare children
        Owned<IPropertyTreeIterator> children1 = tree1->getElements("*");
        Owned<IPropertyTreeIterator> children2 = tree2->getElements("*");
        ForEach(*children1)
        {
            if (!children2->next())
                return false;
            if (!areTreesEquivalent(&children1->query(), &children2->query()))
                return false;
        }
    ...
    IPropertyTree *createCompatibilityConfigPropertyTree()
    {
        // Creates a complex nested property tree with multiple compatibility elements for deserialization testing
        Owned<IPropertyTree> root = createPTree("__array__");
