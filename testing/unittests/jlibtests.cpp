...
class PTreeSerializationTest : public CppUnit::TestFixture
{
    ...
    CPPUNIT_TEST(testSerializeVsSerializeToStreamCPTValuePTree);
    CPPUNIT_TEST_SUITE_END();
...
public:
    ...
    void testSerializeVsSerializeToStreamCPTValuePTree()
    {
        testSerializeVsSerializeToStream(createCPTValuePTree());
    }
...
protected:
    ...
    Owned<IPropertyTree> createCPTValuePTree()
    {
        // Create a tree with various CPTValue instances (binary/compressed data)
        ...
        MemoryBuffer smallBinary, largeBinary;

        // Small binary data (under compression threshold)
        const char *smallData = "This is small binary data";
        smallBinary.append(strlen(smallData), smallData);
        tree->setPropBin("smallBinary", smallBinary.length(), smallBinary.toByteArray());
        ...
        // Add binary attribute data
        MemoryBuffer attrBinary;
        const char *attrData = "Binary attribute data";
        attrBinary.append(strlen(attrData), attrData);
        tree->setPropBin("binaryAttr", attrBinary.length(), attrBinary.toByteArray());
