...
Owned<ISocket> createTestListenerSocket()
{
    constexpr unsigned maxRetries = 20;
    for (unsigned attempt = 0; attempt < maxRetries; ++attempt)
    {
        try
        {
            return Owned<ISocket>(ISocket::create(randomTLSPort()));
        }
        catch (IJSOCK_Exception *e)
        {
            if ((e->errorCode() != JSOCKERR_port_in_use) || (attempt + 1 == maxRetries))
                throw;
            e->Release();
        }
    }
    throw MakeStringException(-1, "Unable to allocate test listener socket");
}
...
public:
    ...
    void start()
    {
        ...
        if (!started.wait(10000))
            throw MakeStringException(-1, "SimpleTLSEchoServer startup timed out");
        if (!startupSucceeded.load())
            throw MakeStringException(-1, "SimpleTLSEchoServer startup failed");
    }
    ...
    virtual int run() override
    {
        ...
        catch (...)
        {
            startupSucceeded = false;
            signalStarted();
            DBGLOG("SimpleTLSEchoServer unknown startup/runtime error");
        }
        return 0;
    }
};
...
private:
    ...
    public:
        ...
        {
            // Callback has initial refcount 1; completion consumes that ownership.
            Release();
        }
    };
...
public:
    ...
    void start()
    {
        Thread::start(false);
        if (!started.wait(10000))
            throw MakeStringException(-1, "AsyncTLSEchoServer startup timed out");
        if (!startupSucceeded.load())
            throw MakeStringException(-1, "AsyncTLSEchoServer startup failed");
    }
    ...
    virtual int run() override
    {
        ...
        try
        {
            ...
            while (running)
            {
                try
                {
                    ...
                    // Handler releases itself via afterCompletion() when operation completes.
                }
                catch (IJSOCK_Exception *e)
                {
                    switch (e->errorCode())
        ...
        catch (...)
        {
            startupSucceeded = false;
            signalStarted();
            DBGLOG("AsyncTLSEchoServer unknown startup/runtime error");
        }
        return 0;
    }
};
...
public:
    ...
    void tearDown() override
    {
        ...
        if (config)
        {
            ...
            {
                // If expert didn't exist originally, remove the temporary subtree entirely.
                config->removeProp("expert");
            }
...
public:
    ...
    void testAsyncTLSAcceptWithData()
    {
        ...
        try
        {
            ...
            // Wait for async accept callback to fire.
            unsigned timeout = 50;
    ...
    void testRapidConnectDisconnect()
    {
        ...
        try
        {
            ...
            ASSERT(acceptCount >= numCycles - 2);
    ...
    void testManySequentialConnections()
    {
        ...
        try
        {
            ...
            ASSERT(successCount >= numConnections - 2);
    ...
    void testConnectionBurst()
    {
        ...
        try
        {
            ...
            ASSERT(acceptCount >= expectedMin);
    ...
    void testAsyncTLSReadLargeData()
    {
        ...
        try
        {
            ...
            // Keep this reasonably large but below thresholds that can deadlock this synchronous echo harness
            // when client writes a large burst before it starts reading echoed data.
            const size32_t dataSize = 8192;
    ...
    {
        // DISABLED: This intermittently hangs in the current synchronous echo test harness.
        // Async write is still covered by testAsyncTLSWriteBasic.
        CPPUNIT_ASSERT(true);
        return;
        ...
        try
        {
            ...
            secureSocket->startAsyncConnect(uring, connectCallback, SSLogMin);

            bool connected = connectCallback.completed.wait(5000);
            ASSERT(connected);
            ASSERT(connectCallback.result == 0);
    ...
    {
        // DISABLED: Large async write with this synchronous echo harness is prone to deadlock.
        // Keep this disabled until a fully async test server is available.
        CPPUNIT_ASSERT(true);
        return;
    ...
    void testAsyncTLSBothSidesAsync()
    {
        ...
        serverConfig->setPropInt("@queueDepth", 64);
        ...
        clientConfig->setPropInt("@queueDepth", 64);
