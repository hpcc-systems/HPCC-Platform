...
struct Init{
          ...
          sasha(createIFile("/tmp/hpcctest/sasha.xml"))
	{
		try{
			...
			esdlDefObj.getFiles(IFiles,"/tmp/hpcctest");
...
protected:
    ...
    {
		set<string> fileSet;
		set<string> refSet{"/tmp/hpcctest/sasha.xml", "/tmp/hpcctest/common.xml", "/tmp/hpcctest/store.xml"};

        for(Owned<IFile>& myfile: setup.IFiles)
        {
			fileSet.insert(myfile->queryFilename());
        }
        CPPUNIT_ASSERT(fileSet == refSet);
    }
	void testLoadAllServices()
	{
		set<string> serviceSet;
		set<string> refSet{"WSSasha", "wsstore"};
		vector<const char*> services = setup.esdlDefObj.getAllServices();
		for(const char* service : services)
		{
			serviceSet.insert(service);
		}
		CPPUNIT_ASSERT(serviceSet == refSet);
	}
	void testDescribeAllServices()
	{
		...
		res = oss.str();
		CPPUNIT_ASSERT(refString == res);
	}
	void testPrintAllMethods()
	{
		set<string> methodSet;
		set<string> refSet{"ArchiveWU", "GetVersion", "ListWU", "Ping", "RestoreWU"};
		auto methods = setup.esdlDefObj.getAllMethods();
		for(const char* &method:methods)
		{
			methodSet.insert(method);
		}
		CPPUNIT_ASSERT(methodSet == refSet);
	}
	void testDescribeAllMethods()
	{
		string refString =
		"WSSasha [version (1.01), auth_feature (DEFERRED), exceptions_inline (./smc_xslt/exceptions.xslt), name (WSSasha), default_client_version (1.01)]\n"
	...
	void testCheckValidService()
	{
		...
		CPPUNIT_ASSERT(validCheck == true && invalidCheck == false);
	}
	...
	void testDescribe()
	{
		...
		CPPUNIT_ASSERT(refString == res && refInvalidServiceString == invalidServiceRes && refInvalidMethodString == invalidMethodRes);
	}
};
