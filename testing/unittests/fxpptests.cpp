...
static IPTree* getPTree(const char* subSection, const char* xml)
{
  ...
  {
    StringBuffer msg("Exception creating ");
    if (subSection)
      msg.appendf("'%s' PTree: ", subSection);
    else
      msg.append("PTree: ");
    e->errorMessage(msg);
    e->Release();
...
public:
    ...
    void testSkipDocument()
    {
      ...
      {
        CPPUNIT_FAIL(VStringBuffer("Parser threw an exception: %s", xe.what()));
      }
    }
    ...
    void testSkipMalformed()
    {
      ...
      {
        parser->skipSubTree();
        CPPUNIT_FAIL("Parser did not throw expected exception");
      }
      catch (XmlPullParserException& xe)
      {
        //DBGLOG("skip-malformed passed with exception: %s\n", xe.what());
      }
    }
    ...
    void runTest(const TestConfig& cfg)
    {
      ...
      {
        CPPUNIT_FAIL(VStringBuffer("XmlTokenizerException thrown: %s", e.what()));
      }
      catch (XmlPullParserException& xe)
      {
        CPPUNIT_ASSERT_MESSAGE(VStringBuffer("XmlPullParserException thrown: %s", xe.what()), !cfg.expectSuccess);
      }
      catch (IException* e)
      {
        StringBuffer m;
        ...
        e->errorMessage(m);
        e->Release();
        CPPUNIT_FAIL(VStringBuffer("IException thrown: %d - %s", e->errorCode(), e->errorMessage(m).str()));
      }
      ...
      {
        CPPUNIT_FAIL(VStringBuffer("Standard exception thrown: %s", e.what()));
      }
      catch (...)
      {
    ...
    void runTest(const TestConfig& cfg, const char* testInput)
    {
      ...
      try
      {
        ...
        CPPUNIT_ASSERT_MESSAGE(VStringBuffer("Test did not throw an expected exception. \n- Actual: %s", testOutput.str()), cfg.expectSuccess);
      }
      catch (sxt::XmlTokenizerException& e)
      {
        CPPUNIT_FAIL(VStringBuffer("XmlTokenizerException thrown: %s", e.what()));
      ...
      {
        CPPUNIT_ASSERT_MESSAGE(VStringBuffer("XmlPullParserException thrown: %s\n- Actual: %s", xe.what(), testOutput.str()), !cfg.expectSuccess);
      }
      catch (IException* e)
      {
        StringBuffer m;
        ...
        e->errorMessage(m);
        e->Release();
        CPPUNIT_FAIL(VStringBuffer("IException thrown: %d - %s\n- Actual: %s", e->errorCode(), e->errorMessage(m).str(), testOutput.str()));
      }
      catch (CppUnit::Exception& e)
      {
        throw;
      ...
      {
        CPPUNIT_FAIL(VStringBuffer("Unknown exception thrown\n- Actual: %s", testOutput.str()));
      }
    }
    ...
    void runTest(const TestConfig& cfg, const char* testInput, const char* equivalentInput)
    {
      ...
      if (compareOutput)
      {
        ...
        {
          CPPUNIT_FAIL(VStringBuffer("XmlPullParserException thrown: %s\n- Actual: %s\n- Expected: %s", xe.what(), testOutput.str(), equivalentOutput.str()));
        }
        catch (IException* e)
        {
          StringBuffer m;
          ...
          e->errorMessage(m);
          e->Release();
          CPPUNIT_FAIL(VStringBuffer("IException thrown: %d - %s\n- Actual: %s\n- Expected: %s", e->errorCode(), e->errorMessage(m).str(), testOutput.str(), equivalentOutput.str()));
        }
        catch (CppUnit::Exception& e)
        {
          throw;
        ...
        {
          CPPUNIT_FAIL(VStringBuffer("Unknown exception thrown\n- Actual: %s\n- Expected: %s", testOutput.str(), equivalentOutput.str()));
        }
