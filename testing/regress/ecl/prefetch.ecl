...
outrec trans1(in L) := TRANSFORM
  SELF.word := L.word;
  SELF.doc := SORTED(Files.getWordIndex()(KEYED(kind = TS.kindType.TextEntry),keyed(word = l.word)), doc)[1].doc;
END;

outrec trans2(in L) := TRANSFORM
  SELF.word := L.word;
  SELF.doc := SORTED(Files.getWordIndex()(KEYED(kind = TS.kindType.TextEntry),keyed(word = 'gobbledegook'+l.word)), doc)[1].doc;
END;
