...
reorderedIndex  := index({ rec.kind, rec.word, rec.doc, rec.segment, rec.wpos }, { rec }, __nameof__(searchIndex));
...
createSamples(iters, numRows) := FUNCTIONMACRO
    ...
    #LOOP
        ...
        output(SUM(createSample2(%count%, iters, numRows), matches) - numRows),
        output(SUM(createSample3(%count%, iters, numRows), matches) - numRows),
        #SET (count, %count%+1)
    #END
        output('Done')
    );
