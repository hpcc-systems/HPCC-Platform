...
//version mode='multipleFieldMultipleLargeRecords'
//version mode='multipleFieldMultipleLargeRecordsSpecTerminator'
//version mode='multipleFieldMultipleLargeRecordsSpecSeparator'
//version mode='multipleFieldMultipleLargeRecordsSpecSeparatorTerminator'
//version mode='multipleFieldMultipleHugeRecords'
//version mode='multipleFieldMultipleHugeRecordsSpecTerminator'
//version mode='multipleFieldMultipleHugeRecordsSpecSeparator'
//version mode='multipleFieldMultipleHugeRecordsSpecSeparatorTerminator'
...
END;
    ...
    singleFieldMultipleRecords := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                   TRANSFORM({SingleFieldLayout},
    ...
    singleFieldMultipleLargeRecords := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                   TRANSFORM({SingleFieldLayout},
    ...
    singleFieldMultipleHugeRecords := DATASET(250,
                                   TRANSFORM({SingleFieldLayout},
    ...
    singleFieldMultipleRecords := DATASET(100,
                                   TRANSFORM({SingleFieldLayout},
                         SELF.field := INTFORMAT(COUNTER,5,1) + ':'+ genfiled(RANDOM() % 150);
                        )
              ,DISTRIBUTED
    ...
    multipleFieldMultipleRecords := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
    ...
    multipleFieldMultipleRecordsSpecTerminator := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % 150),
                                            SELF.field2 := genfiled(RANDOM() % 150),
    ...
    multipleFieldMultipleRecordsSpecSeparator := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % 150),
                                            SELF.field2 := genfiled(RANDOM() % 150),
    ...
    multipleFieldMultipleRecordsSpecSeparatorTerminator := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % 150),
                                            SELF.field2 := genfiled(RANDOM() % 150),
...
#elseif (mode = 'multipleFieldMultipleLargeRecords')

    multipleFieldMultipleLargeRecords := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % (LARGE_REC_SIZE / 3)),
                                            SELF.field2 := genfiled(RANDOM() % (LARGE_REC_SIZE / 3)),
                                            SELF.field3 := genfiled(RANDOM() % (LARGE_REC_SIZE / 3));
                                        )
                                      ,DISTRIBUTED
                                  );

    dsSetup := multipleFieldMultipleLargeRecords : INDEPENDENT;
    recSize := LARGE_REC_SIZE;
    String csvTerminator := '\n';
    String csvSeparator := ',';

#elseif (mode = 'multipleFieldMultipleLargeRecordsSpecTerminator')

    multipleFieldMultipleLargeRecordsSpecTerminator := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % (LARGE_REC_SIZE / 3)),
                                            SELF.field2 := genfiled(RANDOM() % (LARGE_REC_SIZE / 3)),
    ...
    dsSetup := multipleFieldMultipleLargeRecordsSpecTerminator : INDEPENDENT;
    recSize := LARGE_REC_SIZE;
    String csvTerminator := (>STRING<) x'01';
    String csvSeparator := ',';

#elseif (mode = 'multipleFieldMultipleLargeRecordsSpecSeparator')

    multipleFieldMultipleLargeRecordsSpecSeparator := DATASET(REC_COUNT + RANDOM() % REC_COUNT,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % (LARGE_REC_SIZE / 3)),
                                            SELF.field2 := genfiled(RANDOM() % (LARGE_REC_SIZE / 3)),
    ...
    dsSetup := multipleFieldMultipleLargeRecordsSpecSeparator : INDEPENDENT;
    recSize := LARGE_REC_SIZE;
    String csvTerminator := '\n';
    String csvSeparator := (>STRING<) x'01';

#elseif (mode = 'multipleFieldMultipleLargeRecordsSpecSeparatorTerminator')
...
#elseif (mode = 'multipleFieldMultipleHugeRecords')

    multipleFieldMultipleHugeRecords := DATASET(250,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
                                            SELF.field2 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
                                            SELF.field3 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3));
                                        )
                                      ,DISTRIBUTED
                                  );

    dsSetup := multipleFieldMultipleHugeRecords : INDEPENDENT;
    recSize := HUGE_REC_SIZE;
    String csvTerminator := '\n';
    String csvSeparator := ',';

#elseif (mode = 'multipleFieldMultipleHugeRecordsSpecTerminator')

    multipleFieldMultipleHugeRecordsSpecTerminator := DATASET(250,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
                                            SELF.field2 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
    ...
    dsSetup := multipleFieldMultipleHugeRecordsSpecTerminator : INDEPENDENT;
    recSize := HUGE_REC_SIZE;
    String csvTerminator := (>STRING<) x'01';
    String csvSeparator := ',';

#elseif (mode = 'multipleFieldMultipleHugeRecordsSpecSeparator')

    multipleFieldMultipleHugeRecordsSpecSeparator := DATASET(250,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
                                            SELF.field2 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
    ...
    dsSetup := multipleFieldMultipleHugeRecordsSpecSeparator : INDEPENDENT;
    recSize := HUGE_REC_SIZE;
    String csvTerminator := '\n';
    String csvSeparator := (>STRING<) x'01';

#elseif (mode = 'multipleFieldMultipleHugeRecordsSpecSeparatorTerminator')

    multipleFieldMultipleHugeRecordsSpecSeparatorTerminator := DATASET(250,
                                        TRANSFORM({MultipleFieldLayout},
                                            SELF.id := COUNTER,                         
                                            SELF.field1 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
                                            SELF.field2 := genfiled(RANDOM() % (HUGE_REC_SIZE / 3)),
    ...
    dsSetup := multipleFieldMultipleHugeRecordsSpecSeparatorTerminator : INDEPENDENT;
    recSize := HUGE_REC_SIZE;
    String csvTerminator := (>STRING<) x'02';
    String csvSeparator := (>STRING<) x'01';
