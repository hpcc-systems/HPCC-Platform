...
ds1 := NOFOLD(DATASET(100, TRANSFORM(testRecord,
            SELF.name  := (DATA)counter,
            SELF.name1 := (DATA)((STRING)counter + 'x1'),
            SELF.name2 := (DATA)((STRING)counter + 'x2'),
            SELF.name4 := (DATA)((STRING)counter + 'x4'),
            SELF.sentinel := 42;
        )));

// Assign so the sizes are known to be good - v1.
p1 := NOFOLD(PROJECT(ds1, TRANSFORM(testRecord,
            SELF.name  := (DATA)(LEFT.name + 'x0'),
...
p2 := NOFOLD(PROJECT(ds1, TRANSFORM(testRecord,
            SELF.name1 := (DATA)TRIM(LEFT.name1, ALL), // can only reduce the length
            SELF.name2 := (DATA)(LEFT.name1 + 'x');    // assignment from a smaller string
            SELF := LEFT
        )));
...
singleRecord t(DATA x) := TRANSFORM
    SELF.name1 := x;
END;

DATA300 longstring1 := (DATA)('xabc' + (STRING255)' ' + 'yz');
DATA longstring2 := (DATA)('xabc' + (STRING255)' ' + 'yz');
DATA longstring3 := (DATA)('xabc' + (STRING255)' ' + 'yz') : stored('longstring3');
DATA longstring4 := (DATA)('xabc' + (STRING253)' ') : stored('longstring4');
DATA longstring5 := (DATA)(longstring4 + 'yz');
DATA longstring6 := (DATA)(TRIM(longstring4) + 'yz');
DATA50 longstring7 := (DATA)('x') : stored('longstring7');
DATA longstring8 := (DATA)(TRIM(longstring7) + 'yz');
...
output(nofold(ds5), { TRIM(name1), LENGTH(name1), LENGTH(TRIM(name1)) }, all);

pxx1 := PROJECT(ds5, TRANSFORM(singleRecord,
            SELF.name1 := (DATA)(TRIM((STRING50)LEFT.name1) + 'xx');
        ));
