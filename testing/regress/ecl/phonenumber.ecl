...
testNumbers := DATASET([
  ...
  {'2234567890', 'US', true, 'US 10-digit domestic format'},
...
testResults := PROJECT(testNumbers,
    TRANSFORM({TestRecord, lib_phonenumber.phonenumber_data},
        SELF := LEFT,   // Copy original test record fields
        SELF := lib_phonenumber.parsePhoneNumber(LEFT.phonenumber, LEFT.country_code) // Add plugin results
    )
);

// Show only Valid Numbers (using the 'valid' field from phonenumber_data)
validNumbers := testResults(valid = true);
validNumbersResult := IF(COUNT(validNumbers) = 37, 'Pass', 'Fail: valid numbers mismatch ' + COUNT(validNumbers));
OUTPUT(validNumbersResult, NAMED('validNumbers'));

// Show only Invalid Numbers
invalidNumbersResult := IF(COUNT(testResults(valid = false)) = 74, 'Pass', 'Fail: invalid numbers mismatch ' + COUNT(testResults(valid = false)));
OUTPUT(invalidNumbersResult, NAMED('invalidNumbers'));
...
typeCount := TABLE(typeAnalysis, {
    lineType,
    typeDescription,
    typeCount := COUNT(GROUP)
}, lineType, typeDescription);

OUTPUT(SORT(typeCount, lineType), NAMED('phoneTypeCount'));
...
OUTPUT(SORT(countryStats, countryCode), NAMED('countryDistribution'));
