...
//#OPTION('writeInlineContent', true); explicitly don't enable writing inline content
...
output(ds,,'REGRESS::TEMP::inline_content_off_orig.xml', overwrite, xml);

readWrittenOrig := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_orig.xml'), L0Rec, xml('Dataset/Row'));
output(readWrittenOrig, named('readWrittenOrig'));

output(named_dsj, named('OrigInlineJson'));
output(named_dsj,,'REGRESS::TEMP::inline_content_off_orig.json', overwrite, json);

readWrittenOrigJson := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_orig.json'), L0Rec, json('/Row'));
...
output(x,,'REGRESS::TEMP::inline_content_off_named.xml', overwrite, xml);

readWrittenNamed := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_named.xml'), L1Rec, xml('Dataset/Row'));
output(readWrittenNamed, named('readWrittenNamed'));

xj := PROJECT(named_dsj, L1Rec);

output(xj, named('NamedInlineJson'));
output(xj,,'REGRESS::TEMP::inline_content_off_named.json', overwrite, json);

readWrittenNamedJson := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_named.json'), L1Rec, json('/Row'));
...
L2Rec := RECORD
   DATASET(inline2Rec) peeps{xpath('L1/L2<>')}; 
END;

y := PROJECT(x, L2Rec);

output(y, named('UnnamedInline'));
output(y,,'REGRESS::TEMP::inline_content_off_unnamed.xml', overwrite, xml);
...
readWrittenUnnamed := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_unnamed.xml'), R2Rec, xml('Dataset/Row'));
output(readWrittenUnnamed, named('readWrittenUnnamed'));

yj := PROJECT(unnamed_dsj, L2Rec);

output(yj, named('UnnamedInlineJson'));
output(yj,,'REGRESS::TEMP::inline_content_off_unnamed.json', overwrite, json);

readWrittenUnnamedJson := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_unnamed.json'), R2Rec, json('/Row'));
...
L3Rec := RECORD
   DATASET(inline3Rec) peeps{xpath('L1/<>')};
END;

z := PROJECT(ds, L3Rec);

output(z, named('NonameInline'));
output(z,,'REGRESS::TEMP::inline_content_off_noname.xml', overwrite, xml);
...
readWrittenNoname := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_noname.xml'), R3Rec, xml('Dataset/Row'));
output(readWrittenNoname, named('readWrittenNoname'));

zj := PROJECT(unnamed_dsj, L3Rec);

output(zj, named('NonameInlineJson'));
output(zj,,'REGRESS::TEMP::inline_content_off_noname.json', overwrite, json);

readWrittenNonameJson := dataset(DYNAMIC('REGRESS::TEMP::inline_content_off_noname.json'), R3Rec, json('/Row'));
