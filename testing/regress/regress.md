...
There are two parts to the Regression Suite--The Regression Test Engine (RTE) and the set of ECL files collectively called the Regression Suite.

## Regression Test Engine

The RTE is a front-end tool (written in Python3) for the ECL CLI and the ECL CC compiler. The RTE adds functionality to the command line tools to support:

* Execution of test cases on:
  * All target engines defined in the config file, or
  * Only those engines in a comma-separated list using the -t|--target parameter.
* Bulk execution of test cases in alphabetical order.
  * In *run* mode, it executes all test cases of the Regression Suite on the target (except those excluded by '//no<target_engine_name>' tag) with one command.
...
* Handle timeout management
  * It uses the global timeout defined in the RTE config file (ecl-test.json)
  * Handles the command line timeout parameter to redefine the global value
...
* Generate a report (log file) with these sections:
  * Report header: with target engine and number of test cases to be executed
  * Executed tests list: with elapsed time (in seconds), outcome (Pass/Fail), and a link to workunit in the HPCC Platform
  * Result: A summary including the number of passed, failed test cases.
  * Output: If any test is forced to generate output then their output appears here.
...
The Regression Suite is a collection of ECL files and XML result files (the results expected from the ECL). This includes ECL activities with different usage and parameters, plugins (e.g., file, spray, despray, Kafka, Redis, Memcached, MySQL, etc.) and  embedded languages (e.g., CPP, Java, Python, etc.).
...
* HPCC Client tools installed locally. **The Client Tools and Platform versions must match.**
...
When running the regression suite, you may want to clean up your platform by removing the workunits it created.
