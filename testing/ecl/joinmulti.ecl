...
bigstream := NORMALIZE(seed2, LEFT.keyn, gen2(LEFT, counter)) : PERSIST('bigstream');  
...
j0 := JOIN(bigstream, bigstream, LEFT.key=RIGHT.key AND RIGHT.keyn>=delay(RIGHT.keyn, fakeWorkUs));
j1 := JOIN(bigstream, bigstream, LEFT.key=RIGHT.key AND RIGHT.keyn>=delay(RIGHT.keyn, fakeWorkUs), HINT(parallel_match));
j2 := JOIN(bigstream, bigstream, LEFT.key=RIGHT.key AND RIGHT.keyn>=delay(RIGHT.keyn, fakeWorkUs), HINT(parallel_match), HINT(unsorted_output));
...
baseJoin := countGroups(j0);
parallelJoin := countGroups(j1);
parallelUnorderedJoin := countGroups(j2);

// although ordering can differ, group sizes from all varieties should match
cmpj1 := JOIN(baseJoin, parallelJoin, LEFT.key=RIGHT.key AND LEFT.groupSize=RIGHT.groupSize, FULL ONLY);
cmpj2 := JOIN(baseJoin, parallelUnorderedJoin, LEFT.key=RIGHT.key AND LEFT.groupSize=RIGHT.groupSize, FULL ONLY);
