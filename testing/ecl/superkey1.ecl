...
albumTable := DATASET('albums.d00',{AlbumRecordDef,UNSIGNED8 filepos {virtual(fileposition)}},FLAT);

/*
// single key
albumIndex := INDEX(albumTable,{ Artist, Title, filepos }, 'albums.key');

SEQUENTIAL(
  OUTPUT(ds,,'albums.d00',OVERWRITE),
  BUILDINDEX(albumIndex,OVERWRITE),
  OUTPUT(albumIndex)
);
*/

albumIndex1 := INDEX(albumTable(Tracks<=4),{ Artist, Title, filepos }, 'albums1.key');
albumIndex2 := INDEX(albumTable((Tracks>4)AND(Tracks<8)),{ Artist, Title, filepos }, 'albums2.key');
albumIndex3 := INDEX(albumTable((Tracks>=8)AND(Tracks<12)),{ Artist, Title, filepos }, 'albums3.key');
albumIndex4 := INDEX(albumTable((Tracks>=12)AND(Tracks<20)),{ Artist, Title, filepos }, 'albums4.key');
albumIndex5 := INDEX(albumTable(Tracks>=20),{ Artist, Title, filepos }, 'albums5.key');
albumIndex  := INDEX(albumTable,{ Artist, Title, filepos }, 'superalbums.key');

SEQUENTIAL(
  OUTPUT(ds,,'albums.d00',OVERWRITE),
  FileServices.DeleteSuperFile('superalbums.key'),
  FileServices.DeleteSuperFile('superalbums'),
  ...
  FileServices.CreateSuperFile('superalbums.key'),
  FileServices.StartSuperFileTransaction(),
  FileServices.AddSuperFile('superalbums.key','albums1.key'),
  FileServices.AddSuperFile('superalbums.key','albums2.key'),
  FileServices.AddSuperFile('superalbums.key','albums3.key'),
  FileServices.AddSuperFile('superalbums.key','albums4.key'),
  FileServices.AddSuperFile('superalbums.key','albums5.key'),
...
//  OUTPUT(SORTED(albumIndex)),
  FileServices.StartSuperFileTransaction(),
  FileServices.RemoveSuperFile('superalbums.key','albums2.key'),
  FileServices.RemoveSuperFile('superalbums.key','albums4.key'),
  FileServices.RemoveSuperFile('superalbums.key','albums3.key'),
  FileServices.FinishSuperFileTransaction(),
