...
protected:
    ...
    atomic_uint nextSpace; // guaranteed to be set if it is in the maybeFreeList.
...
public:
    ...
    virtual void _setDestructorFlag(const void *_ptr)
    {
        ...
        char *ptr = (char *) _ptr;
        atomic_uint * curCount = (atomic_uint *)(ptr - sizeof(atomic_uint));
...
protected:
    unsigned allocatorId;
    atomic_uint rowCount;  // A separate rowcount is required, otherwise the page could be freed before the destructor is called
...
public:
    CHeap(CChunkingRowManager * _rowManager, const IContextLogger &_logctx, const IRowAllocatorCache *_allocatorCache, unsigned _flags)
        : logctx(_logctx), rowManager(_rowManager), allocatorCache(_allocatorCache), activeHeaplet(NULL), heaplets(NULL), flags(_flags), headMaybeSpace(BLOCKLIST_NULL)
    {
    }
...
protected:
    ...
    atomic_uint headMaybeSpace;  // The head of the list of heaplets which potentially have some space.  When adding must use mo_release, when removing must use mo_acquire
...
protected:
    ...
    CRowManager * owner;
    std::atomic_uint releasingBuffers;  // boolean if pre-emptive releasing thread is active
    atomic_uint releaseSeq;
...
private:
    ...
    atomic_uint possibleGoers = {0};
    atomic_uint totalHeapPages = {0};
...
public:
    ...
    virtual bool attachDataBuff(DataBuffer *dataBuff) 
    {
        ...
        bool needCheck;
        {
            ...
            while (finger && possibleGoers.load(std::memory_order_acquire))
            {
