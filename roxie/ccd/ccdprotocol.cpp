...
public:
    IMPLEMENT_IINTERFACE;
    CHpccProtocolResponse(const char *queryname, SafeSocket *_client, TextMarkupFormat _mlFmt, unsigned flags, bool _isHTTP, const IContextLogger &_logctx, PTreeReaderOptions _xmlReadFlags) :
...
public:
    CHpccJsonResponse(const char *queryname, SafeSocket *_client, unsigned flags, bool _isHttp, const IContextLogger &_logctx, PTreeReaderOptions _xmlReadFlags) :
        CHpccProtocolResponse(queryname, _client, MarkupFmt_JSON, flags, _isHttp, logctx, _xmlReadFlags)
    {
    }
...
public:
    CHpccXmlResponse(const char *queryname, SafeSocket *_client, unsigned flags, bool _isHTTP, const IContextLogger &_logctx, PTreeReaderOptions _xmlReadFlags) :
        CHpccProtocolResponse(queryname, _client, MarkupFmt_XML, flags, _isHTTP, _logctx, _xmlReadFlags)
    {
    }
...
IHpccProtocolResponse *createProtocolResponse(const char *queryname, SafeSocket *client, HttpHelper &httpHelper, const IContextLogger &logctx, unsigned protocolFlags, PTreeReaderOptions xmlReadFlags)
{
    if (protocolFlags & HPCC_PROTOCOL_NATIVE_RAW || protocolFlags & HPCC_PROTOCOL_NATIVE_ASCII)
        return new CHpccProtocolResponse(queryname, client, MarkupFmt_Unknown, protocolFlags, false, logctx, xmlReadFlags);
...
readAnother:
        ...
        try
        {
            if (streq(queryPrefix.str(), "control"))
            {
                ...
                Owned<IHpccProtocolResponse> protocol = createProtocolResponse(queryPT->queryName(), client, httpHelper, logctx, protocolFlags, defaultXmlReadFlags);
                sink->control(msgctx, queryPT, sanitizedText, protocol);
                protocol->finalize(0);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (msgctx->initQuery(querySetName, queryName))
                    {
                        ...
                        {
                            Owned<IHpccProtocolResponse> protocol = createProtocolResponse(queryPT->queryName(), client, httpHelper, logctx, protocolFlags, xmlReadFlags);
