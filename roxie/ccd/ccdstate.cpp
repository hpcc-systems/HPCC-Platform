    ...
    virtual void load(const IPropertyTree *querySet, const IRoxiePackageMap &packages, hash64_t &hash, bool forceRetry)
    {
        ...
        if (numQueries)
        {
            ...
            if (numResolveFilenameThreads)
            {
                ...
                // Expand the nested maps of packees and filesnames into a linear list that is easy to walk in parallel.
                ...
                // Now resolve the filenames in parallel - on a system with remote files most of the time is spend retreiving the file sizes.
