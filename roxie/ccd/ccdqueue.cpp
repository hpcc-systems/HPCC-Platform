...
class IBYTIbuffer
{
    ...
    // Probably ends up going funky for a second or so when msTick() wraps, but again, I don't care.
public:
    ...
    void noteOrphan(const RoxiePacketHeader &hdr)
    {
        ...
        orphans[tail].activityId = now + initIbytiDelay*10;  // MORE - configure separately?
        tail++;
        if (tail == numOrphans)
            tail = 0;
    }
    bool lookup(const RoxiePacketHeader &hdr) const
    {
        ...
        do
        {
            if (!lookat)
                lookat = numOrphans;
            lookat--;
            if (orphans[lookat].activityId < now)
                break;    // expired;
