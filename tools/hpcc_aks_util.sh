...
show_usage() {
    cat << EOF
Usage: $0 -g RESOURCE_GROUP --name CLUSTER_NAME --namespace NAMESPACE [--show-pods]
...
# Includes Enterprise Agreement (30% discount) and Private Pricing Agreement discounts: 
# - Virtual Machines Dds_v4 family (D16ds v4 and larger, Linux) 80% discount
# - All other VMs: 30% Enterprise Agreement discount
get_vm_hourly_cost() {
    local vm_size="$1"
    case "$vm_size" in
        # Standard D-series v5 (30% EA discount applied)
        "Standard_D2s_v5") echo "0.067" ;;   # 0.096 * 0.70 (30% discount)
        "Standard_D4s_v5") echo "0.134" ;;   # 0.192 * 0.70 (30% discount)
        "Standard_D8s_v5") echo "0.269" ;;   # 0.384 * 0.70 (30% discount)
        "Standard_D16s_v5") echo "0.538" ;;  # 0.768 * 0.70 (30% discount)
        "Standard_D32s_v5") echo "1.075" ;;  # 1.536 * 0.70 (30% discount)
        "Standard_D48s_v5") echo "1.613" ;;  # 2.304 * 0.70 (30% discount)
        "Standard_D64s_v5") echo "2.150" ;;  # 3.072 * 0.70 (30% discount)
        
        # Standard D-series v4 (smaller sizes - 30% EA discount applied)
        "Standard_D2s_v4") echo "0.067" ;;   # 0.096 * 0.70 (30% discount)
        "Standard_D4s_v4") echo "0.134" ;;   # 0.192 * 0.70 (30% discount)
        "Standard_D8s_v4") echo "0.269" ;;   # 0.384 * 0.70 (30% discount)
        
        # Standard Dds_v4 family (D16ds v4 and larger, Linux) - 80% private pricing discount applied
        "Standard_D16s_v4") echo "0.154" ;;  # 0.768 * 0.20 (80% discount)
        "Standard_D32s_v4") echo "0.307" ;;  # 1.536 * 0.20 (80% discount)
        "Standard_D48s_v4") echo "0.461" ;;  # 2.304 * 0.20 (80% discount)
        "Standard_D64s_v4") echo "0.614" ;;  # 3.072 * 0.20 (80% discount)
        
        # Standard E-series v5 (memory optimized - 30% EA discount applied)
        "Standard_E2s_v5") echo "0.088" ;;   # 0.126 * 0.70 (30% discount)
        "Standard_E4s_v5") echo "0.176" ;;   # 0.252 * 0.70 (30% discount)
        "Standard_E8s_v5") echo "0.353" ;;   # 0.504 * 0.70 (30% discount)
        "Standard_E16s_v5") echo "0.706" ;;  # 1.008 * 0.70 (30% discount)
        "Standard_E32s_v5") echo "1.411" ;;  # 2.016 * 0.70 (30% discount)
        "Standard_E48s_v5") echo "2.117" ;;  # 3.024 * 0.70 (30% discount)
        "Standard_E64s_v5") echo "2.822" ;;  # 4.032 * 0.70 (30% discount)
        
        # Standard F-series v2 (compute optimized - 30% EA discount applied)
        "Standard_F2s_v2") echo "0.060" ;;   # 0.085 * 0.70 (30% discount)
        "Standard_F4s_v2") echo "0.118" ;;   # 0.169 * 0.70 (30% discount)
        "Standard_F8s_v2") echo "0.237" ;;   # 0.338 * 0.70 (30% discount)
        "Standard_F16s_v2") echo "0.473" ;;  # 0.676 * 0.70 (30% discount)
        "Standard_F32s_v2") echo "0.946" ;;  # 1.352 * 0.70 (30% discount)
        "Standard_F48s_v2") echo "1.420" ;;  # 2.028 * 0.70 (30% discount)
        "Standard_F64s_v2") echo "1.893" ;;  # 2.704 * 0.70 (30% discount)
        "Standard_F72s_v2") echo "2.129" ;;  # 3.042 * 0.70 (30% discount)
        
        # Standard B-series (burstable - 30% EA discount applied)
        "Standard_B2s") echo "0.029" ;;      # 0.041 * 0.70 (30% discount)
        "Standard_B2ms") echo "0.058" ;;     # 0.083 * 0.70 (30% discount)
        "Standard_B4ms") echo "0.116" ;;     # 0.166 * 0.70 (30% discount)
        "Standard_B8ms") echo "0.233" ;;     # 0.333 * 0.70 (30% discount)
        "Standard_B12ms") echo "0.349" ;;    # 0.499 * 0.70 (30% discount)
        "Standard_B16ms") echo "0.466" ;;    # 0.666 * 0.70 (30% discount)
        "Standard_B20ms") echo "0.582" ;;    # 0.832 * 0.70 (30% discount)
        
        # Default fallback - estimate based on CPU count if available (30% EA discount applied)
        *) 
            # Try to extract CPU count from VM size name and estimate
            if [[ "$vm_size" =~ ([0-9]+) ]]; then
                local cpu_estimate="${BASH_REMATCH[1]}"
                echo "scale=3; $cpu_estimate * 0.034" | bc -l  # ~$0.034 per vCPU estimate (30% discount applied)
            else
                echo "0.070"  # Default estimate with 30% discount
            fi
            ;;
    esac
}
