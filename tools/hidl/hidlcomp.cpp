...
void EspServInfo::write_esp_client()
{
    ...
    for (mthi=methods;mthi!=NULL;mthi=mthi->next)
    {
        ...
        outf("\t\tCClient%sThread(IRpcRequestBinding *_esprequest) { m_esprequest.set(_esprequest); }\n", name_);
        outs("\t\tint run()\n");
        outs("\t\t{\n");
        outf("\t\t\tCClient%s::espWorkerThread((void *)m_esprequest);\n", name_);
        outs("\t\t\treturn 0;\n");
        outs("\t\t}\n");
        outs("\t} clientthread(esprequest);\n\n");
        outs("\tclientthread.setStackSize(0x10000);\n");
        outs("\tclientthread.start();\n");
        outs("}\n");
