    ...
    if (useHTTP)
    {
        ...
        if (generateClientSpans && clientSpan->isValid())
        {
            ...
            if (retrievedClientHeaders->hasProp("traceparent"))
                tpHeader.appendf("--header traceparent: %s\r\n", retrievedClientHeaders->queryProp("traceparent"));
        }
    ...
    else
    {
        ...
        if (generateClientSpans && clientSpan->isValid())
        {
            ...
            if (retrievedClientHeaders->hasProp("traceparent"))
            {
                ...
                Owned<IPTree> p = createPTreeFromXMLString(fullQuery.str(), ipt_none, ptr_none);
                const char *queryName = p->queryName();
                ...
                if (traceHeader)
                {
                    Owned<IPTreeIterator> traceElemets = traceHeader->getElements("./*");
                    ForEach(*traceElemets)
                    {
                        ...
                        if (strcmp(traceHeaderElem.queryName(), "traceparent") != 0) //if not _trace
                            toXML(&traceHeaderElem, fullQuery, 0, XML_SingleQuoteAttributeValues);
                    }
                }

                fullQuery.append("</_trace>");

                Owned<IPTreeIterator> elements = p->getElements("./*");
                ForEach(*elements)
                {
                    IPTree &elem = elements->query();
                    if (strcmp(elem.queryName(), "_trace") != 0) //if not _trace
                        toXML(&elem, fullQuery, 0, XML_SingleQuoteAttributeValues);
                }
                fullQuery.appendf("</%s>", queryName);
            }
        }
    }
...
void usage(int exitCode)
{
    ...
    printf("  -ctc       customized tracing configuration provided\n");
