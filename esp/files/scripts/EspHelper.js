...
define([
	...
	"dojox/xml/parser", 
	"dojox/xml/DomParser"
], function(baseConfig, declare, xmlParser, xmlDomParser) {
	return declare(null, {
		...
		parseEclDirectRunEclResponse : function(xml) {
			//  RunEclResponse/results/Result/Dataset/Row/*
			var myObj = xmlDomParser.parse(xml);
			var retVal = {
				exceptions : [],
				datasets : []
			};

			retVal.exceptions = this.parseRows(myObj, "Exception");
			retVal.datasets = this.parseDatasets(myObj, "Dataset", "Row");
			return retVal;
		},
		
		parseWsWorkunitsRunEclResponse : function(xml) {
			var myObj = xmlDomParser.parse(xml);
			var retVal = {
				exceptions : [],
				errors : [],
			...
			retVal.exceptions = this.parseRows(myObj, "Exception");
			retVal.errors = this.parseRows(myObj, "ECLException");
			retVal.datasets = this.parseDatasets(myObj, "Dataset", "Row");
			var graphs = myObj.getElementsByTagName("graphsXGMML");
			if(graphs.length) {
				if(graphs[0].childNodes.length) {
					retVal.xgmml = graphs[0].childNodes[0].nodeValue;
				}
			}
		...
		parseWsWorkunitsWUInfoResponse : function(xml) {
			var myObj = xmlDomParser.parse(xml);
			var retVal = {
				exceptions : [],
				errors : [],
