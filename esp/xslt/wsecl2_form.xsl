...
<xsl:text disable-output-escaping="yes"><![CDATA[    var html =  get_]]></xsl:text>
                    <xsl:value-of select="translate($requestElement, '.', '_')"/>
...
function setESPFormAction()  // reqType: 0: regular form, 1: soap, 2: form param passing, 3: roxiexml
{
    ...
    var actionpath="]]><xsl:value-of select="concat('/WsEcl2/submit/query/', $queryPath, '/', $methodName)"/> <![CDATA[";

    var actval = document.getElementById('submit_type');
    if (actval && actval.value)
    {
        if (actval.value=="esp_soap")
            actionpath = "]]><xsl:value-of select="concat('/WsEcl2/forms/soap/', $queryPath, '/', $methodName)"/><![CDATA[";
        else if (actval.value=="roxie_soap")
            actionpath = "]]><xsl:value-of select="concat('/WsEcl2/forms/roxiesoap/', $queryPath, '/', $methodName)"/><![CDATA[";
        else if (actval.value=="roxie_xml")
            actionpath = "]]><xsl:value-of select="concat('/WsEcl2/forms/roxiexml/', $queryPath, '/', $methodName)"/><![CDATA[";
        else if (actval.value=="run_xslt")
            actionpath = "]]><xsl:value-of select="concat('/WsEcl2/xslt/query/', $queryPath, '/', $methodName)"/><![CDATA[";
    }
...
<xsl:call-template name="GetHtmlHeadAddon"/>
                ...
                <tr class='commands'>
                  <td align='left'>
                    <select id="submit_type" name="submit_type_">
                        ...
                        <option value="esp_soap">ESP SOAP TEST</option>
                        <option value="roxie_soap">ROXIE SOAP TEST</option>
                        <option value="roxie_xml">ROXIE XML TEST</option>
                    </select>&nbsp;
  ...
  function get_<xsl:value-of select="translate($type, '.', '_')"/>_Item(parentId,itemName) {
  ...
  function get_<xsl:value-of select="translate($type, '.', '_')"/>_Enum(ctrl_id, selVal) {
  ...
  ========================================================================================= -->
   ...
   ======================================================================================= -->  
    <xsl:template name="GetInputCtrlHtml">
        ...
        <xsl:if test="$verbose">
            <xsl:variable name="translated_name" select="translate($node/@name,'.', '_')"/>
                    ...
                    <xsl:when test="count($ems)=1 and ($cpxType/xsd:sequence/@maxOccurs='unbounded' or $ems[1]/@maxOccurs='unbounded')">
                        ...
                        <xsl:choose>
                            <xsl:when test="$ems/xsd:complexType">
                                ...
                                <xsl:value-of select="concat('&quot;+get_Array_Input(&quot;',$fieldId,'&quot;,&quot;',$btype,'&quot;,&quot;',translate($ems[1]/@name, '.','_'),'&quot;)+&quot;')"/>
                            </xsl:when>
                            ...
                            <xsl:when test="starts-with($stype,'xsd:') or starts-with($stype,'xs:')">
                                ...
                                <xsl:if test="not($useTextareaForStringArray)">
                                    ...
                                    <xsl:text disable-output-escaping="yes"><![CDATA[","]]></xsl:text>
                                    <xsl:value-of select="translate($ems[1]/@name, '.', '_')"/>
                                    <xsl:text disable-output-escaping="yes"><![CDATA[")+"]]></xsl:text>
                                </xsl:if>
                            </xsl:when>
                        </xsl:choose>
   ...
   ======================================================================================= -->
    ...
    <xsl:template name="GenOneInputCtrlHtml">
        ...
        <xsl:variable name="fieldName" select="translate($node/@name, '.', '_')"/>
    ...
    <xsl:template name="build_link">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                    ...
                    <xsl:choose>
                        <xsl:when test="$type='reqxml'"><xsl:value-of select="concat('/WsEcl2/example/request/', $queryPath, '/', $methodName,'.xml?',$params)"/></xsl:when>
                        <xsl:when test="$type='respxml'"><xsl:value-of select="concat('/WsEcl2/example/response/', $queryPath, '/', $methodName,'.xml?',$params)"/></xsl:when>
                        <xsl:when test="$type='xsd'"><xsl:value-of select="concat('/WsEcl2/definitions/query/', $queryPath, '/', $methodName,'/main/',$methodName,'.xsd')"/></xsl:when>
                        <xsl:when test="$type='wsdl'"><xsl:value-of select="concat('/WsEcl2/definitions/query/', $queryPath, '/', $methodName,'/main/',$methodName,'.wsdl')"/></xsl:when>
                        <xsl:when test="$type='action'"><xsl:value-of select="concat('/WsEcl2/submit/query/', $queryPath, '/', $methodName,$queryParams)"/></xsl:when>
                    </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
