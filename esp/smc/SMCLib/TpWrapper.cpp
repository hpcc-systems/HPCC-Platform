...
extern TPWRAPPER_API bool verifyDropZonePath(const char* dropZoneName, const char* netAddr, const char* pathToCheck)
{
    if (isEmptyString(netAddr) || isEmptyString(pathToCheck))
        throw MakeStringException(ECLWATCH_INVALID_INPUT, "NetworkAddress or Path not defined.");

    if (hasSpecialPathCharacters(pathToCheck)) //Detect a path like: /home/lexis/runtime/var/lib/HPCCSystems/mydropzone/../../../
        return false;
    ...
    ForEachItemIn(i, allTpDropZones)
    {
        ...
        unsigned dirLen = directory.length();
        if ((strlen(pathToCheck) < dirLen) || (strnicmp(pathToCheck, directory.str(), dirLen) != 0))
            continue;
