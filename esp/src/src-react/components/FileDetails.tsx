...
export const FileDetails: React.FunctionComponent<FileDetailsProps> = ({
    ...
    React.useEffect(() => {
        ...
        if (file?.DFUFilePartsOnClusters?.DFUFilePartsOnCluster?.length > 0) {
            const part = file?.DFUFilePartsOnClusters?.DFUFilePartsOnCluster[0];
            setCanReplicateFlag(part.CanReplicate);
            setReplicateFlag(part.Replicate);
        }

        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [file?.Description, file?.ProtectList?.DFUFileProtect, file?.IsRestricted, file?.DFUFilePartsOnClusters]);
