...
import { CopyRegular, DismissRegular, InfoRegular, FolderZipRegular, LockClosedRegular, LockOpenRegular } from "@fluentui/react-icons";
...
import { convertedSize, copyToClipboard, formatDecimal, safeFormatNum } from "src/Utility";
    ...
    const [DeleteConfirm, setShowDeleteConfirm] = useConfirm({
        ...
        onSubmit: React.useCallback(() => {
            dfuService.DFUArrayAction({ Type: WsDfu.DFUArrayActions.Delete, LogicalFiles: { Item: [file.Name] } }).then(({ ActionResults }) => {
                ...
                if (actionInfo.length && !actionInfo[0].Failed) {
                    replaceUrl("/files");
                } else {
                    logger.error(actionInfo?.[0]?.ActionResult ?? "DFU Delete Failed");
                }
                        ...
                        <div className={`${styles.fileSummaryWrapper} pane-content`}>
                            <div className={styles.fileSummaryHeader}>
                                <h2>
                                    ...
                                    <InfoRegular style={{ margin: "2px 6px 0 0" }} />
                                    {file?.Name}
