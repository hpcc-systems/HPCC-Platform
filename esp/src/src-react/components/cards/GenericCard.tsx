...
export interface GenericCardProps {
    ...
    /** Minimum number of header action buttons to keep visible before overflowing (default 2) */
    headerActionsMinVisible?: number;
    /** If true, render the header actions floating over the content (no header row space) */
    headerOverlay?: boolean;

    /** When true, GenericCard will render a built-in Minimize/Maximize toggle action */
    minimizable?: boolean;
    /** Current minimized state, used to choose icon/label for the built-in toggle */
    minimized?: boolean;
    /** Handler invoked when the built-in toggle is clicked */
    ...
    /** If true, when not minimized the card will span 2x2 cells in a fixed 160px grid to grow width and height */
...
export const GenericCard: React.FC<GenericCardProps> = ({
    ...
    const computedStyle: React.CSSProperties = React.useMemo(() => {
        // Default to fill parent cell; allow caller style to override
    ...
    const actionNodes = React.useMemo(() => {
        ...
        // Built-in Minimize/Maximize toggle (added after caller-provided actions)
    ...
    // Build overflow-aware toolbar items and menu items from provided headerActions
    const [toolbarItems, menuItems, idToOnClick] = React.useMemo(() => {
        ...
        actionNodes.forEach((child, idx) => {
            ...
            // Pass through explicit dividers/groups without overflow behavior
            if (child.type === ToolbarDivider) {
                items.push(child);
                return;
            }

            // If caller provided an OverflowItem, include as-is and try to infer menu entry from its child
            if (childEl.type === OverflowItem) {
                const id = (childEl.props?.id as string) || `hdr-action-${idx}`;
                if (seenIds.has(id)) return; // avoid duplicates
                seenIds.add(id);
                ...
                if (inner && React.isValidElement(inner)) {
                    // If inner is a ToolbarButton, extract icon/label/click
            ...
            // If the caller provided a ToolbarButton directly
            ...
            // Back-compat: Convert generic buttons with icon/handlers into toolbar buttons
            // Derive a stable id
            let id = (childEl.props?.id as string) || `hdr-action-${idx}`;
            while (seenIds.has(id)) id = `${id}-${idx}`;
            seenIds.add(id);
    ...
    // Ensure the header text gets proper truncation, including Tooltip+Text cases
    const renderHeaderText = React.useCallback((node?: React.ReactNode) => {
        ...
        if (React.isValidElement(node)) {
            // Try to detect Tooltip with Text child, preserve Tooltip and inject truncate on Text
        ...
        // Fallback: wrap custom content in truncation container
