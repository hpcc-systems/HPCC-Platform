...
const DiskUsageCard: React.FunctionComponent<DiskUsageCardProps> = ({
    cluster,
    percentUsed,
    machines,
    disks,
...
export const DiskUsageCards: React.FunctionComponent<DiskUsageCardsProps> = ({
    refreshToken
}) => {
...
const FolderDiskUsageCard: React.FunctionComponent<FolderDiskUsageCardProps> = ({
    folder,
    inUse,
    total,
    className,
    ...
    return <GenericCard
        ...
        headerActions={<></>}
        contentClassName={minimized ? styles.content : styles.details}
        footerText={!minimized ? `${nlsHPCC.InUse}: ${Utility.convertedSize(inUse)} • ${nlsHPCC.Total}: ${Utility.convertedSize(total)}` : undefined}
    >
        {
            minimized ?
                <AutosizeHpccJSComponent widget={gauge} padding={6} /> :
...
export const FolderUsageCards: React.FunctionComponent<FolderUsageCardsProps> = ({
    cluster,
    refreshToken
}) => {
