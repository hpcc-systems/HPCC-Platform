...
export const WorkunitDetails: React.FunctionComponent<WorkunitDetailsProps> = ({
    ...
    const buttons = React.useMemo((): ICommandBarItemProps[] => [
        ...
        },
        {
            ...
            onClick: () => {
                // there seems to be an issue with clone() returning a 404 when
                // attempting to refresh / query the status of the new workunit
                ...
                formData.append("Wuids", wuid);
                formData.append("ResetWorkflow", "0");
                formData.append("CloneWorkunit", "1");
                fetch("/WsWorkunits/WUResubmit.json", {
                    method: "POST",
                    body: formData
                })
                ...
                .then(json => {
                    let clonedWU = null;
                    if (json?.WUResubmitResponse?.WUs?.WU.length === 1) {
                        clonedWU = json?.WUResubmitResponse?.WUs?.WU[0];
                        pushUrl(`/workunits/${clonedWU?.WUID}`);
                    }
                });
            }
        },
