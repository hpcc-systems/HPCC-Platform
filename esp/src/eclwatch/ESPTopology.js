...
define([
    ...
    var TopologyItem = declare([ESPTree.Item], {
        ...
        getLogDirectory: function () {
            if (this.__hpcc_parent && this.__hpcc_parent.LogDirectory) {
    ...
    var ServiceType = declare([TpCommon], {
        ...
        addServices: function (items) {
            arrayUtil.forEach(items, function (item) {
                //  Push Bindings/LogDirectory into machines  ---
                /*
                if (lang.exists("TpMachines.TpMachine", item) {
                    if (lang.exists("TpBindings.TpBinding", item)) {
                        arrayUtil.forEach(item.TpMachines.TpMachine, function (machine) {
                            machine.TpBindings = item.TpBindings;
                        });
                    }
                    if (lang.exists("LogDirectory", item)) {
                        arrayUtil.forEach(item.TpMachines.TpMachine, function (machine) {
                            machine.LogDirectory = item.LogDirectory;
                        });
                    }
                }
                */
    ...
    var TopologyTreeStore = declare([ESPTree.Store], {
        ...
        createTreeNode: function (parentNode, treeItem) {
            ...
            retVal.hasLogs = function () {
                return this.__hpcc_treeItem.OS && this.__hpcc_treeItem.Netaddress && (this.__hpcc_treeItem.LogDirectory || this.__hpcc_parentNode.__hpcc_treeItem.LogDirectory);
            };
