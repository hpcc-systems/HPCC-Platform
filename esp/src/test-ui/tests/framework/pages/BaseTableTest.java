...
public abstract class BaseTableTest<T> {
	...
	Common.logDebug("TEST_DETAIL_PAGE_FIELD_NAMES_ALL: '" + Config.TEST_DETAIL_PAGE_FIELD_NAMES_ALL + "'.");
    ...
    private void testPresenceOfColumnNames(List<String> columnNames, String tabValue) {
        ...
        double ratio = 100 * elementFound / numOfElements;
        if ( ratio < 50 ) {
            Common.logError("Failure: "+ getPageName() + " Details Page. Tab click on: '" + tabValue + "' -> numOfElements:" + numOfElements + ", elementFound:" + elementFound + ", ratio: " + String.format("%3.0f%%",ratio) + "." );
        } else {
            Common.logDetail("Success: " + getPageName() + " Details Page. Tab click on: " + tabValue + " (WEB element found ratio: " + String.format("%3.0f%%",ratio) + ").");
        }
    }
            ...
            for (int i = 0; i < numOfItems; i++) {
                ...
                try {
                    ...
                    if (Common.driver.getPageSource().contains(name)) {
                        String msg = "Success: " + getPageName() + ": Link Test Pass for " + (i + 1) + "/" + numOfItems + ". " + name + ". URL : " + href;
    ...
    private void testContentAndSortingOrder() {
        ...
        Common.logDetail("Tests started for: " + getPageName() + " page: Testing Content");

        if (testTableContent()) {

            Common.logDetail("Tests started for: " + getPageName() + " page: Testing Sorting Order");
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (currentSortOrder != null) {
                ...
                if (compareData(columnDataFromUI, columnDataFromJSON, columnDataIDFromUI, columnName)) {
                    Common.logDetail("Success: " + getPageName() + ": Values are correctly sorted in " + currentSortOrder + " order by: " + columnName);
                } else {
