...
import framework.model.WUECLWorkunit;
...
public class ECLWorkUnitsTest extends BaseTableTest<WUECLWorkunit> {
    ...
    String sortByColumnKeyWhenSortedByNone = "Wuid";
    ...
    @Override
    protected Map<String, WUECLWorkunit> getJsonMap() {
        return jsonMap;
    }
	...
	Common.logDebug("TEST_WU_DETAIL_PAGE_PROTECTED_ALL: '" + Config.TEST_WU_DETAIL_PAGE_PROTECTED_ALL + "'.");
        if (Config.TEST_WU_DETAIL_PAGE_PROTECTED_ALL) { // TEST_WU_DETAIL_PAGE_PROTECTED_ALL = true means the test will run for all workunits and false means it will only run for the first workunit
            testProtectedButtonFunctionality(wuName);
        } else if (i == 0) {
            testProtectedButtonFunctionality(wuName);
        }

	Common.logDebug("TEST_WU_DETAIL_PAGE_DESCRIPTION_ALL: '" + Config.TEST_WU_DETAIL_PAGE_DESCRIPTION_ALL + "'.");
        if (Config.TEST_WU_DETAIL_PAGE_DESCRIPTION_ALL) { // TEST_WU_DETAIL_PAGE_DESCRIPTION_ALL = true means the test will run for all workunits and false means it will only run for the first workunit
    ...
    private void testDescriptionUpdateFunctionality(String wuName) {
        Common.logDebug("Test started for: Description " + getPageName() + " Details Page. For: " + wuName);
    ...
    private void testProtectedButtonFunctionality(String wuName) {
        Common.logDebug("Test started for: Protected checkbox " + getPageName() + " Details Page. For: " + wuName);
    ...
    Map<String, WUECLWorkunit> jsonMap = new HashMap<>();

    @Override
    protected List<WUECLWorkunit> parseJson(String filePath) throws Exception {
        ...
        List<WUECLWorkunit> WUECLWorkunits = wuQueryResponse.getWorkunits().getECLWorkunit();

        for (WUECLWorkunit workunit : WUECLWorkunits) {
            jsonMap.put(workunit.getWuid(), workunit);
        }

        return WUECLWorkunits;
    }

    @Override
    protected Object getColumnDataFromJson(WUECLWorkunit workunit, String columnKey) {
