...
import { Tabbed } from "@hpcc-js/layout";
import { DirectoryTree, Treemap } from "@hpcc-js/tree";
...
export class ECLArchiveWidget {
    ...
    updateTreeWidget(treeKey) {
        switch(treeKey){
            case "directoryview":
                this.tabbedPanel.activeTabIdx(0).render();
                break;
            case "treemapview":
                this.tabbedPanel.activeTabIdx(1).render();
                break;
        }
    }
    ...
    init(params) {
        if (this.inherited(arguments))
            return;
        const context = this;
        this.tabbedPanel = new Tabbed();
        this.directoryTree = new DirectoryTreeEx()
            .textFileIcon("fa fa-file-code-o")
            .omitRoot(true)
            ;
        this.treemapTree = new Treemap()
            .paddingInner(1)
            .paddingOuter(1)
            .tilingMethod("treemapResquarify")
            .showParentWeight(false)
            .paddingTop(30)
            ;
        this.treemapTree.leafWeightHTML = (d) => {
            return `<span class="treemap-leaf-label">${d.data.label}</span>
            <br/>
            <span class="treemap-leaf-value">${d.data.valueText}</span>`;
        };
        this.treemapTree.click = (row, column, selected) => {
            console.log("row, column, selected", row, column, selected);
        };
            
        this.tabbedPanel.addTab(this.directoryTree, "Files");
        this.tabbedPanel.addTab(this.treemapTree, "Tree");
        ...
        wu.fetchQuery().then(function (query) {
            ...
            if (!wu.HasArchiveQuery) {
                ...
                context.treemapTree.data(initialData);
    ...
    renderArchive(archiveXmlStr, markerData) {
        ...
        this.directoryTree
            .data({})
            .render(function () {
                context.treemapTree.data(formatTreemapData(JSON.parse(JSON.stringify(data))));
                context.directoryTree
                    .data(data)
                    .iconSize(16)
                    .rowItemPadding(2)
                ...
                context.treemapTree.rowClick = directoryTreeClick;
                if (!context.archiveViewer.isDOMHidden()) {
                    context.archiveViewer.relativeSizes([0.38, 0.62]).resize().render();
                }
            });
        ...
        function formatTreemapData(data) {
            if(data.children) {
                ...
                // data.size = data.markers.length;
                // const avgNode = {label: "avg", size: 0, format: ""};
                // const sumNode = {label: "sum", size: 0, format: ""};
                // let format = "";
                // const rawSize = data.markers.reduce((r, n)=>{
                //     r += Number(n?.rawTime?.RawValue ?? 0);
                //     if(n?.rawTime?.Measure)format = n?.rawTime?.Measure;
                //     return r;
                // }, 0);
                // const avgSize = data.markers.reduce((r, n)=>{
                //     r += Number(n?.rawTime?.RawValue ?? 0);
                //     if(n?.rawTime?.Measure)format = n?.rawTime?.Measure;
                //     return r;
                // }, 0)/data.markers.length;
                // sumNode.size = rawSize;
                // avgNode.size = avgSize;
                // sumNode.format = format;
                // avgNode.format = format;
                // data.children = [
                //     sumNode,
                //     avgNode
                // ];
                data.children = data.markers.map(n=>{
                    return {
                        label: `line ${n.lineNum}`,
                        size: Number(n.rawTime.RawValue ?? 0),
                        valueText: n.rawTime.Formatted
                    };
                });
            }
            return data;
...
function _formatTime(v, asString = true){
    let ret;
    if(v > 1000000000) {
        ret = v / 1000000000;
        if(asString)ret = ret.toFixed(3) + "s";
    } else {
        ret = v / 1000000;
        if(asString)ret = ret.toFixed(3) + "ms";
    }
    return ret;
}
function _formatSize(v, asString = true) {
    let ret;
    if (v > 1000000000) {
        ret = v * 0.000000000931;
        if(asString)ret = ret.toFixed(3) + "Gb";
    }
    else if (v > 1000000) {
        ret = v * 0.0000009537;
        if(asString)ret = ret.toFixed(3) + "Mb";
    } else {
        ret = v * 0.000977;
        if(asString)ret = ret.toFixed(3) + "Kb";
    }
    return ret;
}