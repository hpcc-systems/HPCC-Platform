...
import { LightThemed } from "./Themed";
...
export function lightThemedRender<P>(C: React.FunctionComponent<P>, props: Readonly<P>, _parent: Element | Document | ShadowRoot | DocumentFragment | string) {
    const parent = typeof _parent === "string" ? document.getElementById(_parent) : _parent;
    ReactDOM.render(
        React.createElement(LightThemed, null, React.createElement(C, props)),
        parent
    );
}
