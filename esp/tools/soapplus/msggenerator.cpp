    ...
    :  m_keepfile(keepfile), m_schemaType(st)
{
    ...
    if (globals->queryProp("cfg"))
    {
        ...
        if (loadFile(cfg, globals->queryProp("cfg")))
            ...
            OERRLOG("Can not load cfg file; ignored");
    }
...
void MessageGenerator::genRoxieMessage(const char* templatemsg, StringBuffer& message)
{
    ...
    for (it->first(); it->isValid(); it->next())
    {
        ...
        if (!name)
        {
            OERRLOG("XmlSchema without name");
            continue;
        }
        ...
        if (!type)
        {
            OERRLOG("Can not find type '%s'", name);
            continue;
        }
        // get the Row type
        ...
        if (!type)
        {
            OERRLOG("The root element for %s is not an array", name);
            continue;
        }
