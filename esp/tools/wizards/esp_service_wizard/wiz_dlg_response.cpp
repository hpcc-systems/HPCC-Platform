// response_wiz_dialog.cpp : implementation file
//
...
CEspResponseWizDlg::CEspResponseWizDlg()
    : CAppWizStepDlg(CEspResponseWizDlg::IDD), inited(false)
{
    //{{AFX_DATA_INIT(CEspResponseWizDlg)
    m_parameterToAdd = _T("");
    m_typeToAdd = 2;
    //}}AFX_DATA_INIT
    m_count=0;
...
void CEspResponseWizDlg::DoDataExchange(CDataExchange* pDX)
{
    CAppWizStepDlg::DoDataExchange(pDX);
    //{{AFX_DATA_MAP(CEspResponseWizDlg)
...
BOOL CEspResponseWizDlg::OnDismiss()
{
    if (!UpdateData(TRUE))
        return FALSE;
    ...
    while (--m_count>=0)
    {
        strKey.Format("ESP_RESP_PARM_%d", m_count);
    ...
    espaw.m_Dictionary["ESP_RESP_PARM_COUNT"]=itoa(count, strVal, 10);
...
BEGIN_MESSAGE_MAP(CEspResponseWizDlg, CAppWizStepDlg)
    //{{AFX_MSG_MAP(CEspResponseWizDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CEspResponseWizDlg message handlers
