...
public:
    CServiceDistributedFile(IDFSFile *_dfsFile) : PARENT(_dfsFile)
    {
        ...
        // it is stored in turn into the IFileDescriptor properties for use by clients accessing paths (see getBaseDir())
        fileDesc->queryProperties().setPropInt("@accessMode", dfsFile->queryCommonMeta()->getPropInt("@accessMode"));
