...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:param name="escapeResults" select="count(/DFUSearchDataResponse)"/>
    ...
    <xsl:for-each select="$schema//xs:element[@type!='']">
        <xsl:text disable-output-escaping="yes"><![CDATA[<col id="_col_]]></xsl:text><xsl:value-of select="position()"/><xsl:text disable-output-escaping="yes"><![CDATA["></col>]]></xsl:text>    
      </xsl:for-each>
    <xsl:text disable-output-escaping="yes"><![CDATA[
    </colgroup>
    ]]></xsl:text>
    ...
    <xsl:variable name="rowclass" select="($rowStart+position()) mod 2" />
        <tr id="row_{$rowStart+position()}">
            <xsl:if test="($level mod 2)=0">
                <xsl:attribute name="class">blue</xsl:attribute>
            </xsl:if>
            <xsl:for-each select="$nodes//data[../@level=$level]">
                <xsl:variable name="pos" select="position()"/>
                <xsl:variable name="hide" select="/DFUSearchDataResponse/ColumnsHidden/ColumnHidden[position() = $pos]/ColumnSize = 0"/>
                <td class="rt{$rowclass}">
          ...
          <xsl:if test="position() = 1 and $level = 1">
            <xsl:text disable-output-escaping="yes"><![CDATA[<a href="javascript:void(0)" onclick="viewRow(]]></xsl:text>
    ...
    <xsl:template name="show-header">
        ...
        <xsl:with-param name="parentname" select="translate(@name, '_', ' ')" />
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
