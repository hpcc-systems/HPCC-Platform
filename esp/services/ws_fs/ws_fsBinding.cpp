            ...
            if (*sourceLogicalFile.str())
            {
                ...
                if(username.length() > 0)
                {
                    ...
                    try 
                    {
                        ...
                        {
                            Owned<IDistributedFile> df = queryDistributedFileDirectory().lookup(sourceLogicalFile.str(), userdesc.get(), AccessMode::readLogicalMeta, false, false, nullptr, defaultPrivilegedUser);
                            if(!df)
                            {
