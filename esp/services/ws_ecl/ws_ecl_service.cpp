...
{
    schema.appendf("<xsd:element name=\"%s%s\">", wuinfo.queryname.sget(), isRequest ? "Request" : "Response");
...
int CWsEclBinding::getGenForm(IEspContext &context, CHttpRequest* request, CHttpResponse* response, WsEclWuInfo &wuinfo, bool box)
{
    ...
    appendXMLTag(formxml, "QuerySet", wuinfo.qsetname.sget());
    appendXMLTag(formxml, "QueryName", wuinfo.queryname.sget());
    ...
    IConstWUWebServicesInfo *ws = wu->getWebServicesInfo();
    if (ws)
    {
