...
void Cws_machineEx::init(IPropertyTree *cfg, const char *process, const char *service)
{
    ...
    //usageCacheReader.setown(new CUsageCacheReader((Cws_machineEx *) this));
...
void Cws_machineEx::readOtherComponentUsageReq(const char* name, const char* type, IConstEnvironment* constEnv, IPropertyTree* usageReq)
{
    ...
    if (isEmptyString(componentType))
    ...
    IEspGetComponentUsageResponse& resp)
{
    try
    {
        ...
        bool bypassCachedResult = req.getBypassCachedResult();
        if (!bypassCachedResult)
        {
        ...
        readComponentUsageResult(context, usageReq, bypassCachedResult ? uniqueUsages.getLink() : usage->queryUsages(), componentUsages);
        if (version >= 1.17)
    ...
    IEspGetTargetClusterUsageResponse& resp)
{
    try
    {
        ...
        readTargetClusterUsageResult(context, usageReq, bypassCachedResult ? uniqueUsages.getLink() : usage->queryUsages(), targetClusterUsages);
        if (version >= 1.17)
            resp.setUsageTime(setUsageTimeStr(usage, timeStr));
    ...
    IEspGetNodeGroupUsageResponse& resp)
{
    try
    {
        ...
        readNodeGroupUsageResult(context, usageReq, bypassCachedResult ? uniqueUsages.getLink() : usage->queryUsages(), nodeGroupUsages);
        if (version >= 1.17)
            resp.setUsageTime(setUsageTimeStr(usage, timeStr));
