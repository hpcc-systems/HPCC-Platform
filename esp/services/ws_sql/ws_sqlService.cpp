...
bool CwssqlEx::onGetDBMetaData(IEspContext &context, IEspGetDBMetaDataRequest &req, IEspGetDBMetaDataResponse &resp)
{
    ...
    if (includeStoredProcs)
    {
        ...
        PROGLOG("WsSQL: getting containerTargetClusters...");
        Owned<IStringIterator> targets = getContainerTargetClusters(nullptr, nullptr);
#else
        PROGLOG("WsSQL-legacy: getting targetClusters...");
...
#ifdef _CONTAINERIZED
            DBGLOG("WsSQL: getting containerTargetClusters...");
            Owned<IStringIterator> targets = getContainerTargetClusters(nullptr, nullptr);
            SCMStringBuffer target;
            ForEach(*targets)
            {
                const char *setname = targets->str(target).str();
                DBGLOG("WsSQL: found containerTargetClusters: %s", setname);
                dfuclusters.append(setname);
            }
#else
            DBGLOG("WsSQL-legacy: getting getTargetClusterList...");
...
bool CwssqlEx::onExecuteSQL(IEspContext &context, IEspExecuteSQLRequest &req, IEspExecuteSQLResponse &resp)
{
    try
    {
        ...
        DBGLOG("WsSQL: Parsing sql query...");
        parsedSQL.setown(parseSQL(context, sqltext));
        DBGLOG("WsSQL: Finished parsing sql query...");
        ...
        if (querytype == SQLTypeCall)
        {
            ...
            DBGLOG("WsSQL: Processing call query...");
...
void CwssqlEx::createWUXMLParams(StringBuffer & xmlparams, const IArrayOf <ISQLExpression> * parameterlist)
{
    ...
    for (int expindex = 0; expindex < parameterlist->length(); expindex++)
    {
        ...
        if (exp->getExpType() == Value_ExpressionType)
        {
            ...
            WARNLOG("WsSQL: attempted to create XML params from unexpected expression type.");
    }
    xmlparams.append("</root>");
...
bool CwssqlEx::addQueryToCache(const char * sqlQuery, const char * wuid)
{
    if (sqlQuery && *sqlQuery && wuid && *wuid)
    {
        ...
        if (isCacheExpired())
        {
            DBGLOG("WsSQL: Query Cache has expired and is being flushed.");
...
bool CwssqlEx::onCreateTableAndLoad(IEspContext &context, IEspCreateTableAndLoadRequest &req, IEspCreateTableAndLoadResponse &resp)
{
    ...
    DBGLOG("WsSQL: compiling WU...");
    WsWuHelpers::submitWsWorkunit(context, compiledwuid.str(), cluster, nullptr, 0, 0, true, false, false, nullptr, nullptr, nullptr, nullptr);
    waitForWorkUnitToCompile(compiledwuid.str(), req.getWait());

    DBGLOG("WsSQL: finish compiling WU...");
    ...
    {
        DBGLOG("WsSQL: executing WU(%s)...", compiledwuid.str());
        WsWuHelpers::submitWsWorkunit(context, compiledwuid.str(), cluster, nullptr, 0, 0, false, true, true, nullptr, nullptr, nullptr, nullptr);

        DBGLOG("WsSQL: waiting on WU(%s)...", compiledwuid.str());
        waitForWorkUnitToComplete(compiledwuid.str(), req.getWait());
        DBGLOG("WsSQL: finished waiting on WU(%s)...", compiledwuid.str());
