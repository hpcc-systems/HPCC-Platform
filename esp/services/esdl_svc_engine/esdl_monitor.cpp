    ...
    virtual void onNotify(EsdlNotifyData* data) override
    {
        ...
        else if (ntype == EsdlNotifyType::BindingDelete)
        {
            ...
            if (theBinding)
            {
                ...
                WARNLOG("Can't delete binding %s, it's not currently registered", data->id.str());
        }
        else if (ntype == EsdlNotifyType::BindingUpdate)
        {
            ...
            if (!theBinding)
            {
                WARNLOG("Binding %s not found, can't update", data->id.str());
                return;
            }
        ...
        else if (ntype == EsdlNotifyType::BindingAdd)
        {
            if (!espProcessMatch(data->espProcess.str()))
            {
                WARNLOG("ESDL binding %s is not for this esp process, ignore.", data->id.str());
                return;
            }
            ...
            if (data->port == 0)
            {
                WARNLOG("Port is not provided for binding, can't create binding.");
                return;
            }
            addBinding(data);
        }
