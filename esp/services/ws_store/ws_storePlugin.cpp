...
{
    if (strieq(type, "ws_store"))
    {
        CwsstoreEx* service = new CwsstoreEx;
...
ESP_FACTORY IEspRpcBinding * esp_binding_factory(const char *name, const char* type, IPropertyTree *cfg, const char *process)
{
    ...
    if (strieq(type, "ws_storeSoapBinding")||strieq(type, "ws_store_http"))
    {
        CwsstoreSoapBinding* binding = new CwsstoreSoapBindingEx(cfg, name, process);
        return binding;
    }
    return NULL;
}
