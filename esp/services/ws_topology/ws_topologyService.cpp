...
void CWsTopologyEx::checkFilePath(const char *file, const char *fileType, const char *compType, const char *compName)
{
    StringBuffer actualPath;
    splitUNCFilename(file, nullptr, &actualPath, nullptr, nullptr);
    if (hasSpecialPathCharacters(actualPath)) //Detect a path like: /home/lexis/runtime/var/log/HPCCSystems/myesp/../../../
        throw makeStringExceptionV(ECLWATCH_INVALID_INPUT, "Invalid file path %s", actualPath.str());
    if (!checkConfigurationDirectory(nullptr, fileType, compType, compName, actualPath))
        throw makeStringExceptionV(ECLWATCH_INVALID_INPUT, "Invalid file path %s", actualPath.str());
}
...
bool CWsTopologyEx::onSystemLog(IEspContext &context,IEspSystemLogRequest  &req, IEspSystemLogResponse &resp)
{
    try
    {
        ...
        {
            checkFilePath(name, "log", req.getCompType(), nullptr);
            logname = name;
        }
...
void CWsTopologyEx::readTpLogFile(IEspContext &context,const char* fileName, const char* fileType, IEspTpLogFileRequest  &req, IEspTpLogFileResponse &resp)
{
    ...
    {
        checkFilePath(fileName, "log", req.getCompType(), nullptr);
        logname = fileName;
    }
    else
    ...
    IEspTpGetComponentFileResponse &resp)
{
    try
    {
        ...
        checkFilePath(uncPath, (stricmp(fileType, "log") == 0) ? "log" : "run", compType, compName);
