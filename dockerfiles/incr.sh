...
if [[ -n "$FORCE" ]] ; then
  ...
  build_image platform-build platform-build --build-arg BUILD_USER=${BUILD_USER} --build-arg NUGET_ACTOR=${BUILD_USER} --build-arg NUGET_TOKEN=${GITHUB_TOKEN} --build-arg BUILD_TAG=${HEAD} --build-arg BUILD_THREADS=${BUILD_THREADS}
else
