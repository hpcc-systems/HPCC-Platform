  ...
  <xsl:template match="DaliServerProcess">
    <DALI>
      ...
      <xsl:element name="SDS">
        ...
        <xsl:copy-of select="@nobackup | @recoverFromIncErrors | @saveBinary | @storeBinaryCrc | @snmpSendWarnings | @enableSNMP | @enableSysLog | @snmpErrorMsgLevel | @msgLevel | @lightweightCoalesce | @keepStores | @deltaSaveThresholdSecs | @deltaTransactionQueueLimit | @deltaTransactionMaxMemMB | @leakStore"/>
