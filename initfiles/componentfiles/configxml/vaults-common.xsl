  ...
  <xsl:template name="validateVaultTypeKind">
    <xsl:for-each select="/Environment/Software/vaults/*">
      ...
      <xsl:if test="normalize-space(@name) = ''">
        <xsl:message terminate="yes">A vault definition is missing required attribute name.</xsl:message>
      </xsl:if>
      ...
      <xsl:if test="$isAkeyless and normalize-space(@accessType) != '' and normalize-space(@accessType) != 'access_key'">
        <xsl:message terminate="yes">Vault '<xsl:value-of select="$vaultName"/>' has invalid accessType '<xsl:value-of select="@accessType"/>'. Only 'access_key' is supported.</xsl:message>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
