...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xml:space="default" xmlns:seisint="http://seisint.com" exclude-result-prefixes="seisint">
<xsl:output method="xml" indent="yes"/>
<xsl:param name="process" select="'roxie'"/>
<xsl:param name="instance" select="'2wd20'"/>
<xsl:param name="isLinuxInstance" select="0"/>


<xsl:variable name="roxieNode" select="/Environment/Software/RoxieCluster[@name=$process]"/>
<xsl:variable name="pluginsPath">
    <xsl:variable name="path2" select="translate($roxieNode/@pluginsPath, '/$', '\:')"/>
    <xsl:variable name="path">
        <xsl:choose>
            <xsl:when test="starts-with($path2, '.\')">
                <xsl:value-of select="substring($path2, 3)"/>
                <!--skip .\ prefix -->
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:when test="$len > 0">
            <xsl:if test="$isLinuxInstance = 0">
              <xsl:if test="starts-with($path, '\') or ($len>1 and substring($path, 2, 1) = ':')">
                  <xsl:message terminate="yes">Plugins path for '<xsl:value-of select="$process"/>' must be a relative path!</xsl:message>
              </xsl:if>
            </xsl:if>
...
<xsl:template match="/">
    ...
    <xsl:apply-templates select="$roxieNode"/>
</xsl:template>


<xsl:template match="Software/RoxieCluster">
    <!--found matching component for given process type and process name-->
    <Plugins>
        <xsl:if test="@name=$process">
            <xsl:variable name="buildSetPath">
                <xsl:call-template name="getBuildSetPath">
                    <xsl:with-param name="build" select="$roxieNode/@build"/>
                    <xsl:with-param name="buildSet" select="$roxieNode/@buildSet"/>
                </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:apply-templates select="PluginRef"/>
            <!--handle plugins for data models-->
            <xsl:for-each select="/Environment/Software/Topology/RoxieCluster[@process=$process]/Cluster[HoleCluster]">
                <!--Cluster having at least one HoleCluster-->
                <xsl:variable name="firstHoleClusterName" select="HoleCluster[1]/@process"/>
                <xsl:variable name="firstHoleCluster" select="/Environment/Software/HoleCluster[@name=$firstHoleClusterName]"/>
                <xsl:if test="not($firstHoleCluster)">
                    <xsl:message terminate="yes">Hole Cluster '<xsl:value-of select="$firstHoleClusterName"/>' is not defined!</xsl:message>
                </xsl:if>
                <xsl:variable name="dataBuild" select="$firstHoleCluster/@dataBuild"/>
                <xsl:variable name="dataBuildNode" select="/Environment/Data/Build[@name=$dataBuild]"/>
                <xsl:if test="not($dataBuildNode)">
                    <xsl:message terminate="yes">Data build '<xsl:value-of select="$dataBuild"/>' is not defined!</xsl:message>
                </xsl:if>
                <xsl:variable name="dataModelName" select="$dataBuildNode/@dataModel"/>
                <!--ensure all hole clusters use the same data model-->
                <xsl:for-each select="HoleCluster[position() > 1]">
                    <xsl:variable name="holeClusterName" select="@process"/>
                    <xsl:variable name="holeCluster" select="/Environment/Software/HoleCluster[@name=$holeClusterName]"/>
                    <xsl:if test="not($holeCluster)">
                        <xsl:message terminate="yes">Hole Cluster '<xsl:value-of select="$holeClusterName"/>' is not defined!</xsl:message>
                    </xsl:if>
                    <xsl:if test="$holeCluster/@dataModel != $dataModelName">
                        <xsl:message terminate="yes">Hole clusters for ECL server '<xsl:value-of select="$process"/>' have different data models!</xsl:message>
                    </xsl:if>
                </xsl:for-each>
                <xsl:if test="$dataModelName">
                    <xsl:call-template name="outputPluginInfo">
                        <xsl:with-param name="name" select="'model.xsl'"/>
                        <xsl:with-param name="type" select="'model'"/>
                        <xsl:with-param name="srcPath" select="$buildSetPath"/>
                        <xsl:with-param name="destName" select="concat($dataModelName, '.model')"/>
                        <xsl:with-param name="method" select="'model'"/>
                    </xsl:call-template>
                </xsl:if>
            </xsl:for-each>
        </xsl:if>
...
<xsl:template match="PluginRef">
    ...
    <xsl:if test="$pluginNode/InstallSet/File[1]">
        <!--custom plugin-->
        <xsl:variable name="basePath" select="translate($pluginNode/@basePath, '/', '\')"/>
        <xsl:variable name="len" select="string-length($basePath)"/>
        <xsl:if test="$len=0">
            <xsl:message terminate="yes">Base path is not specified for custom plugin '<xsl:value-of select="$pluginNode/@name"/>
            </xsl:message>
        </xsl:if>
...
<xsl:template match="File" mode="DefaultPlugins">
    ...
    <xsl:variable name="srcPath">
        ...
        <xsl:if test="string(@srcPath) != ''">
            ...
            <xsl:if test="starts-with($srcPathAttr, '\')">
                <xsl:value-of select="substring(@srcPathAttr, 2)"/>
            </xsl:if>
    ...
    <xsl:variable name="destName">
        <xsl:choose>
            <xsl:when test="string(@destName) != ''">
                <xsl:value-of select="translate(@destName, '/', '\')"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:if test="starts-with($name, 'plugins\') and ($extension='dll' or $extension='so')">
        <xsl:call-template name="outputPluginInfo">
...
<xsl:template match="File">
    ...
    <xsl:if test="$extension='dll' or $extension='so'">
        <xsl:call-template name="outputPluginInfo">
            <xsl:with-param name="name" select="@name"/>
            <xsl:with-param name="type" select="$extension"/>
            <xsl:with-param name="srcPath" select="$srcPath"/>
...
<xsl:template name="outputPluginInfo">
    <xsl:param name="name"/>
    <!--this can have a directory prefix like plugin\xyz.dll but any / converted to \ -->
    <xsl:param name="type"/>
    ...
    <Plugin>
        ...
        <xsl:if test="string($srcPath) != ''">
            ...
            <xsl:variable name="parameter">
                ...
                <xsl:value-of select="$srcName"/>
                <xsl:text>+</xsl:text>
                <xsl:value-of select="$srcPath2"/>
                <xsl:text>+</xsl:text>
                <xsl:value-of select="$destName2"/>
                <xsl:text>+</xsl:text>
