...
xmlns:xalan="http://xml.apache.org/xalan" xmlns:seisint="http://seisint.com" exclude-result-prefixes="xalan seisint">
    ...
    <xsl:template name="processServiceSpecifics">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$serviceType='ws_roxieconfig'">
...
This is required by its binding with ws_roxieconfig service '<xsl:value-of select="$espServiceName"/>'.</xsl:message>
                </xsl:if>
                <xsl:if test="string(@roxieCluster) = ''">
                    <xsl:message terminate="yes">Roxie config service '<xsl:value-of select="$espServiceName"/>': No Roxie cluster was specified!</xsl:message>
                </xsl:if>
                <xsl:variable name="roxieClusterNode" select="/Environment/Software/RoxieCluster[@name=current()/@roxieCluster]"/>
                <xsl:if test="not($roxieClusterNode)">
                    <xsl:message terminate="yes">Roxie config service '<xsl:value-of select="$espServiceName"/>': Roxie cluster '<xsl:value-of select="@roxieCluster"/>' is not defined!</xsl:message>
                </xsl:if>
                <xsl:variable name="firstRoxieServer" select="$roxieClusterNode/RoxieServerProcess[1]"/>
                <xsl:if test="string($firstRoxieServer/@computer)=''">
                    <xsl:message terminate="yes">Roxie config service '<xsl:value-of select="$espServiceName"/>': Roxie cluster '<xsl:value-of select="@roxieCluster"/>' has invalid server!</xsl:message>
                </xsl:if>
                <RoxieCluster>
                    <xsl:value-of select="@roxieCluster"/>
                </RoxieCluster>
                <RoxieAddress>
                    <xsl:value-of select="/Environment/Hardware/Computer[@name=$firstRoxieServer/@computer]/@netAddress"/>
                    <xsl:text>:</xsl:text>
                    <xsl:value-of select="$firstRoxieServer/@port"/>
                </RoxieAddress>
                <xsl:if test="string(@wuTimeout) = ''">
                    <xsl:message terminate="yes">Workunit timeout is not specified for Roxie config service '<xsl:value-of select="$espServiceName"/>'.</xsl:message>
                </xsl:if>
                <xsl:if test="string(@roxieTimeout) = ''">
                    <xsl:message terminate="yes">Roxie timeout is not specified for Roxie config service '<xsl:value-of select="$espServiceName"/>'.</xsl:message>
                </xsl:if>
                        ...
                        <AllowNewRoxieOnDemandQuery><xsl:value-of select="@allowNewRoxieOnDemandQuery"/></AllowNewRoxieOnDemandQuery>
                                ...
                                <WuTimeout>
                    <xsl:value-of select="@wuTimeout"/>
                </WuTimeout>
                <RoxieTimeout>
                    <xsl:value-of select="@roxieTimeout"/>
                </RoxieTimeout>
                <StyleSheets>
                    <xslt name="atts">smc_xslt/atts.xslt</xslt>
                    <xslt name="dot_update">smc_xslt/dot_update.xslt</xslt>
                    <xslt name="dot">smc_xslt/dot.xslt</xslt>
                    <xslt name="graphStats">./smc_xslt/graphStats.xslt</xslt>
                </StyleSheets>
                <SourceRepositories>
                    <xsl:for-each select="SourceAttributeServer | SourceRoxie">
                        <SourceRepository>
                            <xsl:for-each select="@name|@type|@netAddress">
                                <xsl:if test="string(.) = ''">
                                    <xsl:message terminate="yes">Incomplete definition for source attribute server or Roxie cluster '<xsl:value-of select="@name"/>' for Roxie config service.</xsl:message>
                                </xsl:if>
                                <xsl:attribute name="{name()}">
                                    <xsl:choose>
                                        <xsl:when test="name()='netAddress' and name(..)='SourceAttributeServer'">
                                            <xsl:if test="not(starts-with(., 'http://') and starts-with(.,'https://'))">
                                                <xsl:text>http://</xsl:text>
                                            </xsl:if>
                                            <xsl:variable name="protocolFreeAddress">
                                                <xsl:choose>
                                            ...
                                            <xsl:call-template name="addPortIfMissing">
                                                <xsl:with-param name="netAddress" select="$protocolFreeAddress"/>
                                                <xsl:with-param name="defaultPort" select="'8145'"/>
                                                <xsl:with-param name="prefix" select="'http://'"/>
                                            </xsl:call-template>
                                        </xsl:when>
                                        <xsl:when test="name()='netAddress' and name(..)='SourceRoxie'">
                                            <xsl:call-template name="addPortIfMissing">
                                                <xsl:with-param name="netAddress" select="."/>
                            ...
                            <xsl:if test="@type='Attribute Server'">
                                <xsl:attribute name="netAddress"><xsl:call-template name="GetEspBindingAddress"><xsl:with-param name="espBindingInfo" select="@espBinding"/></xsl:call-template></xsl:attribute>
                                <xsl:variable name="espName" select="substring-before(@espBinding, '/')"/>
                                <xsl:variable name="bindingName" select="substring-after(@espBinding, '/')"/>
                                ...
                                <!--already verified valid in GetEspBindingAddress -->
                                <xsl:variable name="serviceNode" select="/Environment/Software/EspService[@name=$bindingNode/@service]"/>
                                <xsl:variable name="eclServer" select="$serviceNode/@eclServer"/>
                                <xsl:if test="string($eclServer) = ''">
                                    <xsl:message terminate="yes">Roxie config service '<xsl:value-of select="$espServiceName"/>': ECL server is not specified in attribute server configuration!</xsl:message>
                                </xsl:if>
                                <xsl:variable name="eclServerNode" select="/Environment/Software/EclServerProcess[@name=$eclServer]"/>
                                <xsl:if test="not($eclServerNode)">
                                    <xsl:message terminate="yes">Roxie config service '<xsl:value-of select="$espServiceName"/>': ECL server '<xsl:value-of select="$eclServer"/>' is not defined!      </xsl:message>
                                </xsl:if>
                                <xsl:if test="$serviceNode/@repository != $eclServerNode/@repository">
                                    <xsl:message terminate="yes">
                                        <xsl:text>Roxie config service '</xsl:text>
                                        <xsl:value-of select="$espServiceName"/>
                                        <xsl:text>': ECL server and attribute server must use the same repository!</xsl:text>
                                    </xsl:message>
                                </xsl:if>
                                <xsl:attribute name="eclServerQueue"><xsl:value-of select="$eclServerNode/@queue"/></xsl:attribute>
                                <xsl:variable name="clusters" select="/Environment/Software/Topology/EclServerProcess[@process=$eclServer]/Cluster"/>
                                <xsl:variable name="clusterName">
                                    <xsl:choose>
                                        <xsl:when test="not($clusters) or string($clusters[1]/@name)=''">
                                            <xsl:message terminate="yes">Roxie config service '<xsl:value-of select="$espServiceName"/>': Failed to resolve ECL server cluster information!</xsl:message>
                                        </xsl:when>
                                        <xsl:when test="$clusters[@name='hthor']">hthor</xsl:when>
                                        <xsl:otherwise>
                                            <xsl:value-of select="$clusters[1]/@name"/>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                </xsl:variable>
                                <xsl:attribute name="clusterName"><xsl:value-of select="$clusterName"/></xsl:attribute>
                            </xsl:if>
                        </SourceRepository>
                    </xsl:for-each>
                </SourceRepositories>
                <NameServices>
                    <xsl:for-each select="NameService">
                        <xsl:if test="string(@name)=''">
                            <xsl:message terminate="yes">Roxie config service: Invalid name service definition!</xsl:message>
                        </xsl:if>
                        <xsl:if test="string(@daliAddress)=''">
                            <xsl:message terminate="yes">Roxie config service: No Dali server address defined for name service definition '<xsl:value-of select="@name"/>'!</xsl:message>
                        </xsl:if>
                        <NameService>
                            <Name>
                            ...
                            <DaliAddress>
                                <xsl:call-template name="addPortIfMissing">
                                    <xsl:with-param name="netAddress" select="@daliAddress"/>
                                    <xsl:with-param name="defaultPort" select="7070"/>
                                </xsl:call-template>
                            </DaliAddress>
                        </NameService>
                    </xsl:for-each>
                    <xsl:for-each select="SourceRoxie">
                        <NameService>
                            ...
                            <xsl:for-each select="../NameService">
                                <xsl:if test="$name=translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')">
                                    <xsl:message terminate="yes">Roxie config service: Name service name cannot be same as that of a source Roxie cluster!</xsl:message>
                                </xsl:if>
                            </xsl:for-each>
                            <Name>
                                <xsl:value-of select="@name"/>
                            ...
                            <RoxieAddress>
                                <xsl:call-template name="addPortIfMissing">
                                    <xsl:with-param name="netAddress" select="@netAddress"/>
                                    <xsl:with-param name="defaultPort" select="'9876'"/>
                                </xsl:call-template>
                            </RoxieAddress>
                        </NameService>
