            ...
            try
            {
                while (true)
                {
                    if (!gotSize)
                    {
                        ...
                        size32_t szRead;
                        sock->read(msgWritePtr, 0, sizeof(size32_t)-left, szRead); // can fire graceful close
                        left += szRead;
                        msgWritePtr += szRead;
                    ...
                    {
                        size32_t szRead;
                        sock->read(msgWritePtr, 0, left, szRead); // can fire graceful close
                        msgWritePtr += szRead;
                        left -= szRead;
        ...
        for (;;)
        {
            ...
            if (sockavail || securesockavail || rowServiceSockAvail)
            {
                ...
                if (rowServiceSockAvail)
                {
                    ...
                    if (isContainerized())
                        secure = true;
                    else if (_connectMethod != SSLNone)
                        secure = true;
                    addClient(acceptedRSSock.getClear(), secure, true);
                }
            }
            else
                checkTimeout();
