...
static void validateStore(bool fix, bool deleteFiles, bool removeStaleLocations, bool verbose)
{
    ...
    if (!generatedDllsConn)
        throw makeStringException(0, "Failed to connect to /GeneratedDlls");
        ...
        if (!entryRemoved && removeStaleLocations)
        {
            ...
            ForEach(*locationIter)
            {
                ...
                if (!isEmptyString(host))
                {
                    ...
                    {
                        if (verbose)
                            PROGLOG("Removing stale entry for %s at location %s", name, host);
    ...
    if (fix)
    {
        ...
        PROGLOG("Removed %d unassociated file entries, and %u stale locations. Took %u ms", numDeleted, removedLocations, ts.elapsed());
        ...
        PROGLOG("%d unassociated file entries to remove, and %u stale locations. Took %u ms - use 'fix=true' to remove them", numDeleted, removedLocations, ts.elapsed());
