        ...
        for (drv=0;drv<drvs;drv++) {
            ...
            for (unsigned pn=0;pn<f->N;pn++) {
                if (f->testpresent(drv,pn)&&!f->testmarked(drv,pn)) {
                    RemoteFilename rfn;
                    constructPartFilename(grp, pn+1, drv, f->N, fnameHash, drv, f->isDirPerPart, scopeBuf.str(), prefix, mask.str(), numDevices, rfn);
                    offset_t sz;
                    CDateTime dt;
                    bool found;
                    {
        ...
        if (ncomplete!=f->N) {  // if a found file ignore misplaces
            ...
            while (mp) {
                ...
                if (!mp->marked) {
                    ...
                    RemoteFilename rfn;
                    constructPartFilename(grp, mp->pn, drv, f->N, fnameHash, drv, f->isDirPerPart, scopeBuf.str(), prefix, mask.str(), numDevices, rfn);
                    offset_t sz;
                    CDateTime dt;
