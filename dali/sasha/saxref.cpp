...
//                              WARNLOG("DANXREF(scanFiles):  %s has alt group %s",filename,parent.clusters.item(i));
    ...
    void listLost(bool &abort,bool ignorelazylost)
    {
        ...
        ForEachItemIn(i0,lostfiles) {
            ...
            try {
                file.setown(queryDistributedFileDirectory().lookup(lfn,NULL));
            }
            catch (IException *e) {
...
public:
    ...
    void runExpiry()
    {
        ...
        Owned<IDFAttributesIterator> iter = queryDistributedFileDirectory().getDFAttributesIterator("*",NULL,true,false);//MORE:Pass IUserDescriptor
        ForEach(*iter) {
        ...
        ForEachItemIn(i,expirylist) {
            ...
            try {
                queryDistributedFileDirectory().removePhysical(lfn,NULL);//MORE:Pass IUserDescriptor
            }
