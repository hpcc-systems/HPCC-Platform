...
void XRefAllocator::dealloc(void *ptr, unsigned sz)
{
    free(ptr);
...
cMisplacedRec::cMisplacedRec(XRefAllocator *_allocator)
    : allocator(_allocator)
{}

cMisplacedRec *cMisplacedRec::create(XRefAllocator *allocator)
{
    return new(allocator) cMisplacedRec(allocator);
}

void *cMisplacedRec::operator new(size_t baseSize, XRefAllocator *allocator)
{
    return allocator->alloc(baseSize);
}

void cMisplacedRec::operator delete(void* p, XRefAllocator* allocator) noexcept
{
    // Called if constructor throws
    allocator->dealloc(p, sizeof(cMisplacedRec));
}

void cMisplacedRec::operator delete(void *ptr) noexcept
{
    cMisplacedRec *desc = static_cast<cMisplacedRec *>(ptr);
    desc->allocator->dealloc(ptr, sizeof(cMisplacedRec));
}

void cMisplacedRec::init(unsigned drv,
            unsigned pf,      // part
...
bool cMisplacedRec::eq(unsigned drv,
            unsigned pf,      // part
            unsigned xn,      // node located on
            unsigned tn)      // total nodes
{
...
unsigned cMisplacedRec::getDrv(unsigned tn)
{
    return nn/tn;
}

unsigned cMisplacedRec::getNode(unsigned tn)
{
    return nn%tn;
}


cFileDesc::cFileDesc(const char *_name, unsigned nameLen, unsigned mapLen, unsigned numParts, bool d, unsigned fnLen, XRefAllocator *_allocator)
    : allocator(_allocator)
{
...
cFileDesc *cFileDesc::create(const char *name, unsigned numParts, bool isDirPerPart, unsigned fnLen, XRefAllocator *allocator)
{
    unsigned mapLen;
...
void *cFileDesc::operator new(size_t baseSize, unsigned nameLen, unsigned mapLen, XRefAllocator *allocator)
{
    return allocator->alloc(baseSize+nameLen+mapLen);
}

void cFileDesc::operator delete(void *ptr, XRefAllocator* allocator) noexcept
{
    // Called if constructor throws
...
void cFileDesc::operator delete(void *ptr) noexcept
{
    cFileDesc *desc = static_cast<cFileDesc *>(ptr);
...
unsigned cFileDesc::getSize() const
{
...
inline byte *cFileDesc::map() const
{
    return (byte *)&name+1+name[0];
}

bool cFileDesc::setpresent(unsigned d,unsigned i) // returns old value
{
    if (d)
        i += N*d;
    byte v = 1<<(i%8);
...
bool cFileDesc::testpresent(unsigned d,unsigned i)
{
    if (d)
        i += N*d;
...
bool cFileDesc::setmarked(unsigned d,unsigned i) // returns old value
{
    return setpresent(d+2,i);
}

bool cFileDesc::testmarked(unsigned d,unsigned i)
{
    return testpresent(d+2,i);
}

bool cFileDesc::eq(const char *key)
{
...
bool cFileDesc::isHPCCFile() const
{
    return filenameLen > 0;
}

bool cFileDesc::getName(StringBuffer &buf) const
{
    // Check mask exists
    if (filenameLen == 0)
        return false;
...
StringBuffer &cFileDesc::getNameMask(StringBuffer &buf) const
{
    return buf.append((size32_t)name[0],(const char *)(name+1));
}

StringBuffer &cFileDesc::getPartName(StringBuffer &buf,unsigned p) const
{
    StringBuffer mask;
    getNameMask(mask);
...
cFileDesc * cFileDesc::create(const char *)
{
    assertex(false);
    return NULL;
    // not used
...
unsigned cFileDesc::getHash(const char *key)
{
    size32_t sl = strlen(key);
    if (sl>255)
        sl = 255;
...
cDirDesc::cDirDesc(const char *_name, size32_t sl, XRefAllocator *_allocator)
    : allocator(_allocator)
{
    name[0] = (byte)sl;
...
cDirDesc *cDirDesc::create(const char * name, XRefAllocator *allocator)
{
...
void *cDirDesc::operator new(size_t baseSize, unsigned nameLen, XRefAllocator *allocator)
{
    return allocator->alloc(baseSize+nameLen);
}

void cDirDesc::operator delete(void *ptr, XRefAllocator* allocator) noexcept
{
    // Called if constructor throws
...
void cDirDesc::operator delete(void *ptr) noexcept
{
    cDirDesc *desc = static_cast<cDirDesc *>(ptr);
    desc->allocator->dealloc(ptr, desc->getSize());
}

unsigned cDirDesc::getSize() const
{
    size32_t nameLen = name[0];
...
bool cDirDesc::eq(const char *key)
{
    size32_t sl = strlen(key);
    if (sl>255)
        sl = 255;
...
unsigned cDirDesc::getHash(const char *key)
{
    size32_t sl = strlen(key);
    if (sl>255)
        sl = 255;
...
StringBuffer &cDirDesc::getName(StringBuffer &buf)
{
    return buf.append((size32_t)name[0],(const char *)name+1);
}

cDirDesc *cDirDesc::lookupDirNonThreadSafe(const char *name, XRefAllocator *allocator)
{
...
cDirDesc *cDirDesc::lookupDir(const char *name, XRefAllocator *allocator)
{
    CriticalBlock block(dirsCrit);
    return lookupDirNonThreadSafe(name, allocator);
}

const char *cDirDesc::decodeName(unsigned drv,const char *name,unsigned node, unsigned numnodes,
...
bool cDirDesc::isMisplaced(unsigned partNum, unsigned numParts, const SocketEndpoint &ep, IGroup &grp, const char *fullPath, unsigned filePathOffset, unsigned stripeNum, unsigned numStripedDevices)
{
...
cFileDesc *cDirDesc::addFile(unsigned drv,const char *name,const char *filePath,unsigned filePathOffset,unsigned node,const SocketEndpoint &ep,IGroup &grp,unsigned numnodes,unsigned stripeNum,unsigned numStripedDevices,XRefAllocator *allocator)
{
    unsigned nf;          // num parts
...
bool cDirDesc::markFile(unsigned drv,const char *name, unsigned node, const SocketEndpoint &ep, IGroup &grp, unsigned numnodes)
{
    unsigned nf;
    unsigned pf;
...
void cDirDesc::addNodeStats(unsigned node,unsigned drv,offset_t sz)
{
    if (drv>1)
        drv = 1;
...
bool cDirDesc::empty(unsigned drv)
{
...
cMessage::cMessage(const char *_lname,const char *_msg)
...
void mergeDirPerPartDirs(cDirDesc *parent, cDirDesc *dir, const char *currentPath)
{
    if (!isContainerized())
        return;
...
XRefPeriodicTimer::XRefPeriodicTimer(unsigned seconds, bool suppressFirst, const char *_clustname)
: clustname(_clustname) { reset(seconds, suppressFirst, clustname); }

unsigned XRefPeriodicTimer::calcElapsedMinutes() const
{
    int64_t elapsedNS = cycle_to_nanosec(lastElapsedCycles - startCycles);
    return elapsedNS / oneSecondNS / 60;
}

bool XRefPeriodicTimer::hasElapsed()
{
...
void XRefPeriodicTimer::reset(unsigned seconds, bool suppressFirst, const char *_clustname)
{
...
void XRefPeriodicTimer::updatePeriod()
{
...
void CNewXRefManagerBase::log(bool forceStatusUpdate, const char * format, ...)
{
    StringBuffer line;
    va_list args;
...
void CNewXRefManagerBase::error(const char *lname,const char * format, ...)
{
    CriticalBlock block(logsect);
    va_list args;
    va_start(args, format);
...
void CNewXRefManagerBase::warn(const char *lname,const char * format, ...)
{
    CriticalBlock block(logsect);
    va_list args;
    va_start(args, format);
...
void CNewXRefManagerBase::updateStatus(bool uncond)
{
    CriticalBlock block(logsect);
...
void CNewXRefManagerBase::startHeartbeat(const char * op)
{
    processedDirs = 0;
    processedFiles = 0;
...
void CNewXRefManagerBase::checkHeartbeat(const char * op)
{
...
void CNewXRefManagerBase::doSaveToDebugPlane(const char *name, StringBuffer &datastr)
{
...
void CNewXRefManagerBase::addBranch(IPropertyTree *root,const char *name,IPropertyTree *branch)
{
    if (!branch)
        return;
...
CNewXRefManagerBase::CNewXRefManagerBase()
{
    lastlog = 0;
    sfnum = 0;
    fnum = 0;
...
void CNewXRefManagerBase::start(bool updateeclwatch,const char *clname)
{
    StringBuffer xpath;
    {   // remove old tree
...
void CNewXRefManagerBase::finish(bool aborted)
{
    if (aborted)
        log(false, "Aborted");
...
void CNewXRefManagerBase::addErrorsWarnings(IPropertyTree *croot)
{
...
void CNewXRefManagerBase::saveToEclWatch(bool &abort,bool byscheduler)
{
    if (abort)
        return;
...
CNewXRefManager::CNewXRefManager(IPropertyTree *plane,unsigned maxMb, IPropertyTree *_serverConfig)
    : allocator(maxMb), serverConfig(_serverConfig)
{
...
CNewXRefManager::~CNewXRefManager()
{
    if (iphash)
        delete [] iphash;
    delete [] ipnum;
...
void CNewXRefManager::start(bool updateeclwatch)
{
    CNewXRefManagerBase::start(updateeclwatch,clustname);
}

void CNewXRefManager::addIpHash(const IpAddress &ip,unsigned n)
{
    unsigned r;
    _cpyrev4(&r,&ip);
...
unsigned CNewXRefManager::checkIpHash(const IpAddress &ip)
{
    unsigned r;
    _cpyrev4(&r,&ip);
    unsigned h = hashc((const byte *)&r,sizeof(r),0)%iphashsz;
...
bool CNewXRefManager::setGroup(const char *_clustname,const char *_grpname, IArrayOf<IGroup> &done, StringArray &donedir)
{
...
void CNewXRefManager::clear()
{
    root.reset(cDirDesc::create("", &allocator));
}

cDirDesc *CNewXRefManager::findDirectory(const char *name)
{
...
bool CNewXRefManager::dirFiltered(const char *filename)
{
    // TBD (e.g. collections)
    return false;
}

bool CNewXRefManager::fileFiltered(const char *filename,const CDateTime &dt)
{
...
bool CNewXRefManager::scanDirectory(unsigned node,const SocketEndpoint &ep,StringBuffer &path, unsigned drv, cDirDesc *pdir, IFile *cachefile, unsigned level, unsigned filePathOffset, unsigned stripeNum)
{
...
bool CNewXRefManager::scanDirectories(bool &abort, unsigned numThreads)
{
...
void CNewXRefManager::scanLogicalFiles(bool &abort)
{
    if (!grp||abort)
        return;
...
bool CNewXRefManager::checkOrphanPhysicalFile(RemoteFilename &rfn,offset_t &sz,CDateTime &dt)
{
    try {
...
void CNewXRefManager::addOrphanPartNode(Owned<IPropertyTree> &branch,const SocketEndpoint &ep,unsigned i,bool rep)
{
    if (!branch)
...
void CNewXRefManager::addExternalFoundFile(cFileDesc *file, const char *currentPath, unsigned int recentCutoffDays)
{
...
void CNewXRefManager::listOrphans(cFileDesc *f,const char *currentPath,const char *currentScope,bool &abort,unsigned int recentCutoffDays)
{
    if (abort)
        return;
    if (!f)
...
void CNewXRefManager::listDirectory(cDirDesc *d,const char *name,bool &abort)
{
    unsigned drvs = isContainerized() ? 1 : 2;
...
void CNewXRefManager::listOrphans(cDirDesc *d,StringBuffer &basedir,StringBuffer &scope,bool &abort,unsigned int recentCutoffDays)
{
...
int CNewXRefManager::compareDirs(IInterface * const *t1,IInterface * const *t2)
{
...
void CNewXRefManager::listOrphans(bool &abort,unsigned int recentCutoffDays)
{
...
void CNewXRefManager::listLost(bool &abort,bool ignorelazylost,unsigned int recentCutoffDays)
{
...
CSuperfileCheckManager::CSuperfileCheckManager()
{
    clustname.set("SuperFiles");
}

void CSuperfileCheckManager::start(bool updateeclwatch)
{
    CNewXRefManagerBase::start(updateeclwatch,"SuperFiles");
}

void CSuperfileCheckManager::errornotrecent(const char *lname,const char * format, ...)
{
...
void CSuperfileCheckManager::checkSuperFileLinkage()
{
