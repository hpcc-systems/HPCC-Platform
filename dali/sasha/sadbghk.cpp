...
public:
    ...
    void runDebugHousekeeping()
    {
        ...
        unsigned filesFailedToBeDeleted{0};
        ...
        ForEach(*pDirIter)
        {
            ...
            if (iFile.isDirectory() == fileBool::foundYes)
            {
                ...
                if (hasExpired(iFile))
                {
                    ...
                    ++dirsDeleted; // should be really be conditional on success of recursiveRemoveDirectory
                }
            }
            else
            {
                ...
                if (hasExpired(iFile))
                {
                    if (iFile.remove())
                        ...
                        ++filesFailedToBeDeleted;
                }
            }
        }

        if (dirsDeleted || filesDeleted || filesFailedToBeDeleted)
            PROGLOG(LOGDBGHK "Deleted dirs: %d, files: %d, files failed to be deleted: %d", dirsDeleted, filesDeleted, filesFailedToBeDeleted);
    }
