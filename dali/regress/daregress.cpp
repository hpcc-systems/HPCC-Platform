...
static void testDFS()
{
    ...
    dir.removeEntry("daregress::superfile1",UNKNOWN_USER);
    ...
    for (i=0;i<100;i++) {
        ...
        dir.removeEntry(s.str(),UNKNOWN_USER);
        StringBuffer cname;
        ...
        s.clear().append("daregress::test").append(i);
        dfile->attach(s.str(),UNKNOWN_USER);
    }
    ...
    for (i=0;i<100;i++) {
        ...
        if (!dir.exists(s.str(),UNKNOWN_USER))
            ERROR1("Could not find %s",s.str());
        Owned<IDistributedFile> dfile = dir.lookup(s.str(),UNKNOWN_USER);
        if (!dfile) {
            ERROR1("Could not find %s",s.str());
            continue;
        }
...
*/
    ...
    sfile.setown(dir.createSuperFile("daregress::superfile1",UNKNOWN_USER,true));
    for (i = 0;i<100;i++) {
        s.clear().append("daregress::test").append(i);
        sfile->addSubFile(s.str());
    }
    sfile.clear();
    sfile.setown(dir.lookupSuperFile("daregress::superfile1",UNKNOWN_USER));
    if (!sfile) {
...
static bool setupDFS()
{
    ...
    if (dir.exists("regress::trans::super1",UNKNOWN_USER,false,true) && !dir.removeEntry("regress::trans::super1",UNKNOWN_USER)) {
        ERROR("Can't remove super1");
        return false;
    }
    if (dir.exists("regress::trans::super2",UNKNOWN_USER,false,true) && !dir.removeEntry("regress::trans::super2",UNKNOWN_USER)) {
        ERROR("Can't remove super2");
        return false;
    }
    if (dir.exists("regress::trans::super3",UNKNOWN_USER,false,true) && !dir.removeEntry("regress::trans::super3",UNKNOWN_USER)) {
        ERROR("Can't remove super3");
        return false;
    }

    if (dir.exists("regress::trans::sub1",UNKNOWN_USER,true,false) && !dir.removeEntry("regress::trans::sub1",UNKNOWN_USER)) {
    ...
    dsub1->attach("regress::trans::sub1",UNKNOWN_USER);
    dsub1.clear();

    if (dir.exists("regress::trans::sub2",UNKNOWN_USER,true,false) && !dir.removeEntry("regress::trans::sub2",UNKNOWN_USER)) {
    ...
    dsub2->attach("regress::trans::sub2",UNKNOWN_USER);
    dsub2.clear();

    if (dir.exists("regress::trans::sub3",UNKNOWN_USER,true,false) && !dir.removeEntry("regress::trans::sub3",UNKNOWN_USER)) {
    ...
    dsub3->attach("regress::trans::sub3",UNKNOWN_USER);
    dsub3.clear();

    if (dir.exists("regress::trans::sub4",UNKNOWN_USER,true,false) && !dir.removeEntry("regress::trans::sub4",UNKNOWN_USER)) {
    ...
    dsub4->attach("regress::trans::sub4",UNKNOWN_USER);
    dsub4.clear();
...
static void testDFSTrans()
{
    ...
    Owned<IDistributedSuperFile> sfile1 = dir.createSuperFile("regress::trans::super1", UNKNOWN_USER,false, false, transaction);
    ...
    sfile1.clear();
    sfile1.setown(dir.lookupSuperFile("regress::trans::super1", UNKNOWN_USER, transaction));
    if (!sfile1.get())
    ...
    transaction->start();
    Owned<IDistributedSuperFile> sfile2 = dir.createSuperFile("regress::trans::super2", UNKNOWN_USER, false, false, transaction);
    ...
    sfile2.clear();
    sfile2.setown(dir.lookupSuperFile("regress::trans::super2", UNKNOWN_USER, transaction));
    if (sfile2.get())
    ...
    transaction->start();
    Owned<IDistributedSuperFile> sfile3 = dir.createSuperFile("regress::trans::super3", UNKNOWN_USER, false, false, transaction);
    ...
    transaction->commit();
    sfile3.clear();
    sfile3.setown(dir.lookupSuperFile("regress::trans::super3", UNKNOWN_USER, transaction));
    if (!sfile3.get())
...
static void testDFSPromote()
{
    ...
    dir.promoteSuperFiles(3, sfnames, "regress::trans::sub1", delsub, createonlyone, UNKNOWN_USER, timeout, outlinked);
    {
        Owned<IDistributedSuperFile> sfile1 = dir.lookupSuperFile("regress::trans::super1", UNKNOWN_USER, NULL, timeout);
        if (!sfile1.get())
        ...
        Owned<IDistributedSuperFile> sfile2 = dir.lookupSuperFile("regress::trans::super2", UNKNOWN_USER, NULL, timeout);
        if (sfile2.get())
    ...
    dir.promoteSuperFiles(3, sfnames, "regress::trans::sub2", delsub, createonlyone, UNKNOWN_USER, timeout, outlinked);
    {
        Owned<IDistributedSuperFile> sfile1 = dir.lookupSuperFile("regress::trans::super1", UNKNOWN_USER, NULL, timeout);
        if (!sfile1.get())
            ERROR("promote failed, super1 doesn't exist");
        ...
        if (strcmp(sfile2->querySubFile(0).queryLogicalName(), "regress::trans::sub1") != 0)
            ERROR("promote failed, wrong name for sub1");
        Owned<IDistributedSuperFile> sfile3 = dir.lookupSuperFile("regress::trans::super3", UNKNOWN_USER, NULL, timeout);
        if (sfile3.get())
    ...
    dir.promoteSuperFiles(3, sfnames, "regress::trans::sub3", delsub, createonlyone, UNKNOWN_USER, timeout, outlinked);
    {
        Owned<IDistributedSuperFile> sfile1 = dir.lookupSuperFile("regress::trans::super1", UNKNOWN_USER, NULL, timeout);
        if (!sfile1.get())
            ERROR("promote failed, super1 doesn't exist");
    ...
    dir.promoteSuperFiles(3, sfnames, "regress::trans::sub4", delsub, createonlyone, UNKNOWN_USER, timeout, outlinked);
    {
        Owned<IDistributedSuperFile> sfile1 = dir.lookupSuperFile("regress::trans::super1", UNKNOWN_USER, NULL, timeout);
        if (!sfile1.get())
            ERROR("promote failed, super1 doesn't exist");
        ...
        Owned<IDistributedFile> sub1 = dir.lookup("regress::trans::sub1", UNKNOWN_USER, false, NULL, timeout);
        if (!sub1.get())
            ERROR("promote failed, sub1 was physically deleted");
    }

    printf("Promote ([1,2], 4, 3) - fifth iteration, two in-files\n");
    dir.promoteSuperFiles(3, sfnames, "regress::trans::sub1,regress::trans::sub2", delsub, createonlyone, UNKNOWN_USER, timeout, outlinked);
    {
        Owned<IDistributedSuperFile> sfile1 = dir.lookupSuperFile("regress::trans::super1", UNKNOWN_USER, NULL, timeout);
        if (!sfile1.get())
            ERROR("promote failed, super1 doesn't exist");
        ...
        if (!sub1.get())
            ERROR("promote failed, sub1 was physically deleted");
        Owned<IDistributedFile> sub2 = dir.lookup("regress::trans::sub2", UNKNOWN_USER, false, NULL, timeout);
        if (!sub2.get())
...
{
    IDistributedFileDirectory &dir = queryDistributedFileDirectory();
    Owned<IDistributedFileTransaction> transaction = createDistributedFileTransaction(UNKNOWN_USER); // disabled, auto-commit

    // Cleanup
    if (dir.exists("regress::del::super1",UNKNOWN_USER,false,true) && !dir.removeEntry("regress::del::super1",UNKNOWN_USER)) {
        ERROR("Can't remove super1");
        return;
    }
    if (dir.exists("regress::del::sub1",UNKNOWN_USER,false,true) && !dir.removeEntry("regress::del::sub1",UNKNOWN_USER)) {
        ERROR("Can't remove sub1");
        return;
    }
    ...
    Owned<IDistributedFile> dsub1 = dir.createNew(sub1);
    dsub1->attach("regress::del::sub1",UNKNOWN_USER);
    dsub1.clear();

    printf("Creating 'regress::del::super1 and attaching sub\n");
    Owned<IDistributedSuperFile> sfile1 = dir.createSuperFile("regress::del::super1", UNKNOWN_USER, false, false, transaction);
    sfile1->addSubFile("regress::del::sub1", false, NULL, false, transaction);
    sfile1.clear();

    printf("Deleting 'regress::del::sub1, should fail\n");
    try {
        if (dir.removeEntry("regress::del::sub1",UNKNOWN_USER)) {
    ...
    if (!dir.removeEntry("regress::del::super1",UNKNOWN_USER)) {
        ERROR("Can't remove super1");
        return;
    }
