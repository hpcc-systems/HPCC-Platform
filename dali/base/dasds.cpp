    ...
    virtual void read(const char *name, IPropertyTree &owner, MemoryBuffer &mb, bool withValue)
    {
        ...
        else
        {
            if (withValue)
            {
                ...
                CPTValue v(sz, vmb.toByteArray(), true, false, COMPRESS_METHOD_NONE, COMPRESS_METHOD_NONE);
                v.serialize(mb);
            }
            else
                mb.append((size32_t)0);
...
PDState CServerRemoteTree::checkChange(IPropertyTree &changeTree, CBranchChange &parentBranchChange)
{
    ...
    if (changeTree.getPropBool("@localValue"))
    {
        ...
        setValue(v?new CPTValue(v->queryValueRawSize(), v->queryValueRaw(), binary, true, v->getCompressionType(), COMPRESS_METHOD_NONE):NULL, binary);
    ...
    else if (changeTree.getPropBool("@appendValue"))
    {
        ...
        if (binary)
            setPropBin(NULL, mb.length(), mb.toByteArray(), COMPRESS_METHOD_NONE);
        else
        {
            mb.append('\0');
