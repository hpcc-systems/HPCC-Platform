...
inline unsigned getLfnHashFromPath(size32_t len, const char *path, unsigned initval = 0)
{
    while (len>0)
    {
        ...
        if (!slash)
        {
            ...
            initval = hashc((const unsigned char *)path, len, initval);
            break;
        }
        else
        {
            // Hash until path separator and hash logical filename separator separately
            initval = hashc((const unsigned char *)path, slash-path, initval);
            initval = hashc((const unsigned char *)"::", 2, initval);
            len -= (slash-path)+1;
            path = slash+1;
        }
