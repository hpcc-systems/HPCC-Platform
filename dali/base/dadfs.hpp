...
public:
    ...
    void updateFileProperties() const
    {
        for (auto & [logicalName, curStatItem] : stats)
        {
            ...
            // the following condition is to avoid re-writing unchanged file attributes
            // (it is not for thread-safety, as the updateFileProperties shouldn't be called
            //  while addCostAndNumReads may be called)
            if (curStatItem.lastWrittenNumReads != curStatItem.numDiskReads)
            {
                curStatItem.lastWrittenNumReads = curStatItem.numDiskReads;
                updateCostAndNumReads(curStatItem.file, curStatItem.lastWrittenNumReads, curStatItem.readCost);
            }
        }
    }
