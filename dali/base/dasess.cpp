...
#define CLDAPE_challengeresponsefailure (-3)
...
public:
    ...
    int exchangeChallengeResponse(ICoven &coven, CMessageBuffer &mb, StringBuffer &_username)
    {
        ...
        else if (now.compare(nextTrustedUserMapFlush) >= 0)
        {
    ...
    void processMessage(CMessageBuffer &mb)
    {
        ...
        case MSR_CR_LOOKUP_LDAP_PERMISSIONS:{
                ...
                int perms = 0;
                int err = 0;
                if (fn == MSR_CR_LOOKUP_LDAP_PERMISSIONS && useChallengeResponse)
                {
...
public:
    ...
    int getPermissionsLDAP(const char *key,const char *obj,IUserDescriptor *udesc,unsigned auditflags,int *err)
    {
        ...
        CMessageBuffer mb;
        if (useChallengeResponse && queryDaliServerVersion().compare("3.11") >= 0)
        {
            StringBuffer usr;
