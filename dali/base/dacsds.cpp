...
IPropertyTree *CClientRemoteTree::collateData()
{
    ...
    if ((CPS_Changed & state) || (0 == serverId && queryValue()))
    {
        ...
        if (queryValue())
        {
            ...
            ((PTree *)ct.queryTree())->setValue(new CPTValue(queryValue()->queryValueRawSize(), queryValue()->queryValueRaw(), binary, true, getCompressionType(), COMPRESS_METHOD_NONE), binary);
        }
        else
    ...
    else if (CPS_PropAppend & state)
    {
        ...
        ((PTree *)ct.queryTree())->setValue(new CPTValue(mb.length()-from, mb.toByteArray()+from, binary, false, COMPRESS_METHOD_NONE, COMPRESS_METHOD_NONE), binary);
    }
