...
const int DefValue_Failed_malloc = 701;
...
VarStringValue::VarStringValue(unsigned len, const char *v, ITypeInfo *_type) : CValue(_type)
{
    ...
    {
        char * temp = (char *)checked_malloc(typeLen+1, DefValue_Failed_malloc);
        memcpy(temp, v, len);
        temp[len] = 0;
...
void MemoryValue::deserialize(MemoryBuffer &src)
{
    size32_t size;
    src.read(size);
    void *mem = checked_malloc(size, DefValue_Failed_malloc);
    assertex(mem);
    src.read(size, mem);
    val.set(size, mem);
}
...
IValue *createStringValue(const char *val, ITypeInfo *type, int srcLength, ICharsetInfo *srcCharset)
{
    ...
    {
        char * extended = (char *)checked_malloc(tgtLength, DefValue_Failed_malloc);
...
IValue *createUnicodeValue(char const * value, ITypeInfo * type, unsigned srclen)
{
    ...
    UChar * buff = (UChar *)checked_malloc(type->getSize(), DefValue_Failed_malloc);
...
{
    free(text);
    text = (UChar *) checked_malloc((_len+1)*2, DefValue_Failed_malloc);
...
VarUnicodeValue::VarUnicodeValue(unsigned len, const UChar * v, ITypeInfo * _type) : CValue(_type)
{
    ...
    {
        UChar * temp = (UChar *)checked_malloc((typeLen+1)*2, DefValue_Failed_malloc);
        memcpy(temp, v, len*2);
        temp[len] = 0;
        val.set(temp, typeLen);
        free(temp);
...
void VarUnicodeValue::deserialize(MemoryBuffer & src)
{
    size32_t len;
    src.read(len);
    UChar * buff = (UChar *) checked_malloc(len*2, DefValue_Failed_malloc);
    src.read(len*2, buff);
    val.set(buff, len);
}
...
IValue *DataValue::castTo(ITypeInfo *t)
{
    ...
    case type_data:
        ...
        else
        {
            ...
            char *newstr = (char *) checked_malloc(nsize, DefValue_Failed_malloc);
...
const char *QStringValue::generateECL(StringBuffer &out)
{
    ...
    char * strData = (char *)checked_malloc(strLen, DefValue_Failed_malloc);
...
IValue *IntValue::castTo(ITypeInfo *t)
{
    ...
    case type_varstring:
    {
        ...
        char *newstr = (char *) checked_malloc(nLen, DefValue_Failed_malloc);
        if (type->isSigned())
...
{
    unsigned len = _type->getSize();
    val = (char *)checked_malloc(len, DefValue_Failed_malloc);
    memcpy(val, v, len);
}
...
void DecimalValue::deserialize(MemoryBuffer &src)
{
    size32_t size;
    src.read(size);
    val = checked_malloc(size, DefValue_Failed_malloc);
    assertex(val);
}
...
void appendValueToBuffer(MemoryBuffer & mem, IValue * value)
{
    ...
    unsigned len = type->getSize();
    void * temp = checked_malloc(len, DefValue_Failed_malloc);
    value->toMem(temp);
...
unsigned extractUnicode(IValue * in, rtlDataAttr & out)
{
    ...
    if (!out.refdata())
        throw MakeStringException(DefValue_Failed_malloc, "Unable to allocate %d bytes",2*bufflen);
