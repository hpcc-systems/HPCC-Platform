...
{
    switch (parent.modes[kind])
    {
    case ExpansionMode::OnLoad:
        if (parent.estimating)
            return parent.estimates[kind].expanded;
        else
        {
            ActualHistory::const_iterator it = parent.actualHistory.find(key);
            if (it != parent.actualHistory.end())
    ...
    case ExpansionMode::OnLoadToOnDemand:
        return parent.estimates[kind].compressed;
