...
{
public:
    CEventFilter(CEventConsumingOp& _operation) : operation(_operation) {}
...
protected:
    ...
    {
        FilterTerm(CEventConsumingOp& _operation) : operation(_operation) {}
            ...
            case EvAttrPath:
                {
                    ...
                    const char* filePath = operation.queryMetaParser().queryFilePath(fileId);
...
protected:
    ...
    FilterTerm* ensureTerm(EventAttr id)
    {
        ...
        if (!terms[id])
            terms[id].setown(new term_type_t(operation));
#if defined(_DEBUG)
...
IEventFilter* createEventFilter(CEventConsumingOp& operation)
{
    return new CEventFilter(operation);
}

IEventFilter* createEventFilter(const IPropertyTree& config, CEventConsumingOp& operation)
{
    Owned<IEventFilter> filter = createEventFilter(operation);
