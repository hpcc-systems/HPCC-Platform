...
protected:
    ...
    void onIndexCacheHit(std::vector<CEvent> &events)
    {
        ...
        if (!page.cacheHit)
        {
            ...
            events.push_back(event); // copy before transforming the hit to a miss to preserve load attributes
            CEvent &reclassified = events.at(0); // 'event' can be invalidated by the push_back
            CEvent &simulated = events.at(1);
            reclassified.transform(EventIndexCacheMiss);
            simulated.transform(EventIndexLoad);
            ...
            if (memory.caching(reclassified))
            {
    ...
    void onIndexCacheMiss(std::vector<CEvent> &events)
    {
        ...
        if (page.cacheHit)
        {
            event.transform(EventIndexCacheHit);
            switch (page.expansionMode)
            {
            case ExpansionMode::OnLoad:
                event.setValue(EvAttrInMemorySize, page.expanded.size);
...
protected:
    ...
    public:
        void reportDropped(const IndexHashKey &key, __uint64 size) override
        {
            ...
            eviction.transform(EventIndexEviction);
