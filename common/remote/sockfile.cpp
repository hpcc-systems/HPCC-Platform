...
class CRemoteFileServer : public CInterface, implements IRemoteFileServer, implements IThreadFactory
{
    ...
    unsigned throttleLimit, throttleDelayMs, throttleCPULimit;
...
public:
    ...
    bool cmdSetThrottle(MemoryBuffer & msg, MemoryBuffer & reply,CRemoteClientHandler &client)
    {
        ...
        try
        {
            ...
            if (_throttleLimit > throttleLimit)
            {
                PROGLOG("Increasing throttleLimit from %d to %d", throttleLimit, _throttleLimit);
                unsigned diff = _throttleLimit - throttleLimit;
                throttlesem.signal(diff);
            ...
            {
                unsigned diff = throttleLimit - _throttleLimit;
                unsigned prevLimit = throttleLimit;
                ...
                {   /* disabling - set limit immediately to let all new transaction through
                     * but still need to consume the signals, to avoid issues if throttling changed again
                     */
                    throttleLimit = 0;
                }
                ...
                while (diff)
                {
                    if (throttlesem.wait(1000))
                        ...
                        PROGLOG("Waited %d seconds so far for a total of %d transactions to complete, %d completed", timer.elapsedMs(), prevLimit, diff);
                }
                throttleLimit = _throttleLimit;
