...
ViewFile * ViewFileWeb::walkFile(const char * filename, IDistributedFile * alreadyResolved, ViewWalkOptions & options)
{
    ...
    Owned<IDistributedFile> resolved = alreadyResolved ? LINK(alreadyResolved) : directory.lookup(filename,udesc,AccessMode::readLogicalMeta,false,true,nullptr,defaultPrivilegedUser); // lock super-owners
    if (!resolved)
        return NULL;
