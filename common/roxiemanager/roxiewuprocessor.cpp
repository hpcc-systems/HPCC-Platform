...
//          MTimeSection timing(NULL, "entering info in dali");
        ...
        if (numParts > 1)
        {
            ...
            ForEach(*patches)
            {
                ...
                patchTree->addPropTree("BaseIndex", baseIndex);

                file->addPropTree("Patch", patchTree);
            }
        }

        file->setProp("@id", fileName);

        IPropertyTree &fdesc_tree = fdesc->queryProperties();

        StringBuffer renamedFrom;

        if (processingInfo.getUseRenamedFileInfo())
            renamedFrom.append(fdesc_tree.queryProp("@renamedFrom"));

        if (renamedFrom.length())
        {
            StringBuffer xpath;
            // break up the comma separated list and put it into xml
            const char *ptr = renamedFrom.str();
            const char *current = renamedFrom.str();

            while (ptr = strchr(current, ','))
            {
                StringBuffer temp( ptr-current, current);
                xpath.appendf("<name id='%s'/>", temp.str());
                ptr++;
                current = ptr;
            }

            xpath.appendf("<fileNames id='%s'/>", current);  // lets not forget the last (or only) one
        }

        const char *partmask = fdesc->queryPartMask();

        StringBuffer directory(fdesc->queryDefaultDir());

        StringBuffer common_dir_name;
        StringBuffer delim;
        StringBuffer directory_only;

        if (directory.length())
        {
            if (strchr(directory.str(), '/') != 0)
                delim.append('/');
            else if (strchr(directory.str(), '\\') != 0)
                delim.append('\\');

            if (directory[directory.length()-1] != delim.charAt(0))
                directory.append(delim.str());

            splitFilename(directory, NULL, &directory_only, &directory_only, &directory_only);
            // now lets remove the ~path from the input filename
            if (fileName.charAt(0) == '~')
            {
                const char *name = fileName.str()+1;
        
                const char *s1 = strstr(name, "::");
                if (s1)
                {
                    StringBuffer baseDir;
                    baseDir.append(s1 - name, name).trim();
                    baseDir.append(delim.str());  // always put on the directory slash
                    common_dir_name.append(strstr(directory_only.str(), baseDir.str()));
                }
                else
                {
                    if ((directory_only[0] == '/') || (directory_only[0] == '\\'))
                    {
                        delim.append(directory_only[0]);
                        common_dir_name.append(directory_only.str()+1);
                    }
                    else
                        common_dir_name.append(directory_only.str());
                }
            }
            else
                common_dir_name.append(directory);
        }

        if (fdesc->isCompressed())
            file->setPropBool("@isCompressed", true);
        file->setProp("@partmask", fdesc->queryPartMask());
        file->setPropBool("@memfile", preload);
        file->setPropInt("@numparts", numParts);
        file->setPropInt64("@crc", file_crc);
        file->setPropInt64("@recordCount", recordCount);
        file->setPropInt64("@size", totalSize);
        if (foreignIP.length())
            file->setProp("@lookupDali", foreignIP.str());
        if (formatCrc)
            file->setPropInt64("@formatCrc", formatCrc);

        MemoryBuffer diskMetaSerialized;
        fdesc->queryProperties().getPropBin("_record_layout", diskMetaSerialized);
        if (diskMetaSerialized.length())
            file->setPropBin("_record_layout", diskMetaSerialized.length(), diskMetaSerialized.toByteArray());
        ...
        for (unsigned partIdx=0; partIdx<numParts; partIdx++) 
        {
            ...
            part->setProp("@modified", modifiedDt);
            unsigned crc;
            if (partdesc->getCrc(crc))
                part->setPropInt64("@crc", crc);
            if (!isBaseIndex)  // baseIndex files MUST be local already - don't write out remote location info
            {
                unsigned numCopies = std::max( fdesc->numCopies(partIdx), 2U);   // only care about maximum of 2 locations at this time
                for (unsigned copy = 0; copy < numCopies; copy++)
                {
                    RemoteFilename r;
                    partdesc->getFilename(copy,r);
                    StringBuffer origName;
                    r.getRemotePath(origName);

                    const char *ptr = strstr(origName, common_dir_name.str());
                    if (ptr)
                        origName.setLength(ptr - origName.str());

                    if (delim.length() == 0)
                    {
                        if ((common_dir_name[common_dir_name.length()-1] == '/') ||
                            (common_dir_name[common_dir_name.length()-1] == '\\'))
            ...
            part->setPropInt("@num", partIdx+1);
            StringBuffer elem("Part_");
            elem.append(partIdx+1);
            file->addPropTree(elem.str(), part);
        }
        // add @directory at the end so we can add a '/' or '\' if one is needed
        if ((common_dir_name[common_dir_name.length()-1] != '/') &&
            (common_dir_name[common_dir_name.length()-1] != '\\'))
        {
            if (delim.length())
                common_dir_name.append(delim.str());
            else
            {
                if (strchr(common_dir_name, '/') != 0)
                    common_dir_name.append('/');
                else if (strchr(common_dir_name, '\\') != 0)
                    common_dir_name.append('\\');
                else if (strchr(directory_only.str(), '/') != 0)
                    common_dir_name.append('/');
                else if (strchr(directory_only.str(), '\\') != 0)
                    common_dir_name.append('\\');
        ...
        file->setProp("@directory", common_dir_name.str());
        xml->addPropTree(tagname.str(), file);
    }
