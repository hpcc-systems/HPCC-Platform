...
//
// The template parameter specifies the interface that the subclass needs this class to implement.
// In most cases, `IEventVisitor` will be the correct choice. However, if the subclass needs to
// implement a different interface, it may do so by specifying a different template parameter.
//
...
template <typename base_t = IEventVisitor>
class TDumpEventVisitor : public CInterfaceOf<base_t>
{
public:
...
// Concrete extension of TDumpEventVisitor<IPTreeEventVisitor> that uses an IPropertyTree to store
// visited data. The tree is constructed in memory and is available via the `queryTree` method
// after visitation.
class CPTreeEventVisitor : public TDumpEventVisitor<IPTreeEventVisitor>
{
...
IPTreeEventVisitor* createPTreeEventVisitor()
{
    return new CPTreeEventVisitor();
}

// Concrete extension of TDumpEventVisitor<> that writes visited data to an output stream. One line
// of text is produced for each event and attribute visited, plus additional lines for the file
// name, version, and number of bytes read from the file.
class CDumpTextEventVisitor : public TDumpEventVisitor<>
{
public:
    virtual bool visitFile(const char* filename, uint32_t version) override
    {
...
public:
    CDumpTextEventVisitor(std::ostream& _out)
        : out(_out)
    {
    }
...
// Extension of CDumpTextEventVisitor that outputs XML-formatted text. Header and Footer elements
// synthesized to hold the file name, version, and number of bytes read from the file.
class CDumpXMLEventVisitor : public CDumpTextEventVisitor
{
public:
    virtual bool visitFile(const char* filename, uint32_t version) override
    {
...
public:
    using CDumpTextEventVisitor::CDumpTextEventVisitor;
...
// Extension of CDumpTextEventVisitor that outputs JSON-formatted text. Header and Footer objects
// are synthesized to hold the file name, version, and number of bytes read from the file.
class CDumpJSONEventVisitor : public CDumpTextEventVisitor
{
public:
    virtual bool visitFile(const char* filename, uint32_t version) override
    {
        state = State::Header;
        openObject(true);
        openObject(DUMP_STRUCTURE_HEADER, true);
        doVisitHeader(filename, version);
        return true;
    }
    ...
    virtual Continuation visitEvent(EventType id) override
    {
        ...
        if (firstEvent)
        {
            openArray(DUMP_STRUCTURE_EVENT, false);
            openObject(true);
            firstEvent = false;
        }
        else
    ...
    virtual void departFile(uint32_t bytesRead) override
    {
        if (inHeader())
            ...
            closeArray();
        openObject(DUMP_STRUCTURE_FOOTER, false);
        doVisitFooter(bytesRead);
        closeObject();
        closeObject(true);
    }
...
public:
    ...
    inline void openObject(bool first)
    {
        openObject(nullptr, first);
    }

protected:
    void openObject(const char* name, bool first)
    {
        openContainer(name, "{ ");
    }

    void openArray(const char* name, bool first)
    {
        openContainer(name, "[ ");
    }
    ...
    void indent()
    {
        ...
        if (!firstFlags.empty())
        {
            for (size_t idx = 0; idx < firstFlags.size(); ++idx)
                markup.append("  ");
        }
    }
...
// Extension of CDumpTextEventVisitor that outputs YAML-formatted text. Header and Footer objects
// are synthesized to hold the file name, version, and number of bytes read from the file.
class CDumpYAMLEventVisitor : public CDumpTextEventVisitor
{
public:
    virtual bool visitFile(const char* filename, uint32_t version) override
    {
