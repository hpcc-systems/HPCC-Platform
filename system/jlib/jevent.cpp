...
EventContext queryEventContext(EventType event)
{
    switch (event)
    {
    case EventDaliChangeMode:
    case EventDaliCommit:
    case EventDaliConnect:
    case EventDaliEnsureLocal:
    case EventDaliGet:
    case EventDaliGetChildren:
    case EventDaliGetChildrenFor:
    case EventDaliGetElements:
    case EventDaliSubscribe:
        return EventCtxDali;
    ...
    default:
        if (EventNone < event && event < EventMax)
...
bool readEvents(const char* filename, IEventVisitor& visitor, IEventFilter* filter)
{
    CEventFileReader reader;
    if (filter)
    {
        filter->setTarget(visitor);
        return reader.traverse(filename, *filter);
    }
...
protected:
        ...
        bool matches(const char* value) const
        {
            ...
            for (const std::string& pattern : patterns)
            {
                ...
                else
                {
                    try
                    {
                        RegExpr re(pattern.c_str(), false);
                        if (re.find(value))
                            return true;
                    }
                    catch (IException* e)
                    {
                        // Ignore exception and try next pattern
                        e->Release();
                    }
                }
            }
    ...
    {
        bool acceptTrue{false};
        bool acceptFalse{false};

        bool matches(EventType evt, bool value) const override
        {
            return (value && acceptTrue) || (!value && acceptFalse);
        }
    ...
    struct UnsignedFilterTerm : public FilterTerm
    {
        ...
        bool acceptToken(const char* token) override
        {
            ...
            if (!delim)
            {
                __uint64 first = strtoull(token, &tmp, 0);
                if (*tmp == '\0')
                    return acceptRange(first, first);
            }
            else if (delim == token)
            {
                __uint64 last = strtoull(delim + 1, &tmp, 0);
                if (*tmp == '\0')
            ...
            else
            {
                ...
                if (*tmp == '-')
                {
                    ...
                    __uint64 last = strtoull(tmp + 1, &tmp, 0);
                    if (*tmp == '\0')
        ...
        bool acceptRange(__uint64 first, __uint64 last)
        {
            ...
            return accepted.insert(std::make_pair(last, first)).second;
        }
    };
...
public: // IEventVisitor
    ...
    virtual bool departEvent() override
    {
        ...
        {
            switch (attrInformation[id].type)
            {
            case EATbool:
            ...
            case EATnone:
            case EATmax:
                throw makeStringExceptionV(-1, "event attribute id %d has invalid type %d", int(id), int(attrInformation[id].type));
            default:
...
public:
    ...
    virtual bool acceptAttribute(EventAttr id, const char* values) override
    {
        ...
        else
        {
            ...
            case EATnone:
            case EATmax:
                throw makeStringExceptionV(-1, "event attribute id %d has an invalid type %d", int(id), int(attrInformation[id].type));
            default:
                throw makeStringExceptionV(-1, "event attribute id %d has an unknown type %d", int(id), int(attrInformation[id].type));
            }
        }
    }
...
protected:
    using AcceptedEvents = std::set<EventType>;
