...
//
// The principle target is to be record every index node lookup, load and eviction in roxie so that we can
...
//
// How balance being able to interpret the data stream without having to know what each message means?
//
// The events are recorded as a sequence of (tag, values), where the tags are part of a 2 byte enumeration, and
// each tag has an associated type - which includes the size.
// The output is a binary file containing a sequence of events.  This may be followed by a set of meta data e.g.
// what how ids using in the events map to physical filenames.
//
...
//
// Adding blocking to the mix:  Likely to be needed for the full solution (especiallyI'm not sure this is likely to be needed, but if it is...
...
void EventRecorder::startRecording(const char * optionsText, const char * optFilename)
{
    ...
    auto processOption = [this](const char * option, const char * valueText)
    {
        ...
        if (strieq(option, "threadid"))
            options = (options & ~ERFthreadid) | (valueBool ? ERFthreadid : ERFnone);
    };

    options = ERFthreadid; // set defaults
