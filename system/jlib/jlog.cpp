...
private:
    ...
    StringBuffer logDirSubdir;//user specified, appended at end of config log dir
    ...
    StringBuffer logDir;        //queryLogDir()
    StringBuffer aliasFileSpec; //queryLogDir()
    StringBuffer expandedLogSpec;//queryLogFileSpec()

private:
    void setDefaults()
    {
        rolling = true;
        enterQueueMode = false;
        append = true;
        flushes = true;
        msgFields = MSGFIELD_timeDate | MSGFIELD_msgID | MSGFIELD_process | MSGFIELD_thread | MSGFIELD_code;
...
public:
    ...
    CComponentLogFileCreator(IPropertyTree * _properties, const char *_component) : component(_component)
    {
        if (!component)
        {
            const char * err = "CComponentLogFileCreator:Missing Component Name";
            ERRLOG("%s",err);
            throw MakeStringException(3000,"%s",err); // 3000: internal error
        }
        setDefaults();
    ...
    void createAliasFile(bool _create)       { createAlias = _create; }
    ...
    void setQueueMode(const bool _queueMode) { enterQueueMode = _queueMode; }

    //ILogMsgHandler fields
    void setAppend(const bool _append)       { append = _append; }       //append to existing with same name
