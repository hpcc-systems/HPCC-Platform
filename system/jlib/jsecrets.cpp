...
IVaultManager *ensureVaultManager()
{
    ...
    if (!vaultManager)
    {
        // In this branch, vault definitions are treated as static for process lifetime.
        // Secret content may still rotate/remount and is handled by provider request/auth paths.
        // Newer branches may install a config update hook to rebuild/swap vaultManager
        // when vault configuration changes are expected at runtime.
...
{
    Owned<IVaultManager> vaultmgr = LINK(ensureVaultManager());
    if (isEmptyString(vaultId))
    {
