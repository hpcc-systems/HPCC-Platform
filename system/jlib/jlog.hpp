...
{
    //set methods
    virtual void setExtension(const char * _ext) = 0;
    virtual void setPrefix(const char * _prefix) = 0;
    virtual void setName(const char * _name) = 0;
    virtual void setPostfix(const char * _postfix) = 0;
    virtual void createAliasFile(bool _create) = 0;
    virtual void setAliasName(const char * _aliasName) = 0;
    virtual void setLogDirSubdir(const char * _subdir) = 0;
    virtual void setRolling(const bool _rolls) = 0;
    virtual void setQueueMode(const bool _queueMode) = 0;

    //ILogMsgHandler fields
    virtual void setAppend(const bool _append) = 0;
    virtual void setFlushes(const bool _flushes) = 0;
    virtual void setMsgFields(const unsigned _fields) = 0;

    //ILogMsgFilter fields
    virtual void setMsgAudiences(const unsigned _audiences) = 0;
    virtual void setMsgClasses(const unsigned _classes) = 0;
    virtual void setMaxDetail(const LogMsgDetail _maxDetail) = 0;
    virtual void setLocal(const bool _local) = 0;

    //query methods (not valid until logging started)
    virtual const char * queryLogDir() = 0;
    virtual const char * queryLogFileSpec() = 0;
    virtual const char * queryAliasFileSpec() = 0;

    virtual bool beginLogging() = 0;
};
