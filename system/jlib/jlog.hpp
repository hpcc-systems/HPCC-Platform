...
// INTERFACE must be IContextLogger or derivative
...
public:
    virtual void CTXLOG(const char *format, ...) const  __attribute__((format(printf, 2, 3)))
    {
        va_list args;
        va_start(args, format);
        this->CTXLOGva(MCdebugInfo, unknownJob, NoLogMsgCode, format, args);
        va_end(args);
    }
    ...
    void logOperatorException(IException *E, const char *file, unsigned line, const char *format, ...) const  __attribute__((format(printf, 5, 6)))
    {
        va_list args;
        va_start(args, format);
        this->logOperatorExceptionVA(E, file, line, format, args);
        va_end(args);
    }
};
