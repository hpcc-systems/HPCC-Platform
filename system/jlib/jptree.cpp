...
CPTValue::CPTValue(size32_t size, const void *data, bool binary, bool raw, CompressionMethod currentCompressType, CompressionMethod preferredCompressType)
{
    compressType = currentCompressType;
    if (!raw && (currentCompressType == COMPRESS_METHOD_NONE) && binary && size > PTREE_COMPRESS_THRESHOLD)
    {
        ...
        try
        {
            ...
            CompressionMethod compressMethod = (preferredCompressType != COMPRESS_METHOD_NONE) ? preferredCompressType : defaultBinaryCompressionMethod;
...
void PTree::setLocal(size32_t l, const void *data, bool _binary, CompressionMethod compressType)
{
    if (value) delete value;
    if (l)
        value = new CPTValue(l, data, _binary, false, COMPRESS_METHOD_NONE, compressType);
    else
        value = NULL;
    if (_binary)
...
void PTree::appendLocal(size32_t l, const void *data, bool binary)
{
    ...
    if (l)
        value = new CPTValue(l, data, binary, false, COMPRESS_METHOD_NONE, COMPRESS_METHOD_NONE);
    else
        value = NULL;
    if (binary)
        IptFlagSet(flags, ipt_binary);
...
void PTree::addPropBin(const char *xpath, size32_t size, const void *data)
{
    ...
    else
    {
        ...
        if (parent != this)
            ...
            setPropBin(path, size, data, COMPRESS_METHOD_NONE);
    }
}
...
void PTree::clone(IPropertyTree &srcTree, IPropertyTree &dstTree, bool sub)
{
    ...
    if (srcTree.isBinary(NULL))
    {
        ...
        dstTree.setPropBin(NULL, mb.length(), mb.toByteArray(), COMPRESS_METHOD_NONE);
    }
...
void PTree::addLocal(size32_t l, const void *data, bool _binary, int pos)
{
    ...
    IPTArrayValue *newValue = new CPTValue(l, data, _binary, false, COMPRESS_METHOD_NONE, COMPRESS_METHOD_NONE);
...
void _synchronizePTree(IPropertyTree *target, const IPropertyTree *source, bool removeTargetsNotInSource)
{
    ...
    {
        if (target->isBinary())
            target->setPropBin(NULL, srcMb.length(), srcMb.toByteArray(), COMPRESS_METHOD_NONE);
        else
...
void copyPropIfMissing(IPropertyTree & target, const char * targetName, IPropertyTree & source, const char * sourceName)
{
    if (source.hasProp(sourceName) && !target.hasProp(targetName))
    {
        if (source.isBinary(sourceName))
        {
            ...
            target.setPropBin(targetName, value.length(), value.toByteArray(), COMPRESS_METHOD_NONE);
        }
        else
...
void copyProp(IPropertyTree & target, IPropertyTree & source, const char * name)
{
    if (source.hasProp(name))
    {
        if (source.isBinary(name))
        {
            ...
            target.setPropBin(name, value.length(), value.toByteArray(), COMPRESS_METHOD_NONE);
        }
        else
