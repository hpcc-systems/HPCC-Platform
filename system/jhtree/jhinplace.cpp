...
bool CInplaceLeafWriteNode::add(offset_t pos, const void * _data, size32_t size, unsigned __int64 sequence)
{
    ...
    if (!hasSpace)
    {
        ...
        assertex(oldSize == nowSize);
        if (nowSize > maxBytes)
            throw MakeStringException(0, "Internal error: Leaf grew too large after ignoring row @%llu:%u (%u > %u)", getFpos(), hdr.numKeys, hdr.keyBytes, maxBytes);
...
void CInplaceLeafWriteNode::write(IFileIOStream *out, CRC32 *crc)
{
    ...
    if (hdr.keyBytes > maxBytes)
        throw MakeStringException(0, "Internal error: Inplace leaf node @%llu is too large (%u > %u)", getFpos(), hdr.keyBytes, maxBytes);
