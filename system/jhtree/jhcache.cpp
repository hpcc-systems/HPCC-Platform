...
# endif
        ...
        DBGLOG("mck: sizeof(struct CacheSet): %lu sizeof(struct CacheEntry): %lu cache total mem size: %lu",
                sizeof(struct CacheSet), sizeof(struct CacheEntry), numEntries * sizeof(struct CacheEntry));
    ...
    virtual bool read(unsigned fileId, offset_t offset, size32_t size, CCachedIndexRead & nodeData) override
    {
        ...
        for (uint8_t ix=0; ix<16; ix++)
        {
            if ( (cache[findex].cSet[ix].fileId == fileId) && (cache[findex].cSet[ix].offset == alignedPos) )
            {
                ...
                int rc;
                if (useDirect)
                    rc = pread64(cacheFd, alignedAddr, pageSize, cacheOffset);
                else
                    rc = pread64(cacheFd, data, pageSize, cacheOffset);
#endif
                if (rc == pageSize)
                {
                    if (useDirect)
    ...
    virtual void write(unsigned fileId, offset_t alignedPos, const byte * data) override
    {
        ...
        int rc;
        if (useDirect)
            rc = pwrite64(cacheFd, alignedAddr, pageSize, cacheOffset);
        else
            rc = pwrite64(cacheFd, data, pageSize, cacheOffset);
#endif
