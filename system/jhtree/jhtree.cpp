...
public:
    ...
    const CJHTreeNode *getNodeX(const INodeLoader *key, unsigned keyID, offset_t pos, NodeType type, IContextLogger *ctx, bool isTLK);
...
const CJHSearchNode *CKeyIndex::getRootNode() const
{
    ...
    Owned<const CJHSearchNode> root = (const CJHSearchNode *) nodeCache->getNodeX(this, iD, rootPos, type, NULL, isTopLevelKey());
    ...
    if (root && isTopLevelKey() && !root->isLeaf() && root->getNumKeys()==1)
    {
        ...
        root.setown((const CJHSearchNode *) nodeCache->getNodeX(this, iD, rootPos, NodeLeaf, NULL, true));
    }
...
const CJHSearchNode *CKeyIndex::getNodeY(offset_t offset, NodeType type, IContextLogger *ctx) const
{ 
    latestGetNodeOffset = offset;
    //Call isTLK() rather than isTopLevelKey() so the test is inlined (rather than a virtual)
    return (CJHSearchNode *)cache->getNodeX(this, iD, offset, type, ctx, isTLK());
}

const CJHSearchNode *CKeyIndex::getNodeYY(const INodeLoader *nodeLoader, offset_t offset, NodeType type, IContextLogger *ctx) const
{ 
    latestGetNodeOffset = offset;
    //Call isTLK() rather than isTopLevelKey() so the test is inlined (rather than a virtual)
    return (CJHSearchNode *)cache->getNodeX(nodeLoader, iD, offset, type, ctx, isTLK());
}
...
offset_t CKeyIndex::queryFirstBranchOffset()
{
    ...
    for (unsigned nextBranch = 2; ; nextBranch++)
    {
        ...
        if (nextBranch == branchDepth)
            return branchOffset;
        IContextLogger * ctx = nullptr;
        cur.setown(getNodeY(branchOffset, NodeBranch, ctx));
    }
}
...
bool CKeyIndex::prewarmPage(offset_t offset, NodeType type)
{
    try
    {
        Owned<const CJHTreeNode> page = getNodeY(offset, type, nullptr);
        return page != nullptr;
    }
    catch(IException *E)
    {
...
const CJHSearchNode *CKeyIndex::locateFirstLeafNode(IContextLogger *ctx) const
{
    ...
    if (leafOffset != (offset_t)-1)
    {
        if (leafOffset == 0)
            return nullptr;
        return getNodeY(leafOffset, NodeLeaf, ctx);
    }
    ...
    while (!cur->isLeaf())
    {
        ...
        cur = getNodeY(cur->getFPosAt(0), type, ctx);
        assertex(cur);
        prev->Release();
    }
    return cur;
...
const CJHSearchNode *CKeyIndex::locateLastLeafNode(IContextLogger *ctx) const
{
    ...
    while (!cur->isLeaf())
    {
        ...
        depth++;
        NodeType type = (depth < getBranchDepth()) ? NodeBranch : NodeLeaf;
        cur = getNodeY(cur->nextNodeFpos(), type, ctx);
        assertex(cur);
        prev->Release();
    }
    ...
    for (;;)
    {
        ...
        cur = getNodeY(cur->nextNodeFpos(), NodeLeaf, ctx);
        if (!cur)
            return last;
        ::Release(last);
    }
...
bool CKeyCursor::_next(IContextLogger *ctx)
{
    ...
    else
    {
        ...
        if (!node->isKeyAt(++nodeKey))
        {
            ...
            if (rsib != 0)
            {
                node.setown(getNodeZ(rsib, type, ctx));
                if (node != NULL)
                {
                    nodeKey = 0;
    ...
    for (;;)
    {
        ...
        if (node->isLeaf())
        {
            ...
            else
            {
                ...
                node.setown(getNodeZ(nextPos, NodeLeaf, ctx));
                nodeKey = 0;
            }
            if (node)
                return true; 
        ...
        else
        {
            if (a<node->getNumKeys())
            {
                ...
                node.setown(getNodeZ(npos, type, ctx));
            }
            else
                return false;
        }
    ...
    for (;;)
    {
        ...
        if (node->isLeaf())
        {
            ...
            else
            {
                ...
                node.setown(getNodeZ(prevPos, NodeLeaf, ctx));
                if (node)
                    nodeKey = node->getNumKeys()-1;
            }
            if (node)
...
void CKeyCursor::deserializeCursorPos(MemoryBuffer &mb, IContextLogger *ctx)
{
    ...
    if (!eof)
    {
        ...
        if (nodeAddress)
        {
            node.setown(getNodeZ(nodeAddress, NodeLeaf, ctx));
...
const CJHTreeNode *CNodeCache::getNodeX(const INodeLoader *keyIndex, unsigned iD, offset_t pos, NodeType type, IContextLogger *ctx, bool isTLK)
{
