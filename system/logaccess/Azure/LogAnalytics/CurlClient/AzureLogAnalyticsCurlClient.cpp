...
static void requestLogAnalyticsAccessToken(StringBuffer & token, const char * clientID, const char * clientSecret, const char * tenantID)
{
    ...
    if (curlHandle)
    {
        ...
        {
            DBGLOG("%s: Access token request: curlResponse == CURLE_OK", COMPONENT_NAME);

            long httpResponseCode;
            curl_easy_getinfo(curlHandle, CURLINFO_RESPONSE_CODE, &httpResponseCode);
            DBGLOG("%s: Access token request: HTTP response code %ld", COMPONENT_NAME, httpResponseCode);
            ...
            if(httpResponseCode == 200L) // HTTP OK
            {
                if (captureBuffer.length() > 0)
                {
                    try
                    {
                        ...
                        Owned<IPropertyTree> tokenResponse = createPTreeFromJSONString(std::string(captureBuffer.toByteArray(), captureBuffer.length()).c_str());
                        DBGLOG("%s: Access token request: RAW response: '%s'", COMPONENT_NAME, std::string(captureBuffer.toByteArray(), captureBuffer.length()).c_str()); 
            ...
            {
                VStringBuffer errorMessage("Unknown error");
                if (captureBuffer.length() > 0)
                {
                    // Try to parse the error response from Azure AD
                    // This is expected to be in JSON format with an "error" and "error_description" field
                    //Owned<IPropertyTree> errorResponse = createPTreeFromJSONString(std::string(captureBuffer.toByteArray(), captureBuffer.length()).c_str());
                    //if (errorResponse)
                    //{
                    //    const char * errorCode = errorResponse->queryProp("error");
                    //    const char * errorDesc = errorResponse->queryProp("error_description");
                    //    if (!isEmptyString(errorCode))
                    //        errorMessage.setf("%s: %s", errorCode, nullText(errorDesc));
                    //    else
                    //        errorMessage.setf("Unknown Azure AD error: %s", nullText(errorDesc));
                    //}
                    //else
                    //{
                        // If we cannot parse the response, we will just use the raw response as the error message
                    //    DBGLOG("%s Access token request: Could not parse error response from Azure AD!", COMPONENT_NAME);
                    //}
                    ...
                    */
                    errorMessage.set(std::string(captureBuffer.toByteArray(), captureBuffer.length()).c_str());
                    DBGLOG("%s: Access token request: Raw error message: '%s' ", COMPONENT_NAME, errorMessage.str());
                }

                switch (httpResponseCode)
                {
                    case 400L:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (400): Bad Request - Request is badly formed and failed (permanently): '%s'", COMPONENT_NAME, errorMessage.str());
                    case 401L:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (401): Unauthorized - Client needs to authenticate first: '%s'", COMPONENT_NAME, errorMessage.str());
                    case 403L:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (403): Forbidden - Client request is denied: '%s'", COMPONENT_NAME, errorMessage.str());
                    case 404L:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (404): NotFound - Request references a non-existing entity. Ensure configured tenantID is valid!: '%s'", COMPONENT_NAME, errorMessage.str());
                    case 408L:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (408): Timeout - Request has timed out: '%s'", COMPONENT_NAME, errorMessage.str());
                    case 500L:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (500): Internal Server Error - Azure AD service found an error while processing the request: '%s'", COMPONENT_NAME, errorMessage.str());
                    case 502L:
                    case 503L:
                    case 504L:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (%ld): Gateway/Service Unavailable/Timeout - Temporary Azure AD or network issues: '%s'", COMPONENT_NAME, httpResponseCode, errorMessage.str());
                    default:
                        throw makeStringExceptionV(-1,"%s Access token request: Error (%ld): '%s'", COMPONENT_NAME, httpResponseCode, errorMessage.str());
                }
            }
        }
        else
