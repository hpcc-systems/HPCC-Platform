...
void AzureBlobLogAccess::getMinReturnColumns(std::string & columns)
{
...
void AzureBlobLogAccess::getDefaultReturnColumns(std::string & columns)
{
...
void AzureBlobLogAccess::getAllColumns(std::string & columns)
{
    columns.append( " \"*\" ");
}

AzureBlobLogAccess::AzureBlobLogAccess(const std::vector<std::string> &hostUrlList, IPropertyTree & logAccessPluginConfig)
// : m_esClient(hostUrlList)
{
...
IRemoteLogAccessStream * AzureBlobLogAccess::getLogReader(const LogAccessConditions & options, LogAccessLogFormat format, unsigned int pageSize)
{
    ...
    std::string queryIndex;
    //populateQueryStringAndQueryIndex(queryString, queryIndex, options);
    return new AzureBlobLogStream(queryString, m_esConnectionStr.str(), queryIndex.c_str(), format, pageSize, DEFAULT_SCROLL_TIMEOUT);
}
...
extern "C" IRemoteLogAccess * createInstance(IPropertyTree & logAccessPluginConfig)
{
    ...
    return new AzureBlobLogAccess({elasticSearchConnString}, logAccessPluginConfig);
}
