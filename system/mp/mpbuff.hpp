...
public:
    ...
    StringBuffer &getDetails(StringBuffer &buf)
    {
        ...
        return buf.appendf("CMessageBuffer(%8X) tag=%d, sender=%s, replytag=%d, size== %d, data head = %s", (int)(long)toByteArray(), (int)tag, sender.getEndpointHostText(ep).str(), (int)replytag, length(), data.str());
    }
