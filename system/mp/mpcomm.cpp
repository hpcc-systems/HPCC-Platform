...
#define CONNECT_TIMEOUT_INTERVAL 3000 // 3 secs
...
struct SocketEndpointV4A
{
    byte ip[4];
    unsigned short port;
    unsigned short random;
    SocketEndpointV4A() { };
    SocketEndpointV4A(const SocketEndpoint &val) { set(val); }
    void set(const SocketEndpoint &val)
    {
        port = val.port;
        random = (getRandom() % USHRT_MAX);
        if (val.getNetAddress(sizeof(ip),&ip)!=sizeof(ip))
            IPV6_NOT_IMPLEMENTED();
    }
    void get(SocketEndpoint &val)
    ...
    bool connect(CTimeMon &tm)
    {
        ...
        while (!channelsock) {
            try {
                ...
                // waiting for other side to send confirm) and decide who stops waiting based on random number sent.
                StringBuffer epStr;
                unsigned startMs = msTick();
                int loopCnt = (CONNECT_READ_TIMEOUT / CONNECT_TIMEOUT_INTERVAL) + 1;
                while(loopCnt-- > 0)
                {
                    {
                        CriticalBlock block(attachsect);
#ifdef _TRACE
                        PROGLOG("MP: connect got attachsect, attachchk = %d, loopCnt = %d", atomic_read(&attachchk), loopCnt);
#endif
                        if (atomic_read(&attachchk) > 0)
                        {
                            ...
                            {
#ifdef _TRACE
                                PROGLOG("MP: deadlock situation [] attachrand = %u rand = %u", attachrand, id[0].random);
#endif
                                if (attachrand == id[0].random)
                                {
                                    // if by small chance both randoms are equal then keep trying for an additional
                                    // [hopefully] different amount of time so that one gives up before the other
                                    loopCnt = (getRandom() % 30) + 1;
#ifdef _TRACE
                                    PROGLOG("MP: rand numbers are equal ! - changing timeout, loopCnt now equals %d", loopCnt);
#endif
                                }
                                else if (attachrand < id[0].random)
                                    break;
                            }
                        }
                    }
                    ...
                    {
#ifdef _TRACE
                        PROGLOG("MP: loop exception code = %d, loopCnt = %d", e->errorCode(), loopCnt);
#endif
                ...
                if (reply!=0) {
                    ...
                    if (attachSocket(newsock,remoteep,hostep,true,NULL,id[0].random)) {
                        newsock->Release();
#ifdef _TRACE
...
public:
    ...
    bool attachSocket(ISocket *newsock,const SocketEndpoint &_remoteep,const SocketEndpoint &_localep,bool ismaster,size32_t *confirm, unsigned short rand);
...
bool CMPChannel::attachSocket(ISocket *newsock,const SocketEndpoint &_remoteep,const SocketEndpoint &_localep,bool ismaster,size32_t *confirm, unsigned short rand) // takes ownership if succeeds
{
    struct attachdTor
    {
        atomic_t &attchk;
...
#ifdef _FULLTRACE       
    PROGLOG("MP: attachSocket on entry, ismaster = %d, confirm = %p, channelsock = %p, rand = %u", ismaster, confirm, channelsock, rand);
#endif
    ...
    while (running) {
        ...
        if (sock) {
            try {
                ...
                if (!parent->lookup(remoteep).attachSocket(sock,remoteep,hostep,false,&rd,id[0].random)) {
#ifdef _FULLTRACE       
