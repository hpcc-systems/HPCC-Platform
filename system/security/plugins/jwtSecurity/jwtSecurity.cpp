...
class ScopePermission
{
    public:
        ...
        bool operator< (const ScopePermission& other)
        {
...
public:
    ...
    : CBaseSecurityManager(serviceName, (IPropertyTree*)nullptr), keyIsPublicKey(false)
    {
        ...
        if (secMgrCfg != nullptr)
        {
            ...
            secretsPath = secMgrCfg->queryProp("@secretsPath");
        ...
        if (secretsPath.empty())
            throw MakeStringException(-1, "CJwtSecurityManager: secretsPath not found in configuration");
    }
...
private:
    ...
    bool ensureKeyLoaded()
    {
        if ((keyContentsTimer.elapsedMs() > KEY_FILE_CHECK_INTERVAL_MS) || keyContents.empty())
        {
            ...
            try
            {
                ...
                getSecretValue(rawKeyContents, "system", secretsPath.c_str(), "key", true);

                if (rawKeyContents.isEmpty())
                    throw makeStringExceptionV(-1, "CJwtSecurityManager: Key at path '%s' is empty", secretsPath.c_str());

                keyContents = trailingStringTrim(trailingStringTrim(rawKeyContents.str()));
            ...
            catch(IException* e)
            {
                ...
                e->Release();
                throw makeStringExceptionV(code, "CJwtSecurityManager: Exception accessing key at path '%s': %s", secretsPath.c_str(), msg.str());
            }
        }
    ...
    static std::string generateNonce()
    {
        ...
        char                nonceBuff[NONCE_LEN + 1];

        for (unsigned int x = 0; x < NONCE_LEN; x++)
            nonceBuff[x] = nonceLetters[rand() % NONCE_LETTERS_LEN];
        nonceBuff[NONCE_LEN] = '\0';

        return std::string(nonceBuff, strlen(nonceBuff));
    }
    ...
    bool authenticate(ISecUser& user)
    {
        ...
        // Make sure we have the latest hash or public key
        ensureKeyLoaded();
        ...
        if (resourceName != nullptr)
        {
            switch (rtype)
            {
                ...
                case RT_FILE_SCOPE:
                    {
                        ...
                        if (resourceName && strlen(resourceName) > 14 && strncmp(resourceName, "hpccinternal::", 14) == 0)
                        {
                            if (stricmp(&resourceName[14], user.getName()) == 0)
                ...
                case RT_WORKUNIT_SCOPE:
                    {
                        if (resourceName[0] == '\0' || strcmp(user.getName(), resourceName) == 0)
                            accessFlag = SecAccess_Full;
                        else
...
private:
    mutable CriticalSection     crit;                       //!< Protects access to secretsPath contents
    ...
    std::string                 secretsPath;                //!< Secrets vault space or subdirectory name containing key; from configuration; @see ensureKeyLoaded()
