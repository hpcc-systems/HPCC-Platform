...
void Rijndael::keySched(UINT8 key[_MAX_KEY_COLUMNS][4])
{
    ...
    for(j = 0;(j < uKeyColumns) && ((UINT32)r <= m_uRounds); )
    {
...
void Rijndael::encrypt(const UINT8 a[16], UINT8 b[16])
{
    ...
    *((UINT32*)(b +12)) = *((UINT32*)T1[temp[3][0]])
                        ...
                        ^ *((UINT32*)T3[temp[1][2]]) 
                        ^ *((UINT32*)T4[temp[2][3]]);
    for(r = 1; (UINT32)r < m_uRounds-1; r++)
    {
...
inline Rijndael::KeyLength getAesKeyType(unsigned char keylen)
{
    ...
    {
        char errmsg[256];
        sprintf(errmsg, "AES Encryption error: %d is not a valid key length", keylen);
        throw errmsg;
    }
...
ZBuffer &aes_encrypt(const void *key, unsigned keylen, const void *input, unsigned inlen, ZBuffer &output)
{
    Rijndael rin;
    Rijndael::KeyLength keyType = getAesKeyType(keylen);
    
    rin.init(Rijndael::CBC, Rijndael::Encrypt, (const UINT8 *)key, keyType);
    int len = rin.padEncrypt((const UINT8 *)input, inlen, (UINT8 *) output.clear().reserve(inlen + 16));
    ...
    {
        char errmsg[256];
        sprintf(errmsg, "AES Encryption error: %d, %s", len, getAesErrorText(len));
        throw errmsg;
    }
...
ZBuffer &aes_decrypt(const void *key, unsigned keylen, const void *input, unsigned inlen, ZBuffer &output)
{
    Rijndael rin;
    Rijndael::KeyLength keyType = getAesKeyType(keylen);

    rin.init(Rijndael::CBC, Rijndael::Decrypt, (const UINT8 *)key, keyType);
    int len = rin.padDecrypt((const UINT8 *)input, inlen, (UINT8 *) output.clear().reserve(inlen));
    ...
    {
        char errmsg[256];
        sprintf(errmsg, "AES Decryption error: %d, %s", len, getAesErrorText(len));
        throw errmsg;
    }
