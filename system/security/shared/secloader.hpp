    ...
    static ISecManager* loadSecManager(const char* model_name, const char* servicename, IPropertyTree* cfg)
    {
        if (model_name)
        {
            ...
            else if(stricmp(model_name, "htpasswd") == 0)
            {
                ...
                newSecManager_t_ xproc = NULL;
                xproc = (newSecManager_t_)GetSharedProcedure(ldapseclib, "newHtpasswdSecManager");

                if (xproc)
                    return xproc(servicename, *cfg);
                else
                    throw MakeStringException(-1, "procedure newHtpasswdSecManager of %s can't be loaded", LDAPSECLIB);
           }
        }
