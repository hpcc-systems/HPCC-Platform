...
protected:
    ...
    void testCopy()
    {
        ...
        Owned<ILocalWorkUnit> embeddedWU = createLocalWorkUnit(
                // Note - generated by compiling dedupright.ecl from regression suite then running wuget ./a.out | sed "s/\"/'/g" | sed s/^/\"/ | sed s/$/\"/ then a little massaging/trimming
                ...
                "         <att name='definition' value='../testing/regress/ecl/dedupright.ecl(1,1)'/>"
                "         <att name='name' value='d'/>"
                "         <att name='_kind' value='148'/>"
                "         <att name='ecl' value='DATASET([{1,1},...], { unsigned8 a, unsigned8 b });&#10;'/>"
                "         <att name='recordSize' value='16'/>"
                "         <att name='predictedCount' value='7'/>"
                "        </node>"
                "        <node id='3' label='Dedup'>"
                "         <att name='definition' value='../testing/regress/ecl/dedupright.ecl(3,1)'/>"
                "         <att name='_kind' value='4'/>"
                "         <att name='ecl' value='DEDUP(b, right);&#10;'/>"
                "         <att name='recordSize' value='16'/>"
                "         <att name='predictedCount' value='1..7[tiny]'/>"
                "        </node>"
                "        <node id='4' label='Output&#10;Result #1'>"
                "         <att name='definition' value='../testing/regress/ecl/dedupright.ecl(1,1)'/>"
                "         <att name='name' value='dedupright'/>"
                "         <att name='definition' value='../testing/regress/ecl/dedupright.ecl(3,1)'/>"
                ...
                "    YQABCABiAAEIAAAYHgAAAHsgdW5zaWduZWQ4IGEsIHVuc2lnbmVkOCBiIH07Cg==   </SchemaRaw>"
                "  </Result>"
                " </Results>"
                " <Statistics>"
                "  <Statistic c='eclcc'"
        ...
        embeddedWU->setState(WUStateFailed); // To simplify deletion of the copied wu
        queryExtendedWU(createWu)->copyWorkUnit(embeddedWU, true);
        createWu->setState(WUStateFailed); // So we can delete it
        createWu->commit();
        createWu.clear();
        // Now try to re-read
        ...
        StringBuffer xml1, xml2;
        exportWorkUnitToXML(wu, xml1, false, false, false);
        exportWorkUnitToXML(embeddedWU, xml2, false, false, false);
        // Bit of work needed to get them to match:
        Owned<IPropertyTree> p1 = createPTreeFromXMLString(xml1);
        p1->removeProp("Statistics/Statistic[@kind='WhenCreated']");
        p1->removeProp("Debug/created_by");
        p1->removeProp("@isClone");
        p1->removeProp("@wuidVersion");
        p1->renameProp("/", "W_LOCAL");
        toXML(p1, xml1.clear(), 0, XML_Format|XML_SortTags);
        if (!streq(xml1, xml2))
        {
            int i;
            for (i = 0; xml1.charAt(i) && xml2.charAt(i)==xml1.charAt(i); i++)
                ;
            DBGLOG("Strings differ:\n%s\n%s\n", xml1.str()+i, xml2.str()+i);
        }
        ASSERT(streq(xml1, xml2));
        wu.clear();
