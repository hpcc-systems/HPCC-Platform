...
FilledRecs := join(peopleDataset, sequenceKey, left.id <> 0 AND left.id=right.sequence AND right.alias <> '',doJoin(left,right), left outer, limit(999, skip, fail(99, 'More than 999 matches')), atmost(left.id=right.sequence, 100));
output(FilledRecs);
