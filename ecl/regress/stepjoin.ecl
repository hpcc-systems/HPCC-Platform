...
f3in := JOIN([f1, f2], STEPPED(LEFT.seg = RIGHT.seg) AND count(rows(left)) < 3 and smallTest(LEFT.number, RIGHT.number), tn(LEFT, ROWS(LEFT)), SORTED(seg, letter), assert sorted);
f3oy := JOIN([f1, f2], STEPPED(LEFT.seg = RIGHT.seg) AND smallTest(LEFT.number, RIGHT.number), tn(LEFT, ROWS(LEFT)), LEFT ONLY, SORTED(seg, letter), assert sorted);
f3ou := JOIN([f1, f2], STEPPED(LEFT.seg = RIGHT.seg) AND smallTest(LEFT.number, RIGHT.number), tn(LEFT, ROWS(LEFT)), LEFT OUTER, SORTED(seg, letter), assert sorted);
