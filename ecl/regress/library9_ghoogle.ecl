...
export impGhoogleLibrary(string searchString) := module,library(GhoogleLibrary)
export queryText := searchString;
export parsed := parseLNQuery(searchString);
export annotated := annotateQuery(parsed);
export optimized := optimizeQuery(annotated);
export results := ExecuteFullQuery(optimized);
    end;

#workunit('name','Ghoogle');
BUILD(impGhoogleLibrary);
