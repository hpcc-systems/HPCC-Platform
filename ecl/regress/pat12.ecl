...
//rule NP := d n | NP PP;
rule NP := d n | use(NP) use('PP');         // both forms of use()
//rule VP := v NP | VP PP;
rule VP := v NP | SELF use(PP);             // use SELF and one form of use.
...
outfile1 := PARSE(infile,line,S,results,all,scan,skip(ws*),matched(all),use(PP),whole);
output(outfile1);
