...
r := allnodes(local(t1), limit(99, fail('Too many records returned remotely')));

t2 := topn(r, numResults, -relevance, doc, wpos);

output(t2);

r2 := allnodes(local(t1), limit(99999));

t3 := topn(r2, numResults, -relevance, doc, wpos);
