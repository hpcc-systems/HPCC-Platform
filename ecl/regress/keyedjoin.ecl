...
FilledRecs := join(peopleDataset, mainTable, left.id=right.sequence AND right.alias <> '',doJoin(left,right), KEYED(sequenceKey), limit(100,count));
output(FilledRecs);


FilledRecs2 := join(peopleDataset, sequenceKey, left.id=right.sequence, limit(100, skip), UNORDERED);
output(FilledRecs2);
