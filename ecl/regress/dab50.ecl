...
base_fln_1 := join(base_fn_1,distribute(o_lname,1),left.one=right.one,add_l
(left,right),local);
...
base_flpn_1 := join(base_fln_1,distribute(o_prange,1),left.one=right.one,add_p
(left,right),local);
...
base_flpsn_1 := join(base_flpn_1,distribute(o_street,1),left.one=right.one,add_s
(left,right),local);
...
base_flpszn_1 := join(base_flpsn_1,distribute(o_zips,1),left.one=right.one,add_z
(left,right),local);
...
base_flpszan_1 := join(base_flpszn_1,distribute
(o_age,1),left.one=right.one,add_a(left,right),local);
...
res := dedup
(base_flpszan+base_flpszan_1,fname,lname,prange,street,zips,age,all);
...
ilocal := dedup(sort(iterate(dfres,cnt(left,right),local),fname,-
cnt,local),fname,local);
