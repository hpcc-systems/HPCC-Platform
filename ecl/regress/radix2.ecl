...
// radix sort (using sort then distribute,merge)
s := SORT(in,key,LOCAL,UNSTABLE);
d := DISTRIBUTE(s,radix(key) DIV divisor,MERGE(key));

OUTPUT(d,,'terasortrad_out',OVERWRITE);
