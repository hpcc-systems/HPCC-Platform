...
taxiData := DATASET('~file::.::datadrive::dev::test_data::yellow_tripdata_2023-01.parquet', taxiRecord, TYPE(PARQUET: maxRowCountInTable(48800)));
OUTPUT(CHOOSEN(taxiData, 500), NAMED('TAXI_PARQUET'));
OUTPUT(COUNT(NOCOMBINE(taxiData)), NAMED('TAXI_PARQUET_COUNT'));

githubData := DATASET('~file::.::datadrive::dev::test_data::sandbox::github_data*.parquet', githubRecord, TYPE(PARQUET: maxRowCountInTable(48800)));
