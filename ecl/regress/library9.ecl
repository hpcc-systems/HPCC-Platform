...
export wordIndex        := index({ wordType word, documentId doc, segmentType segment, wordFlags flags, wordType original, wordPosType wpos } , { docPosType dpos}, NameWordIndex);
...
export docMetaIndex := index({ documentId doc }, { string filename, wordCountType numWords, dateType date, unsigned8 size }, NameDocMetaIndex);
...
termType        leftTerm;
termType        rightTerm;
dataset(termRecord) leftTermSet;
dataset(termRecord) rightTermSet;
...
documentId          doc;
dataset(wordPosRecord)  positions;
                END;
...
dataset(wordPosRecord) words;
        end;

export denormCandidateRecord :=
        record(candidateBaseRecord)
dataset(denormTermRecord) matches;
        end;
...
export normCandidateRecord :=
        record(candidateBaseRecord)
dataset(normTermRecord) matches;
        end;


//Combined form - may be what is actually needed for efficiency

export fullTermRecord :=
        record
termType        term;
wordPosType     wpos;
dataset(wordPosRecord) denormWords;
        end;

export fullCandidateRecord :=
        record(candidateBaseRecord)
dataset(fullTermRecord) matches;
        end;
...
termsRecord := { dataset(termRecord) terms; };
extraRecord := record(searchRecord)
dataset(stageRecord) stages;
dataset(termsRecord) leftTermsUsed;
dataset(termsRecord) rightTermsUsed;
termType termCount;
        end;
...
shared denormCandidateRecord wordsToDenormCandidate(minimalMatchRecord firstin, dataset(minimalMatchRecord) in, termType term, stageType stage) :=
        TRANSFORM
            SELF.stage := stage;
...
shared denormCandidateRecord combineDeNormCandidateTransform(stageType stage, denormCandidateRecord l, denormCandidateRecord r) := TRANSFORM
...
export userOutputMatchRecord := RECORD
    termType                term;
    docPosType              dpos;
END;

export userOutputRecord := record
    string  name;
...
export GhoogleLibrary(string searchString) := interface
export string queryText;
export dataset(searchRecord) parsed;
export dataset(searchRecord) annotated;
export dataset(searchRecord) optimized;
export dataset(userOutputRecord) results;
    end;
...
myQuery := GhoogleQuery(q);

output(myQuery.queryText);
