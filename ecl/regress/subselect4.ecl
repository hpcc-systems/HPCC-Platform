...
sortedChildren := sort(parentDataset.children, per_surname, -person_id);

firstChild := sortedChildren[1];
...
projectedFirstChild := table(sortedChildren, { per_name := per_surname + ',' + per_forename, id := person_id * 3; })[1];

rolledupParent2 := table(parentDataset, { projectedFirstChild.per_name, projectedFirstChild.id } );

output(rolledupParent2,,'out.d00');

filteredFirstChild := sortedChildren(person_id != 0)[1];

rolledupParent3 := table(parentDataset, { filteredFirstChild.person_id, filteredFirstChild.per_surname, filteredFirstChild.per_forename } );

output(rolledupParent3,,'out.d00');
