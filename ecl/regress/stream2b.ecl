...
namesRecord t2(namesRecord l) := TRANSFORM
    SELF.name := '!' + l.name;
END;

output(normalize(titles, blockedNames(left.title), t2(right)));
output(normalize(titles, linkedNames(left.title), t2(right)));
output(normalize(titles, streamedNames(left.title), t2(right)));
