...
personByAgeDesc1 := sort(persons, nofold(sqHouseExDs.addr[id]), dob);
output(sqHouseExDs, { addr, oldest := personByAgeDesc1[1].forename + ' ' + personByAgeDesc1[1].surname });

personByAgeDesc2 := sort(persons, nofold(xxid+id), nofold(sqHouseExDs.addr[id]), dob);
output(sqHouseExDs, { addr, oldest := personByAgeDesc2[1].forename + ' ' + personByAgeDesc2[1].surname });

personByAgeDesc3 := sort(persons, nofold(xxid+id), nofold(sqHouseExDs.filepos+id), dob);
