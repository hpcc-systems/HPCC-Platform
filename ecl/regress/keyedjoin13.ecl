...
keyBaseRecord := record
integer3        value := mainTable.blah.value;
              end;

keyDerivedRecord := record(baseRecord), maxlength(100)
string              extra := mainTable.blah.extra;
                end;

nameKey := INDEX(mainTable, { surname, forename, filepos }, 'name.idx');
sequenceKey := INDEX(mainTable, { sequence }, keyDerivedRecord, 'sequence.idx');
