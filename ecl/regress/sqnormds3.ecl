...
#if (mask & 0x08 != 0)
output(local(table(sqNamesTable3.books(sqNamesTable3.surname='Halliday'), { name, author, rating100 })));
#end

//Normalized, filter on both levels
#if (mask & 0x10 != 0)
output(local(table(sqNamesTable4.books(rating100>50, sqNamesTable4.surname='Halliday'), { name, author, rating100 })));
#end

//Normalized, filter on both levels - diff syntax, location of filter is optimized.
#if (mask & 0x20 != 0)
output(local(table(sqNamesTable5(surname='Halliday').books(rating100>50), { name, author, rating100 })));
#end
