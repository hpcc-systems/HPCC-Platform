...
void HqlGram::ensureMapToRecordsMatch(IHqlExpression * defaultExpr, HqlExprArray & args, const attribute & errpos, bool isRow)
{
    ...
    ForEachItemIn(i, args)
    {
        ...
        if (isGrouped(value) != isGrouped(defaultExpr))
            groupingDiffers = true;
        OwnedHqlExpr checked = checkEnsureRecordsMatch(defaultExpr, value, errpos, isRow);
        if (value != checked)
