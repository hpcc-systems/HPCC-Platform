...
IHqlExpression * HqlGram::checkIndexRecord(IHqlExpression * record, const attribute & errpos, OwnedHqlExpr & indexAttrs)
{
    ...
    return checkPackIndexRecord(record, nullptr, indexAttrs);
}

IHqlExpression * HqlGram::checkPackIndexRecord(IHqlExpression * record, IHqlExpression * optIndex, IHqlExpression * optIndexAttrs)
{
    IHqlExpression * packedAttr;
    unsigned payloadFields;
    if (optIndex)
    {
        packedAttr = optIndex->queryAttribute(packedAtom);
        payloadFields = numPayloadFields(optIndex);
    }
    else
    {
        packedAttr = queryAttributeInList(packedAtom, optIndexAttrs);
