...
static IPropertyTree * addSelect(IPropertyTree * xml, IHqlExpression * expr, bool isUsed, bool isKeyed, bool isPayload)
{
    ...
    if (!isUsed)
        field->setPropBool("@isUnused", true);
    if (isPayload)
...
IPropertyTree * SourceFieldUsage::createReport(const IPropertyTree * exclude) const
{
    ...
    expandSelects(entry, record, queryActiveTableSelector(), usedAll, firstPayload, numFields, numFieldsUsed);
    if (isKey(source))
    {
        #if 0
        ...
        if (getBoolAttribute(source, filepositionAtom, true))
        {
            if (!lastField->hasAttribute(_implicitFpos_Atom))
            {
                numFields++;
                if (usedFilepos || usedAll)
                {
                    if (includeFieldDetail)
                        addSelect(entry, lastField, true, false);
                    numFieldsUsed++;
                }
                else
                {
                    if (includeUnusedFields)
                        addSelect(entry, lastField, false, false);
                }
            }
        }
        else
...
void SourceFieldUsage::expandSelects(IPropertyTree * xml, IHqlExpression * record, IHqlExpression * selector, bool allUsed, unsigned firstPayload, unsigned & numFields, unsigned & numFieldsUsed) const
{
        ...
        case no_field:
            {
                ...
                if (cur->isDatarow())
                {
                    expandSelects(xml, cur->queryRecord(), selected, thisUsed, NotFound, numFields, numFieldsUsed);
                }
                else
                {
                    numFields++;
                    ...
                    {
                        if (includeFieldDetail)
                            addSelect(xml, selected, true, isKeyed, i >= firstPayload);
                        numFieldsUsed++;
                    }
                    else
                    {
                        if (includeUnusedFields)
                            addSelect(xml, selected, false, false, i >= firstPayload);
                    }
                }
                break;
            }
        case no_record:
            expandSelects(xml, cur, selector, allUsed, NotFound, numFields, numFieldsUsed);
            break;
        case no_ifblock:
            ...
            expandSelects(xml, cur->queryChild(1), selector, allUsed, NotFound, numFields, numFieldsUsed);
            break;
        }
    }
}
...
void SourceFieldTracker::analyseExpr(IHqlExpression * expr)
{
    ...
    bool wasInsideKeyed = insideKeyed;
