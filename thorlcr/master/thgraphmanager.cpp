...
bool CJobManager::executeGraph(IConstWorkUnit &workunit, const char *graphName, const SocketEndpoint &agentEp)
{
    ...
    Owned<IPropertyTree> memorySpecification = job->getMemorySettings("worker");
    job.clear();
    PROGLOG("Finished wuid=%s, graph=%s", wuid.str(), graphName);

    fatalHdlr->clear();

    runWorkunitAnalyser(workunit, getComponentConfigSP(), graphName, false, calculateThorCostPerHour(queryNodeClusterWidth()), memorySpecification);
    setWuid(NULL);
    return allDone;
}
 ...
 */
static int recvNextGraph(unsigned timeoutMs, StringBuffer &retWfid, StringBuffer &retWuid, StringBuffer &retGraphName, unsigned __int64 priority)
{
    dbgassertex(thorQueue);
