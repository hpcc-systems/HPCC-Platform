...
class CFileCache;
class CLazyFileIO : public CInterface, implements IDelayedFile
{
    CFileCache &cache;
...
public:
    ...
    void close()
    {
        Owned<IFileIO> _iFileIO = getClearFileIO();
        if (_iFileIO)
        {
            _iFileIO->close();
            mergeStats(fileStats, _iFileIO);
        }
    }
    unsigned __int64 getStatistic(StatisticKind kind)
    {
        ...
        Owned<IFileIO> _iFileIO = getFileIO();
        unsigned __int64 openValue = _iFileIO ? iFileIO->getStatistic(kind) : 0;
        return openValue + fileStats.getStatisticValue(kind);
    }
// IDelayedFile impl.
    virtual IMemoryMappedFile *queryMappedFile() override { throwUnexpected(); }
    virtual IFileIO *queryFileIO() override { throwUnexpected(); }
...
class CFileCache : public CInterface, implements IThorFileCache
{
    ...
    public:
        ...
        virtual IFileIO *queryFileIO() override { return this; }
        virtual IFileIO *getFileIO() override { return lFile->getOpenFileIO(activity); }
        // IFileIO impl.
