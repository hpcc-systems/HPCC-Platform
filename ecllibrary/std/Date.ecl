 ...
 * in effect locally (on the HPCC environment that is running the code).
 *
 * @return      TRUE if daylight savings time is currently in effect, FALSE otherwise.
 *
 * @see         IsUSDaylightSavingsInEffect()
 */
 ...
 *  - DST is treated as active starting on the start-transition date (inclusive)
 *  - and inactive starting on the end-transition date (exclusive).
 *
 ...
 *
 * @see     IsLocalDaylightSavingsInEffect()
 */
EXPORT BOOLEAN IsUSDaylightSavingsInEffect(Date_t date = CurrentDate(FALSE)) := DEFINE FUNCTION
    // Helper functions
    Date_t SundayDateBefore(Date_t d, UNSIGNED1 nthSunday = 1) := DEFINE FUNCTION
        delta := (DayOfWeek(d) - 1) + ((nthSunday - 1) * 7);
        RETURN AdjustDate(d, day_delta := -delta);
    END;
    Date_t SundayDateAfter(Date_t d, UNSIGNED1 nthSunday = 1) := DEFINE FUNCTION
