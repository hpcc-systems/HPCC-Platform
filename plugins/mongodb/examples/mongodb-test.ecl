...
dataset(mongodb.updateResultRecord) updateInfo(REAL salary, REAL raise) := EMBED(mongodb : user(user), password(pwd), server(server), database('mydb'),  collection('test1'))
...
dataset(layoutEmployee) findInfo(BOOLEAN mybool) := EMBED(mongodb : user(user), password(pwd), server(server), database('mydb'))
    runCommand(
        {
            findAndModify: "test2",
...
SEQUENTIAL
(
    OUTPUT(getAll(), NAMED('CountAllDocuments'));
