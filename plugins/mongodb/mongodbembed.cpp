...
static const char *MODULE_NAME = "mongodb";
static const char *MODULE_DESCRIPTION = "MongoDB Embed Helper";
static const char *VERSION = "MongoDB Embed Helper 1.0.0";
static const char *COMPATIBLE_VERSIONS[] = { VERSION, nullptr };
static const NullFieldProcessor NULLFIELD(NULL);

/**
 * @brief Takes a pointer to an ECLPluginEfinitionBlock and passes in all the important info
 * about the plugin. 
 */
...
namespace mongodbembed
{
     ...
     */
    void reportIfQueryFailure(const mongocxx::exception &e)
    {
    ...
    void deserializeEJSON(std::string &result, const char * row)
    {
        ...
        while (*start != '}' || depth > 1) 
        {
            if (*start == '{') // If we see a document increase the depth
            {
                depth++;
                lastBrkt = start; // Keep track of last bracket in case we need to backtrack
            }
            if (*start == '$' && *(start-1) == '\"') // Look for '$' to mark the start of a datatype
            { 
                auto end = start;
                while (*end != '\"') end++;
                auto key = std::string(start, end - start); // Get datatype
                result += std::string(row, lastBrkt - row); // Add everything before we went into nested document
                if (key == "$regularExpression") // Some data types are unsupported as they are not straightforward to deserialize
                {
                    UNSUPPORTED("Regular Expressions are currently Unsupported"); // TO DO handle unsupported types by not throwing an exception.
                } 
                else if (key == "$timestamp") 
                {
                    while (*end != '}') end++; // Skip over timestamp
                    row = ++end;
                    start = end;
                    result += "\"\"";
                } 
                else if (key == "$date" || key == "$oid") // Both of these get deserialized to strings and are surround by quotation marks
                {
                    end++;
                    while (*end != '\"') end++; // Move to opposite quotation mark
                    if (*(end+1) == '$') // The $date datatype can have a nested $numberLong and this checks for that
                    {
                        while (true) 
                        {
                            if (*end == '\"') 
                        ...
                        while (*end != '}') end++; // Get out of both nested documents
                        end++;
                    ...
                    else 
                    {
                        ...
                        while (*end != '}') end++; // Only have to get out of one nested document
                        end++;
                        depth--;
                        row = end; // Set row to just after the nested document
                        start = end; // move start to the next place for parsing
    ...
    {
        throw MakeStringException(-1, "UNSUPPORTED feature: %s not supported in %s", feature, VERSION);
    }
    ...
    void bindDataParam(unsigned len, const void *value, const RtlFieldInfo * field, std::shared_ptr<MongoDBQuery> query)
    {
        ...
        query->build()->append(kvp(std::string(field->name), std::string(data.getstr())));
    }
    ...
    void MongoDBEmbedFunctionContext::bindStringParam(const char *name, size32_t len, const char *val)
    {
        ...
        query->build()->append(kvp(std::string(name), bsoncxx::types::b_utf8{std::string(val)}));
    }
    ...
    void MongoDBEmbedFunctionContext::bindUnicodeParam(const char *name, size32_t chars, const UChar *val) 
    {
        ...
        query->build()->append(kvp(std::string(name), bsoncxx::types::b_utf8{std::string(utf8, utf8chars)}));
    }
    ...
    byte * MongoDBEmbedFunctionContext::getRowResult(IEngineRowAllocator * _resultAllocator)
    {
        Owned<MongoDBRowStream> mdbRowStream;
        mdbRowStream.setown(new MongoDBRowStream(_resultAllocator, query));
        return (byte *)mdbRowStream.getLink();
    }
    ...
    /**
     * @brief Returns the success of the operation.
     * 
     * @return true Result Rows has a length greater than 0.
     * @return false If no result was returned and the result rows are empty.
     */
    bool MongoDBEmbedFunctionContext::getBooleanResult() 
    {
        // If the operation returned a result then it was a success and we should return true
        return count > 0;
    }
    
    /**
     * @brief Returns the count of all the documents.
     * 
     * @return __int64 Length of result rows.
     */
     ...
     * @brief Returns the count of all the documents.
     * 
     * @return unsigned Length of result rows.
     */
    ...
    /* The use of having a single scalar return type is limited to a few useful situations.
    * Data, Real, String, UTF8, Unicode, and Decimal don't have any implementation currently 
    * because it is much simpler to get a dataset result since MongoDB always returns
    * results in document form.
    */
    ...
    void MongoDBEmbedFunctionContext::compileEmbeddedScript(size32_t chars, const char *script)
    {
        if (script && *script) 
        {
            ...
            if (len > 0) 
            {
                ...
                query->setEmbed(chars, queryScript.get()); // Now null terminated
     ...
     */
    template<typename T>
    void insertPair(T &builder, const bsoncxx::document::view &view, std::string &key, bool isRsvd, std::string value)
    {
    ...
    void MongoDBEmbedFunctionContext::bindKVP(T &builder, const bsoncxx::document::view &view, const char *&start)
    {
        ...
        while (*end != ':' && *end != '\"' && *end != ' ') end++; // pointer to end of key
        ...
        if (!isRsvd) key = key.substr(1); // Remove "$" for the key if it's in view
        ...
        else if (*start == '{') // Open subdocument
        {
            if (isRsvd) 
            {
            ...
            else 
            {
                ...
                else failx("Key must be type String.");
            }
        }
        else if (*start == '[') // Open subarray
        { 
            if (!isRsvd) 
            {
                if (view[key].type() == bsoncxx::type::k_utf8)
    ...
    void MongoDBEmbedFunctionContext::buildDocument(T &builder, const bsoncxx::document::view &view, const char *&start)
    {
        ...
        while (true)
        {
            if (findStr.find(*start) == std::string::npos) // If key is found bind pair
    ...
    void MongoDBEmbedFunctionContext::mdbinsert(mongocxx::collection &coll, bsoncxx::document::view &view, bsoncxx::document::value &doc_value)
    {
        ...
        try 
        {
            ...
            count++; // Increment Count for boolean result to return true
        } 
        catch (const mongocxx::exception& e) 
        {
            reportIfQueryFailure(e);
        }
    }
    ...
    void MongoDBEmbedFunctionContext::mdbfind(mongocxx::collection &coll, bsoncxx::document::view &view, bsoncxx::document::value &doc_value, std::string com)
    {
        ...
        else 
        {
            try 
            {
                ...
                while (*start == ' ') start++; // Move past whitespace if there is any
                if (*start == ',') // if there is a comma then we have a projection to build
                {
                ...
                for (auto&& doc : cursor) 
                {
                    ...
                    query->result()->append(deserialized.c_str());
                    count++; // Increment count as each result row is added. This is instead of calling length() as I feel it is more efficient.
                } 
            } 
            catch (const mongocxx::exception& e)
            {
    ...
    void MongoDBEmbedFunctionContext::mdbaggregate(mongocxx::collection &coll, bsoncxx::document::view &view, bsoncxx::document::value &doc_value)
    {
        ...
        try 
        {
            ...
            for (auto&& doc : cursor) 
            {
                ...
                deserializeEJSON(deserialized, bsoncxx::to_json(doc, bsoncxx::ExtendedJsonMode::k_relaxed).c_str());
                query->result()->append(deserialized.c_str());
                count++; // Increment count when the row is added
            }
        } 
        catch (const mongocxx::exception& e) 
        {
    ...
    void MongoDBEmbedFunctionContext::mdbrunCommand(mongocxx::database &db, bsoncxx::document::view &view, bsoncxx::document::value &doc_value)
    {
        ...
        try 
        {
            ...
            if (doc.view()["ok"].get_double() == double{1}) 
            {
                ...
                query->result()->append(deserialized.c_str());
                count++; // Increment count so that boolean result returns true
            }
        } 
        catch (const mongocxx::exception& e) 
        {
    ...
    void MongoDBEmbedFunctionContext::mdbcreateIndex(mongocxx::collection &coll, bsoncxx::document::view &view, bsoncxx::document::value &doc_value)
    {
        ...
        while (*start == ' ') start++; // Remove Whitespace
        if (*start == ',') // If there is a comma then we have an options document to build
        {
    ...
    void MongoDBEmbedFunctionContext::mdbupdate(mongocxx::collection &coll, bsoncxx::document::view &view, bsoncxx::document::value &doc_value, std::string com)
    {
        ...
        while (*start == ',' || *start == ' ') start++; // Move to next argument

        if (*start == '{') // Look for document ('{'), a pipeline ('['), or an error
        {
            builder.clear();
            buildDocument(builder, view, ++start);
        ...
        else failx("Incorrect Arguments given to update(). Expected: (bsoncxx::document, bsoncxx::document), (bsoncxx::document, mongocxx::pipeline).");
        if (result) // Check if there is a result document and extract useful fields 
        {
            StringBuffer json;
    ...
    void MongoDBEmbedFunctionContext::runQuery(mongocxx::database &db, mongocxx::collection &coll, bsoncxx::document::view &view, bsoncxx::document::value &doc_value) 
    {
        ...
        else if (com == "find_one" || com == "find") // Returns the first document found that matches the filter
        {
        ...
        else if (com == "runCommand") // Takes a MongoDB document and can run various commands at a database level.
        {
            mdbrunCommand(db, view, doc_value);
        } 
        else if (com == "aggregate") // Takes a MongoDB pipeline with various stages for chaining commands together
        {
    ...
    {
        if (m_oInputStream)
            m_oInputStream->executeAll(m_oMDBConnection, count);
        else 
        {
    ...
    public:
        ...
        virtual IEmbedFunctionContext * createFunctionContextEx(ICodeContext * ctx, const IThorActivityContext *activityCtx, unsigned flags, const char *options) override
        {
            ...
            else return new MongoDBEmbedFunctionContext(ctx ? ctx->queryContextLogger() : queryDummyContextLogger(), options, flags);
        }
