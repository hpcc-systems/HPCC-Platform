...
void loadLFS(const char * lfs, IUserDescriptor * user, StringBuffer &sb)
{
    ...
    try
    {
        Owned<IDistributedFile> df = queryDistributedFileDirectory().lookup(lfn, user, AccessMode::readSequential, false, false, nullptr, defaultPrivilegedUser);//scope checks
        if (!df)
        {
