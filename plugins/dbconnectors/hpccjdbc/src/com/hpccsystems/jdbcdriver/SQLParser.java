...
public class SQLParser
{
    ...
    public void process(String insql) throws SQLException
    {
        ...
        else if (insql.matches("^(?i)select\\s+(.*?)"))
        {
            ...
            /*
             * Allow multiple spaces between USE and INDEX? if (useindexstrpos >
             * 0) { String afterUSE = upperSql.substring(useindexstrpos+4); int
             * inderelativepos = afterUSE.indexOf("INDEX(");
             *
             * if(inderelativepos < 0) throw new SQLException
             * (" Malformed SQL statement near \'USE\' "); else
             * if(inderelativepos > 1) { String use [] =
             * afterUSE.split("INDEX"); if(use[0].trim().length() != 0) throw
             * new SQLException (" Malformed SQL statement near \'USE\' "); }
             *
             * }
             */
            ...
            if (wherePos != -1)
            {
                ...
                for (int i = 0; i < splitedwhereands.length; i++)
                {
                    ...
                    for (int y = 0; y < splitedwhereandors.length; y++)
                    {
                        ...
                        if (splitedsqlexp.length <= 0) // something
                                                       // went
                                                       // wrong,
                                                       // only
                                                       // the
