...
public class ECLEngine
{
    ...
    public ArrayList executeCall(Map parameters)
    {
        try
        {
            urlString = "http://" + props.getProperty("WsECLAddress") + ":" + props.getProperty("WsECLPort")
                    + "/WsEcl/submit/query/" + hpccQuery.getQuerySet() + "/" + hpccQuery.getName() + "/expanded";
                            ...
                            || currentdatsetname.equalsIgnoreCase(datasetName))
                    {
                        ...
                        if (rowList != null && rowList.getLength() > 0)
                        {
                            ...
                            for (int j = 0; j < rowList.getLength(); j++)
                            {
                                ...
                                {
                                    columnsArray.add(new HPCCColumn(columnList.item(k).getNodeName(), columnList
